...
function calleeName(expression: Node): string {
  if (Node.isIdentifier(expression)) return expression.getText();
  if (Node.isPropertyAccessExpression(expression)) return expression.getName();
  return expression.getText();
}

export function extractCallRelations(
  sourceFile: SourceFile,
  repositoryPath: string
): CallRelation[] {
  return sourceFile.getDescendantsOfKind(SyntaxKind.CallExpression).map((callExpression) => {
    const expression = callExpression.getExpression();
    return {
      calleeExpression: expression.getText(),
      calleeKind: SyntaxKind[expression.getKind()] ?? `SyntaxKind(${expression.getKind()})`,
      calleeName: calleeName(expression),
...
export async function discoverRepository(repositoryRoot = ROOT): Promise<DiscoveryReport> {
  const excluded = [
    '**/node_modules/**',
    '**/.git/**',
    '**/dist/**',
  ...
  const project = new Project({
    ...
    tsConfigFilePath: resolve(repositoryRoot, 'tsconfig.json')
  });
  ...
  for (const sourceFile of project.getSourceFiles()) {
    const repositoryPath = relative(repositoryRoot, sourceFile.getFilePath()).replaceAll('\\', '/');
    ...
    calls.push(...extractCallRelations(sourceFile, repositoryPath));
  }
