...
export interface GitHubCandidateObservationV1 {
  repository: string;
  number: number;
...
export interface GitHubReviewObservationV1 {
  id: string;
  authorNodeId: string;
  authorLogin: string;
  authorType: 'Bot' | 'User';
...
export interface GitHubReviewThreadObservationV1 {
  id: string;
  isResolved: boolean;
  isOutdated: boolean;
  path: string;
...
export interface GitHubReviewRequestObservationV1 {
  nodeId: string;
  login: string;
  kind: 'user' | 'team';
}

export interface GitHubAppReviewCommentObservationV1 {
  id: string;
  authorNodeId: string;
  appId: number;
  body: string;
...
export interface GitHubIssueCommentObservationV1 {
  id: string;
  body: string;
  authorLogin: string;
  authorId: number;
...
export type GitHubCommitResolutionObservationV1 = Readonly<{
  repository: string;
  locator: string;
  status: 'resolved';
  commitSha: string;
...
export interface GitHubCheckObservationV1 {
  id: number;
  name: string;
  status: string;
...
export interface GitHubWorkflowJobStepObservationV1 {
  name: string;
  number: number;
...
export interface GitHubWorkflowJobObservationV1 {
  id: string;
  runId: string;
  runAttempt: number;
  name: string;
...
export type VerificationSessionWorkflowJoinV1 = Readonly<{
  status: 'joined';
...
export const VERIFICATION_SESSION_ARTIFACT_PUBLICATION_WINDOW_MS = 10 * 60 * 1000;

export interface GitHubPrincipalObservationV1 {
  login: string;
  nodeId: string;
  permission: 'admin' | 'maintain' | 'write' | 'triage' | 'read' | 'none';
}

export interface GitHubComparisonObservationV1 {
  status: 'ahead' | 'behind' | 'diverged' | 'identical';
  behindBy: number;
}

export interface GitHubActionsArtifactObservationV1 {
  artifactId: string;
  artifactName: string;
...
export interface GitHubPageV1<T> {
  nodes: readonly T[];
  hasNextPage: boolean;
...
export interface GitHubPullRequestFileInventoryV1 {
...
export interface GitHubPullRequestFileInventoryExpectationV1 {
  readonly repository: string;
  readonly prNumber: number;
  readonly state: 'OPEN';
...
interface VerificationSessionGitHubTransport {
  ...
  workflowJobsForAttempt?(repository: string, runId: string, runAttempt: number): readonly GitHubWorkflowJobObservationV1[];
  ...
  pullRequestFileInventory?(
    repository: string,
    prNumber: number
  ): GitHubPullRequestFileInventoryV1;
  blobText?(repository: string, ref: string, path: string): string;
  comparison?(repository: string, baseSha: string, headSha: string): GitHubComparisonObservationV1;
  openPullRequestCountForHead?(repository: string, headSha: string): number;
  principal?(repository: string, login: string): GitHubPrincipalObservationV1;
  principalByNodeId?(repository: string, nodeId: string): GitHubPrincipalObservationV1;
  viewerPrincipal?(repository: string): GitHubPrincipalObservationV1;
  actionsArtifact?(repository: string, artifactId: string): GitHubActionsArtifactObservationV1;
  actionsArtifactsForRun?(repository: string, runId: string): readonly GitHubActionsArtifactObservationV1[];
  actionsArtifacts?(repository: string): readonly GitHubActionsArtifactObservationV1[];
  downloadArtifactText?(repository: string, artifact: GitHubActionsArtifactObservationV1, fileName: string): string;
}

export type GitHubReviewBarrierObservationV1 = Readonly<{
  status: 'clear';
  principal: ReviewPrincipalV1;
...
export interface PlatformEnforcementObservationV1 {
...
export const VERIFICATION_SESSION_REVIEW_LOCATOR_COMMENT_SCHEMA_V3 =
  'sec-verification-session-review-locator-comment-v3' as const;
export const VERIFICATION_SESSION_REVIEW_LOCATOR_COMMENT_MARKER_V3 =
  '<!-- sec-verification-session-review-locator-v3 -->' as const;
export const VERIFICATION_SESSION_REVIEW_WAKEUP_COMMENT_SCHEMA_V1 =
  'sec-verification-session-review-wakeup-comment-v1' as const;
export const VERIFICATION_SESSION_REVIEW_WAKEUP_COMMENT_MARKER_V1 =
...
export function shouldPublishMaintainerReviewWakeupV1(input: Readonly<{
...
function createReviewRequestComment(input: Omit<VerificationSessionReviewRequestCommentV2,
  ...
  if (!/^[A-Za-z0-9][A-Za-z0-9._-]{0,99}\/[A-Za-z0-9][A-Za-z0-9._-]{0,99}$/u.test(input.repository)
    ...
    fail('hosted Review request identity is invalid.');
  }
  const payload = reviewRequestPayload(input);
...
function parseReviewRequestComment(source: string): VerificationSessionReviewRequestCommentV2 | null {
  ...
  const suffix = '\n```';
  if (!source.startsWith(prefix) || !source.endsWith(suffix)) fail('hosted Review request comment shape is invalid.');
...
export interface VerificationSessionReviewLocatorCommentV3 {
  schema: typeof VERIFICATION_SESSION_REVIEW_LOCATOR_COMMENT_SCHEMA_V3;
  repository: string;
  sessionRevision: SessionDigest;
  operationId: SessionDigest;
...
function createReviewLocatorComment(input: Omit<VerificationSessionReviewLocatorCommentV3,
  'schema' | 'locatorDigest' | 'publicationDigest'>): VerificationSessionReviewLocatorCommentV3 {
  if (!/^[A-Za-z0-9][A-Za-z0-9._-]{0,99}\/[A-Za-z0-9][A-Za-z0-9._-]{0,99}$/u.test(input.repository)
    || !/^sha256:[0-9a-f]{64}$/u.test(input.sessionRevision)
    || !/^sha256:[0-9a-f]{64}$/u.test(input.operationId)
    || !Number.isSafeInteger(input.prNumber) || input.prNumber < 1
  ...
  const payload = Object.freeze({ schema: VERIFICATION_SESSION_REVIEW_LOCATOR_COMMENT_SCHEMA_V3,
...
function renderReviewLocatorComment(value: VerificationSessionReviewLocatorCommentV3): string {
  return `${VERIFICATION_SESSION_REVIEW_LOCATOR_COMMENT_MARKER_V3}\n` +
    `\`\`\`json\n${encodeVerificationActionDataV2(value)}\n\`\`\``;
}

function parseReviewLocatorComment(source: string): VerificationSessionReviewLocatorCommentV3 | null {
  if (!source.includes(VERIFICATION_SESSION_REVIEW_LOCATOR_COMMENT_MARKER_V3)) return null;
  const prefix = `${VERIFICATION_SESSION_REVIEW_LOCATOR_COMMENT_MARKER_V3}\n\`\`\`json\n`;
  const suffix = '\n```';
  ...
  if (Object.keys(record).sort().join('\0') !== expected.join('\0')
    || record.schema !== VERIFICATION_SESSION_REVIEW_LOCATOR_COMMENT_SCHEMA_V3) {
...
export interface VerificationSessionReviewWakeupCommentV1 {
  schema: typeof VERIFICATION_SESSION_REVIEW_WAKEUP_COMMENT_SCHEMA_V1;
  repository: string;
  sessionRevision: SessionDigest;
  operationId: SessionDigest;
...
function createReviewWakeupComment(input: Omit<VerificationSessionReviewWakeupCommentV1,
  'schema' | 'wakeupDigest'>): VerificationSessionReviewWakeupCommentV1 {
  if (!/^[A-Za-z0-9][A-Za-z0-9._-]{0,99}\/[A-Za-z0-9][A-Za-z0-9._-]{0,99}$/u.test(input.repository)
    || !/^sha256:[0-9a-f]{64}$/u.test(input.sessionRevision)
    || !/^sha256:[0-9a-f]{64}$/u.test(input.operationId)
  ...
  const payload = Object.freeze({ schema: VERIFICATION_SESSION_REVIEW_WAKEUP_COMMENT_SCHEMA_V1,
    repository: input.repository, sessionRevision: input.sessionRevision,
...
function renderReviewWakeupComment(value: VerificationSessionReviewWakeupCommentV1): string {
  return `@codex review\n\n${VERIFICATION_SESSION_REVIEW_WAKEUP_COMMENT_MARKER_V1}\n` +
    `\`\`\`json\n${encodeVerificationActionDataV2(value)}\n\`\`\``;
}

function parseReviewWakeupComment(source: string): VerificationSessionReviewWakeupCommentV1 | null {
  if (!source.includes(VERIFICATION_SESSION_REVIEW_WAKEUP_COMMENT_MARKER_V1)) return null;
  const prefix = `@codex review\n\n${VERIFICATION_SESSION_REVIEW_WAKEUP_COMMENT_MARKER_V1}\n\`\`\`json\n`;
  const suffix = '\n```';
  ...
  if (Object.keys(record).sort().join('\0') !== expected.join('\0')
    || record.schema !== VERIFICATION_SESSION_REVIEW_WAKEUP_COMMENT_SCHEMA_V1) {
...
function trustedHostedPublisher(comment: GitHubIssueCommentObservationV1): boolean {
  const policy = SEC_HOSTED_COMMENT_PUBLISHER_POLICY_V1;
...
function assertCursorProgress(previous: string | null, page: GitHubPageV1<unknown>, label: string): string | null {
  if (!page.hasNextPage) {
...
const GITHUB_OPEN_PULL_REQUEST_PAGE_SIZE_V1 = 100;
const GITHUB_OPEN_PULL_REQUEST_MAXIMUM_PAGES_V1 = 1000;

function githubOpenPullRequestPageV1(value: unknown, label: string): readonly unknown[] {
  if (!Array.isArray(value) || value.length > GITHUB_OPEN_PULL_REQUEST_PAGE_SIZE_V1) {
    fail(`${label} must be one bounded REST page.`);
  }
  return value;
}

type GitHubOpenPullRequestCensusRecordV1 = Readonly<{
  id: number;
  number: number;
  headSha: string;
}>;

function githubOpenPullRequestCensusRecordV1(
  raw: unknown,
  label: string
): GitHubOpenPullRequestCensusRecordV1 {
...
export function parseGitHubOpenPullRequestCensusV1(input: {
  repository: string;
  headSha: string;
  ...
  const parsePass = (source: string, label: string) => {
    ...
    if (!Array.isArray(rawPages) || rawPages.length < 1
      || rawPages.length > GITHUB_OPEN_PULL_REQUEST_MAXIMUM_PAGES_V1) {
      fail(`${label} page set is empty or overbound.`);
    }
    const pages = rawPages.map((rawPage, pageIndex) => {
      const page = githubOpenPullRequestPageV1(rawPage, `${label} page[${pageIndex}]`);
      const finalPage = pageIndex === rawPages.length - 1;
      if ((!finalPage && page.length !== GITHUB_OPEN_PULL_REQUEST_PAGE_SIZE_V1)
        || (finalPage && page.length === GITHUB_OPEN_PULL_REQUEST_PAGE_SIZE_V1)) {
        fail(`${label} page[${pageIndex}] does not prove complete pagination.`);
      }
      return page.map((raw, recordIndex) => githubOpenPullRequestCensusRecordV1(
        raw,
  ...
  const canonical = (records: readonly GitHubOpenPullRequestCensusRecordV1[]) =>
...
export function parseGitHubPullRequestFileInventoryV1(input: {
  repository: string;
  prNumber: number;
  beforeSource: string;
  pagesSource: string;
...
function classifyActionsArtifactAttemptAuthorityV1(
  ...
  if (SESSION_ATTEMPT_BOUND_ARTIFACT_PREFIXES_V1.some((prefix) => artifactName.startsWith(prefix))) {
...
type GitHubRepositoryActionsArtifactSummaryV1 = Readonly<{
  artifactId: string;
  artifactName: string;
  archiveDigest: SessionDigest | null;
  expired: boolean;
...
type GitHubRepositoryActionsArtifactInventoryV1 = Readonly<{
  ...
  artifacts: readonly GitHubRepositoryActionsArtifactSummaryV1[];
...
export function evaluateGitHubRepositoryActionsArtifactInventoryV1(input: Readonly<{
  repository: string;
  source: unknown;
  observeArtifact(summary: GitHubRepositoryActionsArtifactSummaryV1): GitHubActionsArtifactObservationV1;
}>): Readonly<{
  inventory: GitHubRepositoryActionsArtifactInventoryV1;
  artifacts: readonly GitHubActionsArtifactObservationV1[];
}> {
  repoParts(input.repository);
  ...
  const artifacts: GitHubRepositoryActionsArtifactSummaryV1[] = opaqueArtifacts.map((entry) => {
    const sessionIdentity = VERIFICATION_SESSION_ARTIFACT_NAME_PATTERN_V1.exec(entry.artifactName);
...
function normalizeReviews(reviews: readonly GitHubReviewObservationV1[]): readonly GitHubReviewObservationV1[] {
...
function normalizeThreads(threads: readonly GitHubReviewThreadObservationV1[]): readonly GitHubReviewThreadObservationV1[] {
  const ids = new Set<string>();
...
function normalizeReviewRequests(
  requests: readonly GitHubReviewRequestObservationV1[]
): readonly GitHubReviewRequestObservationV1[] {
...
function normalizeIssueComments(
  comments: readonly GitHubIssueCommentObservationV1[]
): readonly GitHubIssueCommentObservationV1[] {
  const ids = new Set<string>();
  return Object.freeze(comments.map((comment, index) => {
    ...
    if (trustedHostedPublisher(normalized)) {
      if (body.includes(VERIFICATION_SESSION_REVIEW_LOCATOR_COMMENT_MARKER_V3)) {
        const locator = parseReviewLocatorComment(body);
        if (locator === null) fail('trusted hosted Review locator marker did not parse.');
      } else if (body.includes(VERIFICATION_SESSION_REVIEW_REQUEST_COMMENT_MARKER)) {
        const request = parseReviewRequestComment(body);
        if (request === null) fail('trusted historical V2 Review request marker did not parse.');
...
function reviewOrder(left: GitHubReviewObservationV1, right: GitHubReviewObservationV1): number {
  return left.submittedAt.localeCompare(right.submittedAt) || left.id.localeCompare(right.id);
}

function commentOrder(left: GitHubIssueCommentObservationV1, right: GitHubIssueCommentObservationV1): number {
  return left.createdAt.localeCompare(right.createdAt)
    || (BigInt(left.id) < BigInt(right.id) ? -1 : BigInt(left.id) > BigInt(right.id) ? 1 : 0);
}

function reviewDecisionsByPrincipal(
  reviews: readonly GitHubReviewObservationV1[],
  headSha: string
): Map<string, GitHubReviewObservationV1> {
  const result = new Map<string, GitHubReviewObservationV1>();
...
function blockingReviewPrincipals(
  decisions: ReadonlyMap<string, GitHubReviewObservationV1>
): readonly string[] {
...
function assertResolution(
  resolution: GitHubCommitResolutionObservationV1,
  repository: string,
  locator: string
): GitHubCommitResolutionObservationV1 {
...
class VerificationSessionGitHubAdapter {
  ...
  private observeReviewBarrierUnchecked(input: {
    ...
    const resolverRecords: Array<Readonly<{
      comment: GitHubIssueCommentObservationV1;
      trustedApp: { actorNodeId: string; appId: number; appNodeId: string; appSlug: string };
      locator: string;
      clean: boolean;
      resolution: Extract<GitHubCommitResolutionObservationV1, { status: 'resolved' }>;
    ...
    for (const comment of issueComments) {
      ...
      const trustedApp = SEC_REVIEW_STABILITY_POLICY_V1.trustedApps.find((app) => (
    ...
    const waitingSnapshotDigest = createReviewSnapshotDigestV1({ ...snapshotBase,
  ...
  observeVerificationSessionWorkflowJoin(input: {
    ...
    baseSha: string;
    now?: string;
  }): VerificationSessionWorkflowJoinV1 {
    if (!Number.isSafeInteger(input.prNumber) || input.prNumber < 1
      || !/^sha256:[0-9a-f]{64}$/u.test(input.sessionRevision)
  ...
  observeChangedPaths(
    expected: GitHubPullRequestFileInventoryExpectationV1
  ): GitHubPullRequestFileInventoryV1 {
    ...
    if (encodeVerificationActionDataV2(paths) !== encodeVerificationActionDataV2(observed.paths)) {
  ...
  observeHostedReviewLocator(input: {
    ...
    const classify = (comments: readonly GitHubIssueCommentObservationV1[]) => {
      const matching: Array<{ comment: GitHubIssueCommentObservationV1;
        publication: VerificationSessionReviewLocatorCommentV3 }> = [];
      for (const comment of comments) {
        ...
        if (comment.body.includes(VERIFICATION_SESSION_REVIEW_REQUEST_COMMENT_MARKER)) {
          if (trustedHostedPublisher(comment)) parseReviewRequestComment(comment.body);
          continue;
        }
        if (!comment.body.includes(VERIFICATION_SESSION_REVIEW_LOCATOR_COMMENT_MARKER_V3)) continue;
...
export interface VerificationSessionReviewObservationTransactionV1 {
  candidate(repository: string, prNumber: number): GitHubCandidateObservationV1;
  reviewPage(repository: string, prNumber: number, after: string | null): GitHubPageV1<GitHubReviewObservationV1>;
  threadPage(repository: string, prNumber: number, after: string | null): GitHubPageV1<GitHubReviewThreadObservationV1>;
  reviewRequestPage(repository: string, prNumber: number, after: string | null): GitHubPageV1<GitHubReviewRequestObservationV1>;
...
export function evaluateVerificationSessionReviewObservationV1(
  transaction: VerificationSessionReviewObservationTransactionV1,
...
export function evaluateHostedReviewLocatorObservationV1(
...
export interface VerificationSessionWorkflowObservationTransactionV1 {
  workflowRunPage(
    ...
    headSha: string,
    after: string | null
  ): GitHubPageV1<GitHubWorkflowRunObservationV1>;
}

export function evaluateVerificationSessionWorkflowJoinV1(
  transaction: VerificationSessionWorkflowObservationTransactionV1,
...
export function evaluatePlatformEnforcementObservationV1(input: {
  repository: string;
  readRulesets(): unknown;
}): PlatformEnforcementObservationV1 {
  return new VerificationSessionGitHubAdapter({
...
export function evaluateVerificationSessionChangedPathsV1(
 ...
 */
export function parseGitHubWorkflowJobsForAttemptV1(input: {
  source: unknown;
  runId: string;
  runAttempt: number;
}): readonly GitHubWorkflowJobObservationV1[] {
  ...
  const jobs = (pages as Array<Record<string, any>>).flatMap((page) => page.jobs).map((job: any) => {
    ...
    const steps = job.steps.map((step: any) => {
      ...
      }) as GitHubWorkflowJobStepObservationV1;
    });
    const numbers = new Set<number>();
    ...
    return Object.freeze({ id: String(job.id), runId: String(job.run_id), runAttempt,
      ...
      steps: Object.freeze(steps) }) as GitHubWorkflowJobObservationV1;
  });
  const ids = new Set<string>();
...
function providerPageInfo(value: unknown, label: string): GitHubProviderPageInfoV1 {
  ...
  const pageInfo = value as GitHubProviderPageInfoV1;
...
export function parseGitHubReviewPagesV1(input: {
  source: unknown;
  resolveApp(appSlug: string): unknown;
}): Readonly<{ nodes: readonly GitHubReviewObservationV1[]; pageDigest: SessionDigest }> {
  ...
  const nodes = rawNodes.map((node, index) => {
    ...
    const actorApps = author.__typename === 'Bot'
      ? SEC_REVIEW_STABILITY_POLICY_V1.trustedApps.filter((app) => app.actorNodeId === author.id)
      : [];
...
export function parseGitHubReviewThreadPagesV1(input: {
  source: unknown;
  readCommentPage(threadId: string, after: string): unknown;
}): Readonly<{ nodes: readonly GitHubReviewThreadObservationV1[]; pageDigest: SessionDigest }> {
  if (!Array.isArray(input.source) || input.source.length === 0) {
...
function issueCommentObservation(value: any, label: string): GitHubIssueCommentObservationV1 {
...
class GhVerificationSessionTransport implements VerificationSessionGitHubTransport {
  ...
  pullRequestFileInventory(
    ...
    return parseGitHubPullRequestFileInventoryV1({ repository, prNumber,
  ...
  openPullRequestCountForHead(repository: string, headSha: string): number {
    ...
    const readPage = (page: number, label: string) => {
      const source = this.gh([
        ...
        '-f', `per_page=${GITHUB_OPEN_PULL_REQUEST_PAGE_SIZE_V1}`, '-f', `page=${page}`
      ], label);
      return Object.freeze({
        source,
        page: githubOpenPullRequestPageV1(parseJson<unknown>(source, label), label)
      });
    };
    const readCensus = (label: string): string => {
      const pages: unknown[][] = [];
      let complete = false;
      for (let page = 1; page <= GITHUB_OPEN_PULL_REQUEST_MAXIMUM_PAGES_V1; page += 1) {
        const observed = readPage(page, `${label} page ${page}`);
        pages.push([...observed.page]);
        if (observed.page.length < GITHUB_OPEN_PULL_REQUEST_PAGE_SIZE_V1) {
          complete = true;
          break;
        }
      }
    ...
    return parseGitHubOpenPullRequestCensusV1({ repository, headSha,
  ...
  actionsArtifact(repository: string, artifactId: string): GitHubActionsArtifactObservationV1 {
    ...
    const attemptAuthority = classifyActionsArtifactAttemptAuthorityV1(
      artifactName,
      canonicalRunId,
  ...
  actionsArtifacts(repository: string): readonly GitHubActionsArtifactObservationV1[] {
    ...
    return evaluateGitHubRepositoryActionsArtifactInventoryV1({ repository, source,
  ...
  workflowJobsForAttempt(
    ...
    return parseGitHubWorkflowJobsForAttemptV1({ source: pages, runId, runAttempt });
  }
