...
import type { GitHubCandidateObservationV1 } from './verification-session-github.ts';
import { integrationAuthorizationMergeMarkersV1 } from './verification-session-runtime.ts';

export const LOCAL_MAIN_CLOSEOUT_BINDING_SCHEMA_V3 = 'sec-local-main-closeout-binding-v3' as const;

export interface LocalMainCloseoutBindingV3 {
  readonly schema: typeof LOCAL_MAIN_CLOSEOUT_BINDING_SCHEMA_V3;
...
export type LocalMainCloseoutStatusV3 =
  | Readonly<{
...
export interface LocalMainGitResultV2 {
  readonly status: number;
  readonly stdout: string;
  readonly stderr: string;
}

export type LocalMainGitRunnerV2 = (
  repoRoot: string,
  args: readonly string[]
) => LocalMainGitResultV2;
...
function canonicalGitPathInventory(result: LocalMainGitResultV2, label: string): readonly string[] {
...
function shaOf(result: LocalMainGitResultV2): string | null {
  const value = result.stdout.trim();
  return result.status === 0 && /^[0-9a-f]{40}$/u.test(value) ? value : null;
}

export function createLocalMainCloseoutBindingV3(input: Omit<LocalMainCloseoutBindingV3,
  'schema' | 'bindingDigest'>): LocalMainCloseoutBindingV3 {
  const withoutDigest = Object.freeze({ schema: LOCAL_MAIN_CLOSEOUT_BINDING_SCHEMA_V3,
...
function blocked(binding: LocalMainCloseoutBindingV3 | null,
  reason: Extract<LocalMainCloseoutStatusV3, { status: 'LOCAL_MAIN_SYNC_BLOCKED' }>['reason'],
  detail: string, localHeadSha: string | null = null, localTreeSha: string | null = null,
  remoteMainSha: string | null = null, remoteMainTreeSha: string | null = null): LocalMainCloseoutStatusV3 {
...
function assertBinding(binding: LocalMainCloseoutBindingV3): LocalMainCloseoutBindingV3 {
  const current = createLocalMainCloseoutBindingV3(binding);
  if (binding.schema !== LOCAL_MAIN_CLOSEOUT_BINDING_SCHEMA_V3 || current.bindingDigest !== binding.bindingDigest) {
    throw new Error('local-main closeout binding digest mismatch.');
  }
  return current;
}

function inspectLocalMutationPreconditionsV2(
  repoRoot: string,
  binding: LocalMainCloseoutBindingV3,
  git: LocalMainGitRunnerV2
): LocalMainCloseoutStatusV3 | null {
...
export function inspectLocalMainCloseoutV3(
  repoRoot: string,
  sourceBinding: LocalMainCloseoutBindingV3,
  git: LocalMainGitRunnerV2
): LocalMainCloseoutStatusV3 {
  let binding: LocalMainCloseoutBindingV3;
  try {
...
export async function executeLocalMainCloseoutV3(
  repoRoot: string,
  sourceBinding: LocalMainCloseoutBindingV3,
  git: LocalMainGitRunnerV2,
  workspaceWriteLease: WorkspaceWriteLeaseToken
): Promise<LocalMainCloseoutStatusV3> {
  let binding: LocalMainCloseoutBindingV3;
  try { binding = assertBinding(sourceBinding); } catch (error) {
    return blocked(null, 'binding-invalid', error instanceof Error ? error.message : String(error));
  }
  const preflight = inspectLocalMutationPreconditionsV2(repoRoot, binding, git);
  ...
  const inspected = inspectLocalMainCloseoutV3(repoRoot, binding, git);
  if (inspected.status === 'LOCAL_MAIN_READY') return inspected;
  if (inspected.status !== 'LOCAL_MAIN_FF_ELIGIBLE') return inspected;
  await assertWorkspaceWriteLease(repoRoot, workspaceWriteLease);
  const commitFence = inspectLocalMutationPreconditionsV2(repoRoot, binding, git);
  ...
  if (merge.status !== 0) return blocked(binding, 'unresolved', `ff-only merge failed: ${merge.stderr.trim()}`,
    inspected.localHeadSha, inspected.localTreeSha, inspected.remoteMainSha, inspected.remoteMainTreeSha);
  const readback = inspectLocalMainCloseoutV3(repoRoot, binding, git);
...
export function createLocalMainCloseoutBindingFromHostedAuthorityV3(input: {
  readonly protectedRoot: string;
  readonly expectedLocalPreimageSha: string;
  readonly expectedLocalPreimageTreeSha: string;
  readonly livePublication: IntegrationAuthorizationOperationPublicationV1;
  readonly liveCommentId: number;
  readonly liveCandidate: GitHubCandidateObservationV1;
}): LocalMainCloseoutBindingV3 {
  const livePublication = parseIntegrationAuthorizationOperationPublicationV1(input.livePublication);
  ...
  return createLocalMainCloseoutBindingV3({
