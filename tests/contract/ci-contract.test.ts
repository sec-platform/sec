...
test('active GitHub validation workflows structurally enforce fresh exact heads and ordered gates', async () => {
  ...
  expect(contract.verificationContractRevision).toBe('ci-verification-v10');
  ...
  expect(prWorkflowSource).toContain('sec-verification-v10-${{ steps.verification.outputs.profile }}-pr-');
  ...
  expect(releaseWorkflowSource).toContain('sec-verification-v10-full-release-head-');
