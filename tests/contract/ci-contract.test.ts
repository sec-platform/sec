...
test('trusted bootstrap and release readers share the exact registry V2 epoch without merge authority', async () => {
  ...
  expect(releaseSource.match(/actions\/cache@0057852bfaa89a56745cba8c7296529d2fc39830/g)).toHaveLength(2);
  ...
  expect(bootstrapSource).not.toContain('generated-tcb-lock.json');
  for (const source of [bootstrapSource, releaseSource]) {
    expect(source).not.toContain('tcb-closure-lock --mode apply');
    expect(source).not.toContain('tcb-closure-lock --mode dry-run');
  }
  expect(bootstrapSource.indexOf('Check candidate TCB closure lock before bootstrap regression'))
    .toBeLessThan(bootstrapSource.indexOf('Run candidate bootstrap regression suite'));
  expect(bootstrapSource.indexOf('Run candidate bootstrap regression suite'))
    .toBeLessThan(bootstrapSource.indexOf('Check candidate TCB closure lock after bootstrap regression'));
});
