...
test('active GitHub validation workflows structurally enforce fresh exact heads and ordered gates', async () => {
  ...
  expect(contract.verificationContractRevision).toBe('ci-verification-v8');
  ...
  expect(prWorkflowSource).toContain('sec-verification-v8-${{ steps.verification.outputs.profile }}-pr-');
  ...
  expect(releaseWorkflowSource).toContain('sec-verification-v8-full-release-head-');
