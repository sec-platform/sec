...
test('external capability ledger binds current package authority without a self-referential main SHA', async () => {
  ...
  const ledger = parseYaml(ledgerSource) as {
    binding?: { lockAuthority?: string; packageAuthority?: string };
    providers?: Array<{
      decision?: string;
      id?: string;
      lifecycle?: string;
      observedVersion?: string | null;
      versionAuthority?: string | null;
  ...
  expect(ledger.binding).toEqual({ packageAuthority: 'package.json', lockAuthority: 'bun.lock' });
  expect(ledgerSource).not.toMatch(/\b[0-9a-f]{40}\b/u);
  expect(gitnexus?.observedVersion).toBe(packageJson.devDependencies?.gitnexus);
  expect(gitnexus?.versionAuthority).toBe('package.json#devDependencies.gitnexus');
