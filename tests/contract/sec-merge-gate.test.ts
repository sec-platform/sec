...
function fixture(
  ...
  const verificationName = `sec-verification-v12-${parsedManifest.requiredProfile}-pr-123-base-${BASE}-head-${HEAD}-run-200-attempt-1`;
...
async function p0MergeFixture() {
  ...
  const rawInput: CodexDevelopmentMergeGateInputV1 = {
    ...
    verificationArtifact: {
      id: 2000,
      name: `sec-verification-v12-quick-pr-113-base-${P0_BASE}-head-${P0_HEAD}-run-200-attempt-1`,
      digest: `sha256:${'5'.repeat(64)}`,
      expired: false,
      expiresAt: EXPIRES_AT,
      sizeInBytes: 2_048,
...
test('historical Work Package revisions remain parseable but cannot satisfy the current gate', () => {
  ...
  for (const legacyRevision of ['ci-verification-v11', 'ci-verification-v10', 'ci-verification-v9', 'ci-verification-v8', 'ci-verification-v6'] as const) {
    const legacyBytes = Buffer.from(manifestSource().replace('ci-verification-v12', legacyRevision));
...
test('trusted workflows pin actions, revalidate drift, and only materialize candidate Git objects as data', async () => {
  ...
  expect((mergeWorkflow.match(/sec-verification-v12-/gu) ?? []).length).toBe(2);
