...
function buildProgramProofCatalog(packageJson: unknown): readonly DevRunnerAuthorityScenario[] {
  ...
  add({
    scenarioId: 'independent-process-owner',
    label: 'an independent module may consume the shared process owner',
    boundedSource: boundedOwnerReplacement({
      modulePrelude: "import './independent-process-owner.ts';"
    }),
    extraModules: [{
      logicalPath: 'platform/dev-runner/independent-process-owner.ts',
      source: `
import { runCommandBytes } from '../shared/process.ts';
export async function observeIndependentRepository(): Promise<void> {
  await runCommandBytes('git', ['status', '--porcelain=v2', '-z'], { cwd: '.' });
}
`
    }],
    expectedValidation: 'accepted'
...
describe('dev-runner authority program', () => {
  test('finite suite Program proves exact catalog, partitions, bounds, and deterministic bytes', async () => {
    ...
    expect(new Set(scenarioIds).size).toBe(89);
    expect(suite.scenarioProofs).toHaveLength(89);
