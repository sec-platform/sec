...
test('retained byte-CAS deletion preserves changed content on the same FileId', () => {
  const f = fixture();
  try {
    const filePath = path.join(f.source, 'journal.json');
    writeFileSync(filePath, 'original\n');
    const root = inspectNoFollowDirectoryChain(f.source).target;
    const leaf = inspectNoFollowOrdinaryFileEntry(root, 'journal.json')!;
    ...
    if (process.platform === 'linux') {
      expect(() => deleteRetainedNoFollowEntry({
        ...deletion, expectedFileBytes: Buffer.from('changed!\n')
      })).toThrow('needs a native namespace exclusion on Linux');
      expect(readFileSync(filePath, 'utf8')).toBe('changed!\n');
      return;
    }
    expect(() => deleteRetainedNoFollowEntry({
      ...deletion, expectedFileBytes: Buffer.from('original\n')
    })).toThrow('file bytes changed');
    expect(readFileSync(filePath, 'utf8')).toBe('changed!\n');
    deleteRetainedNoFollowEntry({
      ...deletion, expectedFileBytes: Buffer.from('changed!\n')
    });
