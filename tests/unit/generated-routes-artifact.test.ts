...
test('compose consumes the generated routes artifact owner instead of a direct routes writer', async () => {
  const source = await readCompilerFile('platform/compiler/compose/compose-project.ts');

  expect(source).toContain('planGeneratedRoutesArtifactV1');
  expect(source).toContain('publishGeneratedRoutesArtifactV1');
  expect(source).toContain('GENERATED_ROUTES_ARTIFACT_PATH');
  expect(source).not.toContain('function renderRouteGraph');
  expect(source).not.toContain("new CodeBuilder('generated/routes.ts')");
  expect(source).not.toContain("writeText(path.join(generatedDir, 'routes.ts')");
});
