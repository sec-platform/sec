...
function gate(gateId: string, options: GateOptions = {}): VerificationGateResultV1 {
  ...
  const disposition = options.disposition ?? (status === 'passed' || status === 'failed'
    ? 'executed'
    : 'not-executed');
  ...
  const applicability = options.applicability ?? (
    status === 'invalidated' ? 'unresolved' :
      reasonCode === 'not-applicable' ? 'not-applicable' : 'required'
  );
...
function claim(
  ...
  owningEnvironments: string[] = ['linux-x64']
...
test('schema and canonical vocabularies remain fixed', () => {
  ...
  const applicability: VerificationApplicability[] = [
    'required', 'optional', 'not-applicable', 'unresolved'
  ];
  expect(new Set(statuses).size).toBe(5);
  expect(new Set(dispositions).size).toBe(3);
  expect(new Set(applicability).size).toBe(4);
});
...
test('reused pass requires Evidence and environment identity', () => {
  ...
  })).toThrow(/requires non-empty evidenceRefs/);

  expect(() => gate('reused-no-environment', {
    disposition: 'reused', env: null
  })).toThrow(/requires non-null environment identity/);

  const reused = gate('reused', {
    disposition: 'reused', env: environment('linux', 'x64'), evidenceRefs: ['ev-1']
  });
...
test('passed required gate must explicitly support every required claim', () => {
  expect(() => gate('missing-support', {
    claims: ['claim'], supportedClaims: []
  })).toThrow(/must support every required claim/);
  expect(() => gate('foreign-support', {
    claims: ['claim'], supportedClaims: ['other']
...
test('gate arrays and aggregate identities reject duplicates', () => {
  ...
  const first = gate('same-gate');
  const duplicate = gate('same-gate');
  expect(() => aggregate([claim('claim', ['same-gate'])], [first, duplicate]))
...
test('same logical gate may retain distinct owning-environment observations', () => {
  ...
  const result = aggregate([
    claim('claim', ['portable-gate'], ['windows-x64'])
  ], [linux, windows]);
...
test('wrong-environment pass cannot support an owning claim', () => {
  const linux = gate('windows-gate', { env: environment('linux', 'x64') });
  const result = aggregate([
    claim('windows-claim', ['windows-gate'], ['windows-x64'])
  ], [linux]);
...
test('failed, invalidated, unsupported and not-run preserve strict lattice priority', () => {
  ...
  const gates = [
    gate('g-not-run', {
      status: 'not-run', disposition: 'not-executed', reasonCode: 'not-dispatched', env: null,
      claims: ['a-not-run']
    }),
    gate('g-unsupported', {
      status: 'unsupported', disposition: 'not-executed', reasonCode: 'capability-unsupported', env: null,
      claims: ['b-unsupported']
    }),
    gate('g-invalidated', {
      status: 'invalidated', disposition: 'not-executed', reasonCode: 'selection-unresolved', env: null,
      claims: ['c-invalidated']
    }),
    gate('g-failed', {
      status: 'failed', reasonCode: 'cleanup-failed', claims: ['d-failed']
    })
  ];
...
test('aggregate status and reason are invariant under claim and gate permutations', () => {
  const claims = [
    claim('z-not-run', ['g-not-run']),
    claim('a-invalidated', ['g-invalidated'])
  ];
  const gates = [
    gate('g-not-run', {
      status: 'not-run', disposition: 'not-executed', reasonCode: 'not-dispatched', env: null,
    ...
    gate('g-invalidated', {
      status: 'invalidated', disposition: 'not-executed', reasonCode: 'input-invalidated', env: null,
      claims: ['a-invalidated']
    })
  ];
  const forward = aggregate(claims, gates);
  const reversed = aggregate([...claims].reverse(), [...gates].reverse());
...
test('cleanup failure after successful assertions remains failed', () => {
  const result = aggregate([claim('claim', ['test', 'cleanup'])], [
    gate('test'),
    gate('cleanup', { status: 'failed', reasonCode: 'cleanup-failed' })
  ]);
  expect(result.overallStatus).toBe('failed');
  expect(result.overallReasonCode).toBe('cleanup-failed');
});

test('optional unsupported claim stays visible without corrupting passing claim', () => {
  const result = aggregate([
    claim('browser', ['browser-gate']),
    claim('typescript', ['ts-gate'])
  ], [
    gate('browser-gate', {
      status: 'unsupported',
      disposition: 'not-executed',
    ...
    gate('ts-gate', { claims: ['typescript'] })
  ]);
  expect(result.claimResults.find((entry) => entry.claimId === 'typescript')!.status).toBe('passed');
  expect(result.claimResults.find((entry) => entry.claimId === 'browser')!.status).toBe('unsupported');
...
test('empty required claim set remains a vacuous pass', () => {
  const result = aggregate([], []);
...
test('legacy mappings never promote skipped, not-run or blocked to passed', () => {
  const productMappings = [
    mapProductVerificationStatus('skipped', {
      requestedLane: 'fast', lane: 'runtime'
    }),
    mapProductVerificationStatus('skipped', {
  ...
  for (const reason of [null, 'unknown', 'not-applicable', 'prerequisite', 'owning environment', 'artifact missing']) {
