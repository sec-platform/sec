...
test('public input is selected without enumerating internal fields',()=>{
 const input=new Proxy({rematerialize:false,get monotonicNowMs(){assert.fail('internal clock');},get generatedStateLifecycle(){assert.fail('lifecycle');}}, {ownKeys(){assert.fail('enumeration');}});
...
test('captured class method keeps private state after method replacement',async()=>{
 class Input{#calls=0;beforeCommit(){this.#calls++;}get calls(){return this.#calls;}}
