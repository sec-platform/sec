...
test('canonical Engineering Operation writer applies Plan operations outside Workbench', async () => {
  await withTempWorkspace(async (workspaceRoot) => {
    ...
    const results = await applyWithLease(workspaceRoot, [
      ...
      { id: 'add-proof', kind: 'add-acceptance', acceptanceId: 'acceptance.extra-proof' }
    ]);

    expect(results).toEqual([
      { id: 'rename-app', kind: 'set-app-name', status: 'applied', detail: 'set app.name to renamed-app' },
      { id: 'add-proof', kind: 'add-acceptance', status: 'applied', detail: 'added acceptance acceptance.extra-proof' }
    ]);
    ...
    expect(plan.acceptance.some((entry) => entry.id === 'acceptance.extra-proof')).toBe(true);
