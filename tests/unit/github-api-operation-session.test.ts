...
function capability(input: Readonly<{
  effect: 'read' | 'status-write' | 'issue-comment-write' | 'projection-comment-write' | 'repository-dispatch-write' | 'merge-write' | 'runner-admin' | 'branch-closeout-write';
...
test('workflow-scoped Actions principal is confined to projection reads and non-destructive comment publication', async () => {
  const api = capability({
    effect: 'projection-comment-write',
    principal: WORKFLOW_PRINCIPAL,
    transport: async (target, init) => Response.json({
      id: 91,
      full_name: 'sec-platform/sec',
  ...
  })).resolves.toMatchObject({ full_name: 'sec-platform/sec' });
  await expect(withGitHubApiTestSession({
    capability: api,
    operation: () => executeGitHubApiOperation(api, {
      kind: 'delete-issue-comment', commentId: 91
    })
  })).rejects.toThrow('projection-comment-write authority permits only fixed projection observations and effects');
  expect(() => capability({
    effect: 'issue-comment-write',
    principal: WORKFLOW_PRINCIPAL,
...
test('projection comment update uses PATCH but cannot inherit general comment retirement authority', async () => {
  const observed: Array<{ target: string; method: string; body: unknown }> = [];
  const api = capability({
    effect: 'projection-comment-write',
    principal: WORKFLOW_PRINCIPAL,
