...
const repositoryVerificationArtifactPaths = (() => {
  const paths = getWorkspacePaths(compilerRoot);
  return [
...
async function readRepositoryVerificationArtifacts(): Promise<Buffer[]> {
  return Promise.all(repositoryVerificationArtifactPaths.map((filePath) => fs.readFile(filePath)));
}

test('workbench server serves same-origin APIs and bounds source writes to current Lock slots', async () => {
  const repositoryArtifactsBefore = await readRepositoryVerificationArtifacts();
  await withTempWorkspace(async (workspaceRoot) => {
    ...
    try {
      ...
      expect(await resRun.text()).toContain('event: log');
      expect(await readRepositoryVerificationArtifacts()).toEqual(repositoryArtifactsBefore);
      ...
      for (const response of results) {
        ...
        expect(await response.text()).toContain('event: log');
      }
      expect(await readRepositoryVerificationArtifacts()).toEqual(repositoryArtifactsBefore);
    } finally {
      server.stop(true);
