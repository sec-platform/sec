...
async function withRepository<T>(callback: (root: string) => Promise<T>): Promise<T> {
  const parent = await isolationParent();
  const root = await fs.mkdtemp(path.join(parent, 'sec-generated-state-'));
  try {
...
async function isolationParent(): Promise<string> {
  isolationSequence += 1;
  const parent = path.join(
    process.cwd(),
    '.tmp',
    'test-workspaces',
    `unit-isolation-${process.pid}-${isolationSequence}`
  );
...
describe('generated-state runtime', () => {
  ...
  test('active owners are preserved and dead owners are automatically reclaimed', async () => {
    await withRepository(async (repositoryRoot) => {
      ...
      await expect(fs.access(activeRoot)).resolves.toBeDefined();
  ...
  test('cross-host and malformed owners are fail-closed and never reaped', async () => {
    await withRepository(async (repositoryRoot) => {
      ...
      await expect(fs.access(crossHostRoot)).resolves.toBeDefined();
      await expect(fs.access(malformedRoot)).resolves.toBeDefined();
    });
  });
  ...
  test('legacy state needs explicit safe cleanup and recovery or unknown state remains protected', async () => {
    await withRepository(async (repositoryRoot) => {
      ...
      await expect(fs.access(legacyRoot)).resolves.toBeDefined();
      ...
      await expect(fs.access(path.join(generatedRoot, 'recovery', 'branch.bundle'))).resolves.toBeDefined();
      await expect(fs.access(path.join(generatedRoot, 'mystery.bin'))).resolves.toBeDefined();
  ...
  test('expired diagnostics require explicit safe cleanup and are not automatic cache eviction', async () => {
    await withRepository(async (repositoryRoot) => {
      ...
      const automatic = await cleanGeneratedState({ ...runtime, profile: 'automatic' });
      expect(automatic.selected).toEqual([]);
      await expect(fs.access(diagnosticPath)).resolves.toBeDefined();
  ...
  test('atomic evidence staging residue is removed by safe cleanup while final evidence is preserved', async () => {
    await withRepository(async (repositoryRoot) => {
      const generatedRoot = getGeneratedStateRoot(repositoryRoot);
      await fs.mkdir(generatedRoot, { recursive: true });
      const evidencePath = path.join(generatedRoot, 'ci-verification-evidence.json');
      const riskPath = path.join(generatedRoot, 'ci-risk-batch-evidence.json');
      const stagingRelative = 'ci-verification-evidence.json.4242.0f5c2f10-5f3b-4a1e-8c0d-9f0a1b2c3d4e.tmp';
      const stagingPath = path.join(generatedRoot, stagingRelative);
      await fs.writeFile(evidencePath, '{"evidence":true}', 'utf8');
      await fs.writeFile(riskPath, '{"risk":true}', 'utf8');
      await fs.writeFile(stagingPath, 'partial', 'utf8');
      const old = new Date('2026-08-01T00:00:00.000Z');
      await fs.utimes(stagingPath, old, old);
      ...
      const inventory = await inspectGeneratedState(runtime);
      expect(entryByPath(inventory, 'ci-verification-evidence.json').status).toBe('retained');
      expect(entryByPath(inventory, 'ci-risk-batch-evidence.json').status).toBe('retained');
      expect(entryByPath(inventory, stagingRelative).ruleId).toBe('ci-verification-evidence-staging');
      expect(entryByPath(inventory, stagingRelative).status).toBe('legacy-expired');

      const automatic = await cleanGeneratedState({ ...runtime, profile: 'automatic' });
      expect(automatic.selected).toEqual([]);
      await expect(fs.access(stagingPath)).resolves.toBeDefined();

      const safe = await cleanGeneratedState({ ...runtime, profile: 'safe' });
      expect(safe.status).toBe('completed');
      expect(safe.selected).toEqual([stagingRelative]);
      await expect(fs.access(stagingPath)).rejects.toThrow();
      await expect(fs.readFile(evidencePath, 'utf8')).resolves.toBe('{"evidence":true}');
      await expect(fs.readFile(riskPath, 'utf8')).resolves.toBe('{"risk":true}');
    });
  });
  ...
  test('exact compatibility cleanup removes only the requested namespace', async () => {
    await withRepository(async (repositoryRoot) => {
      ...
      await expect(fs.access(second)).resolves.toBeDefined();
    });
  });
  ...
  test('all-rebuildable removes only registered caches and preserves protected state', async () => {
    await withRepository(async (repositoryRoot) => {
      ...
      await expect(fs.access(path.join(generatedRoot, 'compiler-deps.stamp.json'))).resolves.toBeDefined();
  ...
  test('symlinked roots and descendants are never followed or deleted', async () => {
    await withRepository(async (repositoryRoot) => {
      const generatedRoot = getGeneratedStateRoot(repositoryRoot);
      const parent = await isolationParent();
      const outside = await fs.mkdtemp(path.join(parent, 'sec-generated-state-outside-'));
      try {
