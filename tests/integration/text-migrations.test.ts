  ...
  await withSlotUpgradeDryRunFixture(
    {
      ...
      setup: async ({ paths }) => {
        await fs.mkdir(path.join(paths.workspaceRoot, 'docs', 'upgrade-notes.md'), { recursive: true });
      }
    },
    async ({ workspaceRoot }) => {
...
test('literal text replace migrations update all matching text', async () => {
  await withTempWorkspace(async (workspaceRoot) => {
    const targetPath = path.join(workspaceRoot, 'docs', 'upgrade-notes.md');
  ...
  await withSlotUpgradeDryRunFixture(
    {
      ...
      setup: async ({ paths }) => {
        await fs.mkdir(path.join(paths.workspaceRoot, 'docs'), { recursive: true });
        await fs.writeFile(path.join(paths.workspaceRoot, 'docs', 'upgrade-notes.md'), 'status: already-applied\n', 'utf8');
      }
    },
    async ({ workspaceRoot }) => {
      await expect(upgradeWorkspace(workspaceRoot, 'private/slot-contract', '0.2.0', { dryRun: true })).rejects.toMatchObject({
