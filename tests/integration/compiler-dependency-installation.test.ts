...
describe('compiler dependency installation', () => {
  ...
  test('linked worktree automatically reuses the common repository dependency generation', async () => {
    await withTempWorkspace(async (tempRoot) => {
      const ownerRoot = path.join(tempRoot, 'owner');
      const consumerRoot = path.join(tempRoot, 'consumer');
      const worktreeAdmin = path.join(ownerRoot, '.git', 'worktrees', 'consumer');
      await Promise.all([
        fs.mkdir(ownerRoot),
        fs.mkdir(consumerRoot),
        fs.mkdir(worktreeAdmin, { recursive: true })
      ]);
      await Promise.all([
        writeCompilerDependencyRoot(ownerRoot),
        writeCompilerDependencyRoot(consumerRoot),
        fs.writeFile(path.join(consumerRoot, '.git'), `gitdir: ${worktreeAdmin}\n`, 'utf8'),
        fs.writeFile(path.join(worktreeAdmin, 'commondir'), '../..\n', 'utf8')
      ]);
      await ensureCompilerDepsReady({
        commandRunner: async (_command, _args, command) => {
          await installCompilerDependencyFixture(command.cwd, 'common-worktree-owner');
          return { code: 0, stdout: 'ok', stderr: '' };
        }
      }, ownerRoot);
      const interruptedName = 'c.staging-Ab12Cd';
      ...
      await fs.mkdir(interruptedRoot, { recursive: true });
      await fs.writeFile(path.join(interruptedRoot, 'partial.bin'), 'partial');
      ...
      const reused = await ensureCompilerDepsReady({
        commandRunner: async () => {
          throw new Error('Linked worktree dependency reuse must not install.');
        },
        generatedStateLifecycle: {
          born: async () => undefined,
          disposed: async () => undefined,
          recoverDisposed: async (relativePath, operationId, outcome) => {
            recoveries.push({ operationId, outcome, relativePath });
            await fs.rm(interruptedRoot, { force: true, recursive: true });
          },
          retired: async () => undefined
        }
      }, consumerRoot);
      expect(reused).toMatchObject({
        nodeModulesPath: path.join(consumerRoot, 'node_modules'),
        requiresProcessRelaunch: true,
        root: consumerRoot,
        source: 'existing'
      });
      expect(await fs.realpath(reused.nodeModulesPath))
        .toBe(await fs.realpath(path.join(ownerRoot, 'node_modules')));
      expect(recoveries).toEqual([{
        operationId: `compiler-dependency-generation:${reused.manifestHash}:${interruptedName}`,
        outcome: 'interrupted-generation-recovered',
        relativePath: `.tmp/dependency-installs/${interruptedName}`
      }]);
      await expect(fs.lstat(interruptedRoot)).rejects.toMatchObject({ code: 'ENOENT' });
      expect((await fs.lstat(path.join(consumerRoot, 'node_modules'))).isSymbolicLink()).toBe(true);
    }, 'engineering-compiler-linked-worktree-deps-');
  });

  test('primary dependency reuse settles interrupted staging before returning an existing generation', async () => {
    await withTempWorkspace(async (tempRoot) => {
      await writeCompilerDependencyRoot(tempRoot);
      await ensureCompilerDepsReady({
        commandRunner: async (_command, _args, command) => {
          await installCompilerDependencyFixture(command.cwd, 'primary-owner');
          return { code: 0, stdout: 'ok', stderr: '' };
        }
      }, tempRoot);
      const stagingName = 'c.staging-Zy98Xw';
      const stagingRoot = path.join(tempRoot, '.tmp', 'dependency-installs', stagingName);
      await fs.mkdir(stagingRoot, { recursive: true });
      const recoveries: string[] = [];

      const ready = await ensureCompilerDepsReady({
        commandRunner: async () => {
          throw new Error('Existing primary generation must not reinstall.');
        },
        generatedStateLifecycle: {
          born: async () => undefined,
          disposed: async () => undefined,
          recoverDisposed: async (relativePath, operationId, outcome) => {
            recoveries.push(`${relativePath}:${operationId}:${outcome}`);
      ...
      expect(recoveries).toEqual([
        `.tmp/dependency-installs/${stagingName}:compiler-dependency-generation:${ready.manifestHash}:${stagingName}:interrupted-generation-recovered`
      ]);
      await expect(fs.lstat(stagingRoot)).rejects.toMatchObject({ code: 'ENOENT' });
    }, 'engineering-compiler-primary-recovery-');
  });
