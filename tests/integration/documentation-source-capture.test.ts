...
import {
  ...
  DOCUMENTATION_SOURCE_MANIFEST, DOCUMENTATION_REQUIREMENTS, documentationSourceDigest
...
test.serial('nonempty staging is rejected at the first entry and the directory is closed', async () => {
  await withSource(async root => {
    ...
    const open = spyOn(fs, 'opendir').mockImplementation(async (...args: Parameters<typeof fs.opendir>) => {
      ...
      return {
        read: async () => {
          if (++reads > 1) throw new Error('Staging inspection continued after rejection');
