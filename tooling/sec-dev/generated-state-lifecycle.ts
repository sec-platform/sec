...
export async function registerGeneratedStateBirthV1(input: Readonly<{
...
export async function retireGeneratedStateV1(input: Readonly<{
  repositoryRoot: string;
  workspaceRoot?: string;
  relativePath: string;
  ...
  if (current.phase === 'retired') {
    if (current.retirementRef === input.retirementRef) return current;
    throw new Error('Generated-state registration was retired by a different authority.');
  }
  const retired = retireGeneratedStateRegistrationV1(current, input.retirementRef, {
    clock: options.clock
  });
...
export function generatedStateProducerHooksV1(input: Readonly<{
  ...
  const retirementRef = (relativePath: string, outcome: string): `sha256:${string}` =>
    generatedStateDigestV1(Object.freeze({
  ...
  return Object.freeze({
    born: async (relativePath, operationId) => {
      await registerGeneratedStateBirthV1({ ...input, relativePath, operationId }, options);
    },
    retired: async (relativePath, outcome) => {
      await retireGeneratedStateV1({
        ...input,
        relativePath,
        retirementRef: retirementRef(relativePath, outcome)
      }, options);
    },
