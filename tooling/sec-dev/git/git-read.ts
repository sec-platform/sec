...
function runGitReadWithEnvironment(
...
export function runGitRead(
  ...
  return runGitReadWithEnvironment(repositoryRoot, args, options);
}
...
function gitReadBytesWithEnvironment(
  repositoryRoot: string,
  args: readonly string[],
  options: Readonly<{
    input?: Buffer;
  ...
  const result = runGitReadWithEnvironment(repositoryRoot, args, options);
  if (result.error || result.status !== 0) {
    const stderr = result.stderr.toString('utf8').trim();
    throw new Error(`${options.label ?? `git ${args[0] ?? 'command'}`} failed${stderr ? `: ${stderr}` : ''}`, {
      cause: result.error
    });
  }
  return result.stdout;
