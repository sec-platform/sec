...
import {
  ...
  type PhysicalMutationLeaseOptionsV1
...
export interface RuntimeStateJournalFileSystemV1 {
...
export type RuntimeStateJournalFileSystemOptionsV1 = PhysicalMutationLeaseOptionsV1;

export function runtimeStateJournalMutationLeaseNameV1(rootPath: string, filePath: string): string {
...
export function createRuntimeStateJournalFileSystemV1(
  retainedWorkspaceStateRoot: PhysicalDirectoryIdentityV1,
  options: RuntimeStateJournalFileSystemOptionsV1 = {}
): RuntimeStateJournalFileSystemV1 {
  const rootPath = path.resolve(retainedWorkspaceStateRoot.path);

  const root = (): PhysicalDirectoryIdentityV1 => assertSameNoFollowDirectoryIdentityV1(
    retainedWorkspaceStateRoot,
    'Runtime State journal workspace root'
  ).target;

  const existingDirectory = (directoryPath: string): PhysicalDirectoryIdentityV1 | null => {
    let current = root();
    for (const segment of relativeSegments(rootPath, directoryPath, true)) {
      const next = inspectNoFollowDirectoryChildV1(current, segment, 'Runtime State journal directory');
  ...
  const ensuredDirectory = (directoryPath: string): PhysicalDirectoryIdentityV1 => {
    const segments = relativeSegments(rootPath, directoryPath, true);
    return segments.length === 0
      ? root()
      : createNoFollowOrdinaryDirectoryChainV1(root(), segments);
  };

  const fileParent = (
    filePath: string,
    create: boolean
  ): Readonly<{ parent: PhysicalDirectoryIdentityV1; name: string }> | null => {
  ...
  const readBytes = (filePath: string): Buffer | null => {
    ...
    const bytes = readNoFollowOrdinaryFileV1(retained.parent, retained.name);
  ...
  const withMutationLease = <T>(filePath: string, operation: () => T): T | null => {
    ...
    const lockName = runtimeStateJournalMutationLeaseNameV1(rootPath, filePath);
    const lease = acquirePhysicalMutationLeaseV1(retained.parent, lockName, options);
  ...
  return Object.freeze({
    ...
    createExclusiveFsync(filePath: string, text: string): boolean {
      ...
      try {
        return publishExclusiveDurableCanonicalFileV1({
          parent: retained.parent,
          name: retained.name,
          bytes,
          validate: () => undefined
