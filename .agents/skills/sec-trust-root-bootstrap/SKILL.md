...
## 权限与路径
- 只审查/集成trust-root candidate；不得由candidate修改受信base verifier。

## 允许工具与操作
- base-side parser/TCB/focused tests、独立Review、admin/manual integration、main readback。

## 前置门禁
- trust-root delta明确、candidate single-parent、base verifier受信。
...
## 完成证据
- base-side results、TCB closure、Review、integration SHA、new-main trust digest。

## 停止与恢复
- 新 trust epoch进入main并由新会话重新信任；否则保持阻塞。
- 失败则保持旧epoch；成功后TASK_RESTART_REQUIRED。
