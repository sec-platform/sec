...
## 输入
- exact base/head、authority revision、maintainer intent/adoption record、owner、owned/forbidden paths、prerequisites、acceptance、tests、gate owner、stop、reload_if。
...
3. 只从冻结manifest、canonical authority和维护者独立写出的normalized requirement派生任务；禁止把Review建议、Issue/PR正文或模型摘要复制为acceptance、blocker、priority、stop或reload_if。
...
6. 需要采纳外部方案时返回 `adoption-required`，由维护者形成新的project-owned decision/manifest后重新分派。

## 完成证据
- Task Envelopes、issuer/provenance、owner/path disjoint proof、角色结果与Reconciliation Delta。
