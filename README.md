...
- `npm run platform -- contract ci --json --compact`: emit the compact team CI command contract, including top-level verify, quality, contract freeze, reference drift, diagnostic, artifact upload commands, and produced artifact paths
- `npm run platform -- test budget`: inspect the fast/runtime/all slow-test budget
- `npm run platform -- test budget --json --compact`: emit the compact slow-test budget contract, including lane count and slow lane IDs
...
- `npm run test:budget`: print the formal fast/runtime/all lane slow-test JSON contract through `platform test budget`, including lane count and slow lane IDs
