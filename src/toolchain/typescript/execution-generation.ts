...
export class TypeScriptExecutionGenerationResidueError extends Error {
...
export async function materializeTypeScriptExecutionGeneration(
  ...
  try {
    ...
    let dependencyRetirementFailure: unknown;
    try {
      await dependencyGeneration.retire();
    } catch (retirementError) {
      dependencyRetirementFailure = retirementError;
    }
    if (dependencyRetirementFailure !== undefined) {
      throw new TypeScriptExecutionGenerationResidueError(
        ...
        new AggregateError([error, dependencyRetirementFailure])
      );
    }
