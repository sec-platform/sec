...
import {
  ...
  type IsolatedVerificationEvidenceArtifacts
} from './evidence.ts';

export type IsolatedArtifactAssessment =
  | Readonly<{
      status: 'passed';
...
export function assessIsolatedArtifacts(input: Readonly<{
  attempted: SemanticMutationBase;
  artifactStatus: 'passed' | 'failed';
  artifacts: IsolatedVerificationEvidenceArtifacts;
}>): IsolatedArtifactAssessment {
  ...
  if (!bound) {
    ...
    return Object.freeze({
      ...
      failure,
      evidenceDigest: isolatedVerificationEvidenceDigest({
        status: 'blocked',
        failure
      })
    });
