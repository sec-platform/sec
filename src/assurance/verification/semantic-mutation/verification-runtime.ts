...
} from '../../../compiler/semantic-mutation/verification-policy.ts';
...
import {
  ...
  type SemanticMutationVerifyAllRunner
...
function selectorIsAuthoritative(
  snapshot: ValidatedEngineeringIRSnapshot,
  selector: string
): boolean {
  return isCanonicalSemanticMutationVerificationSelector(selector) &&
    snapshot.ir.facts.some(fact => {
      if (fact.predicate !== 'VERIFIED_BY' ||
      ...
      return Object.keys(value).length === 1 &&
        ...
        fact.assertions.some(assertion =>
          ...
          assertion.authority === 'derived'
        );
    });
}

function hasExternalIrreversibleEffect(
  snapshot: ValidatedEngineeringIRSnapshot
): boolean {
  const externalEffects = new Set(
    snapshot.ir.entities
      .filter(entity => entity.kind === 'effect' && entity.attributes.some(attribute =>
        attribute.key === 'effectKind' &&
        attribute.value === 'external-service-call'
      ))
...
function capabilityFor(
  ...
  if (requirement.kind === 'pass') {
    ...
    runnable = snapshot.ir.entities.some(entity =>
      entity.id === requirement.acceptanceEntityId &&
      entity.kind === 'acceptance'
    );
  } else {
  ...
  const available = runnable &&
    !hasExternalIrreversibleEffect(snapshot) &&
    isolationProven;
  return {
...
/**
 * Plan the exact Verification obligations after the physical isolation provider
 * has been observed. Assurance owns requirement coverage, blocking, identity
 * and the process-local fact that a runnable plan was genuinely issued.
 */
export function planSemanticMutationVerificationCapabilitiesForIsolation(input: {
  ...
  const capabilities = input.requirements
    .map(requirement => capabilityFor(
      input.snapshot,
      requirement,
      input.isolationProven
    ))
  ...
  for (const capability of capabilities) {
    const key = semanticMutationVerificationRequirementKey(
  ...
  if (capabilities.length === 0) {
    blockedKeys.add('empty-requirement-union');
  }
  ...
  if (plan.status === 'runnable' && input.isolationProven) {
...
function exactEndpoint(value: unknown): value is SemanticMutationBase {
  return exactKeys(value, ['transactionId', 'inputRevision', 'semanticRevision']) &&
    ...
    isDigest(value.inputRevision, 'sha256') &&
    isDigest(value.semanticRevision, 'sha256');
}
...
function assertExecutionBinding(input: {
  ...
  const exactSuppliedRequirements = suppliedRequirements.length > 0 &&
    suppliedRequirements.every(requirement =>
      isExactSemanticMutationVerificationRequirement(requirement)
    );
  const requirements = suppliedRequirements
    .map(normalizeSemanticMutationVerificationRequirement)
    .sort((left, right) => compareCodeUnits(
      semanticMutationVerificationRequirementKey(left),
      semanticMutationVerificationRequirementKey(right)
    ));
  const plannedRequirements = input.capabilityPlan.capabilities.map(
    entry => entry.requirement
  );
  const plannedRequirementKeys = plannedRequirements.map(
    semanticMutationVerificationRequirementKey
  );
  const {
    capabilityPlanRevision: suppliedPlanRevision,
    ...capabilityPlanWithoutRevision
  ...
  const exactCapabilityPlan = exactKeys(input.capabilityPlan, [
    ...
    'snapshotSemanticRevision',
    'status',
    'capabilities',
    'blockedRequirementKeys',
    'capabilityPlanRevision'
  ]) &&
    input.capabilityPlan.formatRevision ===
      SEMANTIC_MUTATION_VERIFICATION_CAPABILITY_PLAN_REVISION &&
    Array.isArray(input.capabilityPlan.capabilities) &&
    input.capabilityPlan.capabilities.length > 0 &&
    input.capabilityPlan.capabilities.every(entry =>
      exactKeys(entry, ['requirement', 'status', 'isolated']) &&
      isExactSemanticMutationVerificationRequirement(entry.requirement) &&
  ...
  if (!exactKeys(input, [
    ...
    !exactSuppliedRequirements ||
    !exactEndpoint(input.attempted) ||
    JSON.stringify(input.requirements) !== JSON.stringify(requirements) ||
    !exactCapabilityPlan ||
    suppliedPlanRevision !== capabilityPlanRevision(capabilityPlanWithoutRevision) ||
    input.capabilityPlan.status !== 'runnable' ||
    input.capabilityPlan.adapterId !==
      SEMANTIC_MUTATION_LOCAL_VERIFICATION_ADAPTER_ID ||
    input.capabilityPlan.adapterRevision !==
      SEMANTIC_MUTATION_LOCAL_VERIFICATION_ADAPTER_REVISION ||
    input.attempted.inputRevision !==
      input.capabilityPlan.snapshotInputRevision ||
    input.attempted.semanticRevision !==
      input.capabilityPlan.snapshotSemanticRevision ||
    JSON.stringify(requirements) !== JSON.stringify(plannedRequirements) ||
    input.requiredVerificationDigest !==
      semanticMutationRequiredVerificationDigest(requirements) ||
    !isDigest(input.planRevision, 'sha256') ||
    !isDigest(input.stagedSourceDigest, 'sha256')) {
    throw new Error(
      'Semantic Mutation Verification execution binding is invalid or blocked'
    );
  }
  return requirements;
}
...
/**
 * Assess one already-issued capability plan using the injected verify-all
 * method. Assurance owns exact binding, evidence interpretation and report
 * identity; the runner owns only the physical execution.
 */
export async function executeSemanticMutationVerification(
  ...
  if (typeof runner !== 'function') {
    throw new TypeError('Semantic Mutation verify-all runner must be callable');
  }
  ...
  if ((runnerResult.status !== 'passed' &&
      ...
      runnerResult.status !== 'blocked') ||
      !isDigest(runnerResult.evidenceDigest, 'sha256')) {
    throw new Error(
      'Semantic Mutation verify-all runner returned invalid evidence'
    );
  }
