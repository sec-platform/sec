...
interface ReviewUpgradePreflightSummary {
  group: string;
  checkCount: number;
  evidenceCount: number;
}

interface ReviewUpgradeMigrationSummary {
  id: string;
  kind: string;
  target: string;
  reason: string;
...
type ReviewUpgradeMigrationOperationSummary = UpgradeMigrationOperation;

interface ReviewUpgradeVerificationSummary {
  id: 'required' | 'skipped';
  count: number;
}

interface ReviewUpgradeDiagnosticsSummary {
  status: 'blocked';
...
function buildReviewUpgradePreflightSummaries(
