...
import type {
  ...
  ExplainGraphNode,
  ExplainNodeType
} from '../../../semantics/projection/explain.ts';
...
import type { ReviewRuntimeEntry } from './contract/types.ts';
...
export function buildExplainGraphFromEvidence(
  lock: LockFile,
  provenance: ProvenanceFile,
  coverage: AcceptanceCoverageReport,
  policyReport: PolicyReport | null,
  runtimeAttributions: readonly ReviewRuntimeEntry[],
  ...
  const runtimeAttributionByPath = new Map(
    runtimeAttributions.map(attribution => [attribution.path, attribution] as const)
  );
  ...
  for (const artifact of provenance.artifacts) {
    ...
    const attribution = runtimeAttributionByPath.get(artifact.path);
    if (attribution) {
