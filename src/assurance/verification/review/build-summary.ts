...
import { policiesRelativePath } from '../../../workspace/paths.ts';
import type { AcceptanceCoverageEntry, AcceptanceCoverageReport } from '../../acceptance/coverage.ts';
import {
  CI_ARTIFACT_FILES,
  isCanonicalCiArtifactPath
} from '../ci-artifacts/contract/manifest.ts';
import type { VerificationReport } from '../contract/types.ts';
import { buildReviewPolicySummary } from './contract/policy.ts';
import type {
  ReviewConflictHint,
  ReviewFailurePoint,
  ReviewInstallImpact,
...
import { REVIEW_SUMMARY_FORMAT_VERSION } from './contract/types.ts';
import {
  buildReviewUpgradeSummary,
  upgradeDiagnosticsAttributionParts
} from './contract/upgrade.ts';
import { buildReviewChainSummary } from './matrix.ts';
import {
...
interface ReviewTargetIndex {
  ...
  readonly runtimeEntryByPath: ReadonlyMap<string, ReviewRuntimeEntry>;
}
...
function buildReviewTargetIndex(
  lock: LockFile,
  runtimeEntryByPath: ReadonlyMap<string, ReviewRuntimeEntry>
): ReviewTargetIndex {
...
function repairTargetType(
  ...
  if (isCanonicalCiArtifactPath(targetId)) return 'generated-file';
...
export interface ReviewSummaryEvidence {
  ...
  readonly coverage: AcceptanceCoverageReport;
  readonly overrideManifest: OverrideManifest;
  readonly policyReport: PolicyReport | null;
  readonly repairPlan: RepairPlan | null;
  readonly upgradePlan: UpgradePlan | null;
  readonly upgradeExecutionTerminal: UpgradeExecutionTerminal | null;
  readonly upgradeDiagnostics: UpgradeDiagnostics | null;
  readonly allRuntimeEntries: readonly ReviewRuntimeEntry[];
  readonly artifactSummary: ReviewSummary['artifactSummary'];
}

export function reviewRuntimeAttributionTargets(
...
/**
 * Build Review truth from one captured evidence set. Physical artifact and
 * manifest reads are owned by adapters; Assurance owns all interpretation,
 * attribution, risk, conflict and summary construction.
 */
export function buildReviewSummaryFromEvidence(
  input: ReviewSummaryEvidence
): ReviewSummary {
  const {
    lock,
    provenance,
