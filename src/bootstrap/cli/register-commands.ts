...
export function registerCommands(
  ...
  registerDependencyCommands(program, bindDependencyCommandHandlers({
    doctor: async workspaceRoot => {
      ...
      return domain.getDoctorReport(workspaceRoot);
    },
    status: async workspaceRoot => {
      const domain = await loadDependencyEnvironment();
      return domain.getDependencyEnvironmentStatus(workspaceRoot);
    },
    freshness: async () => {
      const domain = await loadDependencyEnvironment();
      return domain.getDependencyFreshness();
    },
    warmup: async workspaceRoot => {
      const domain = await loadDependencyEnvironment();
      return domain.warmupDependencyEnvironment(workspaceRoot);
    },
    relink: async workspaceRoot => {
      const domain = await loadDependencyEnvironment();
      return domain.relinkProjectDependencies(workspaceRoot);
    },
  ...
  registerVerificationToolingCommands(
    program,
    bindVerificationToolingCommandHandlers({
      benchmarkCatalog: () => buildBenchmarkTaskCatalog(),
      testBudget: async () => {
        ...
        return domain.buildTestBudgetContract(
  ...
  registerTextCommands(program, bindTextCommandHandlers<TextByteClassification>({
    admitThreshold: failOn => {
      ...
      return admission.status === 'rejected'
        ...
        : { status: 'accepted' as const, threshold: admission.status === 'accepted' ? admission.threshold : undefined };
    },
