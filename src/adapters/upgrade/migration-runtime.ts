...
async function applyDbExpandContractMigration(context: MigrationApplyContext<'db-expand-contract'>): Promise<void> {
    ...
    update: (current) => {
      ...
      if (entry.contractField) {
        updatedModelContent = updatedModelContent.split('\n').map((line) => {
          ...
          return trimmed.startsWith(entry.contractField) || trimmed.startsWith(`${entry.contractField} `)
  ...
  if (entry.copyJobCode) {
    const jobFile = path.join(workspaceRoot, 'src', 'jobs', 'db-migrations', `${entry.id}.ts`);
...
`;
    await updateNoFollowMigrationFile({
      ...
      commitFence,
      createParents: true,
      update: () => Buffer.from(jobContent, 'utf8')
    });
  }
}
