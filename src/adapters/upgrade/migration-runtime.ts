...
import {
  ...
  dbExpandContractCopyJobProjectPath,
  ensureJsonObject,
  ensureMigrationString,
  isJsonObject,
  readOwnJsonMutationValue,
...
import {
  createNoFollowMigrationDirectory,
  deleteNoFollowMigrationDirectory,
  readNoFollowMigrationFile,
  removeNoFollowMigrationFile,
  renameNoFollowMigrationDirectory,
  renameNoFollowMigrationFile,
  updateNoFollowMigrationFile
} from './migration-physical.ts';

async function detectOverrideConflicts(
  workspaceRoot: string,
  blockId: string,
  impacts: string[],
  migrationEntries: readonly UpgradeMigrationEntry[]
): Promise<string[]> {
  const manifest = await loadOverrideManifest(workspaceRoot);
  const directoryImpacts = migrationEntries.flatMap((entry) => (
    entry.kind === 'create-directory' || entry.kind === 'delete-directory'
      || entry.kind === 'copy-directory'
      ? [entry.target]
      : entry.kind === 'rename-directory' ? [entry.source, entry.target] : []
  ));
  const conflictingOverride = manifest.overrides.find(
    (entry) =>
      impacts.includes(entry.target) ||
      directoryImpacts.some((directory) => (
        entry.target.startsWith(`${directory}/`) || directory.startsWith(`${entry.target}/`)
      )) ||
...
export async function loadMigrationEntries(
  ...
  for (const migration of migrations) {
    ...
    const retainedEntry = readNoFollowMigrationFile(
      targetManifestRoot,
      entryPath,
    ...
    const entry = JSON.parse(Buffer.from(retainedEntry.bytes).toString('utf8')) as UpgradeMigrationEntry;
    try {
...
function dbExpandContractCopyJobContent(
  entry: Extract<UpgradeMigrationEntry, { kind: 'db-expand-contract' }>
): Buffer | null {
  if (entry.copyJobCode === undefined) return null;
  return Buffer.from(`// @generated-db-migration-job migration-id:${entry.id}
...
function dbExpandContractCopyJobTarget(
  workspaceRoot: string,
  entry: Extract<UpgradeMigrationEntry, { kind: 'db-expand-contract' }>
): Readonly<{ path: string; bytes: Buffer }> | null {
  const jobPath = dbExpandContractCopyJobProjectPath(entry);
  const bytes = dbExpandContractCopyJobContent(entry);
  if (jobPath === null || bytes === null) return null;
  const targetPath = resolveWorkspaceMigrationPath(workspaceRoot, jobPath, {
    migrationId: entry.id, role: 'target'
  });
  return Object.freeze({ path: targetPath, bytes });
}

function assertDbExpandContractCopyJobPreimage(
  workspaceRoot: string,
  entry: Extract<UpgradeMigrationEntry, { kind: 'db-expand-contract' }>,
  job: Readonly<{ path: string; bytes: Buffer }>
): void {
  ...
  await updateNoFollowMigrationFile({
    ...
    update: (current) => {
      ...
      if (entry.contractField) {
        updatedModelContent = updatedModelContent.split('\n').map((line) => {
          const trimmed = line.trim();
          return trimmed.split(/\s+/u, 1)[0] === entry.contractField
            ? `  // [Contracted Old Field]: ${line.trim()}`
            : line;
        }).join('\n');
      }
...
const migrationOperationSpecs = {
  ...
  'db-expand-contract': {
    ...
    collectFileEvidence: async ({ entry, workspaceRoot }) => {
      const job = dbExpandContractCopyJobTarget(workspaceRoot, entry);
      if (job === null) return [];
      assertDbExpandContractCopyJobPreimage(workspaceRoot, entry, job);
      return [`${entry.id}:copy-job:available`];
    },
  }
...
function readJsonPath(config: unknown, pathSegments: string[]): unknown {
  ...
  for (const segment of pathSegments) {
    ...
    current = readOwnJsonMutationValue(current, segment);
  }
  return current;
}
...
async function collectJsonStructureEvidence(
  ...
  const evidence: string[] = [];
  for (const entry of migrationEntries) {
    if (!isJsonMigrationEntry(entry)) continue;
    const targetPath = resolveWorkspaceMigrationPath(workspaceRoot, entry.target, {
      migrationId: entry.id,
      role: 'target'
    });
...
export async function collectUpgradePreflightEvidence(
  ...
  return {
    ...
    scannedOverrides: await detectOverrideConflicts(workspaceRoot, blockId, impacts, migrationEntries)
  };
}
