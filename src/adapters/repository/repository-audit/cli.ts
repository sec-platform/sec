...
import { type GitReadSession, type GitReadSessionCommand } from '../../providers/git-read/runtime/session.ts';
import {
...
async function auditRepositoryWithSession(
  ...
  findings.push(...projectWorkTrackingContractFindings(tracked, textByPath));
...
; meaning: 'root-goal'; task: false; githubIssue: false; progressUnit: false }>;
    U: Readonly<{ pattern: '^U\\d{3}
...
; meaning: 'stable-problem-theme'; task: false; githubIssue: false; progressUnit: false; allocation: 'monotonic-never-reuse' }>;
  }>;
  execution: Readonly<{
