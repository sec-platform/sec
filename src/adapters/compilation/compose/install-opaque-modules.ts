...
async function assertOpaqueNodeModulesLink(targetPath: string, expectedSource: string): Promise<void> {
  ...
  const [actual, expected] = await runTaskGroup([
...
export async function installOpaqueModules(
  ...
  const moduleGeneratedPaths = await mapTaskGroup(moduleEntries, (entry, _index, signal) =>
    installOpaqueModule(entry, workspaceRoot, paths.srcRoot, isBuildMode,
      createTaskGroupEffectFence(signal, commitFence)), { concurrency: 8 });
