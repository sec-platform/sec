...
const GENERATED_STATE_WORKTREE_RETIREMENT_SCHEMA = "sec-generated-state-worktree-retirement-v1" as const;
...
type GeneratedStateObservedClass = GeneratedStateClass | 'orphaned-owned-workspace';
export type GeneratedStateCleanupProfile = 'automatic' | 'safe' | 'all-rebuildable';
type GeneratedStateRegistrationPolicy = 'required-at-birth' | 'domain-owned';
type GeneratedStateRetirementPolicy = 'domain-receipt-required';
type GeneratedStateRootKind = 'directory' | 'file' | 'link';
type GeneratedStateScope = 'workspace' | 'operation';
type GeneratedStateActiveOwnerSignal = 'birth-registration' | 'domain-owner-receipt';
type GeneratedStateContentsPolicy =
  | 'owner-bounded-tree'
  | 'ordinary-file'
  | 'opaque-protected-tree'
  | 'owner-bound-locator';
type GeneratedStateCapacityPolicyRef = 'issue-316' | 'owner-defined-protected';
type GeneratedStateSettlementEffect = 'generated-state-quarantine-delete-readback' | 'domain-owner-only';
type GeneratedStateWorktreeRetirementDisposition =
...
type GeneratedStateReconstruction =
  | 'producer-recompute'
  | 'fixture-rebuild'
  | 'rerun-diagnostic'
  | 'owner-recovery-only';

type GeneratedStateSelector =
...
interface GeneratedStateWorktreePreservedEntry {
  readonly relativePath: string;
...
interface GeneratedStateWorktreeDomainRetiredEntry {
  readonly relativePath: string;
  readonly source: GeneratedStatePhysicalIdentity;
  readonly inventoryDigest: `sha256:${string}`;
  readonly ruleIds: readonly string[];
...
type GeneratedStateWorktreeRetirementEntry =
