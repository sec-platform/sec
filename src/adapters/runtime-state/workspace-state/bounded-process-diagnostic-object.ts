...
export {
  BOUNDED_PROCESS_DIAGNOSTIC_MAXIMUM_STREAM_BYTES,
  BOUNDED_PROCESS_DIAGNOSTIC_METADATA_MAXIMUM_BYTES,
  BOUNDED_PROCESS_DIAGNOSTIC_OBJECT_SCHEMA,
  BoundedProcessDiagnosticObjectError,
  encodeBoundedProcessDiagnosticObjectReceipt,
  parseBoundedProcessDiagnosticObjectReceipt,
  type BoundedProcessDiagnosticFailureKind,
...
export const BOUNDED_PROCESS_DIAGNOSTIC_RETENTION_MS = 30 * 24 * 60 * 60 * 1_000;

const DIGEST = /^sha256:[0-9a-f]{64}$/u;
const OPERATION_MAXIMUM_DURATION_MS = 30_000;
const GC_MAXIMUM_RECORDS = 1_024;

export type BoundedProcessDiagnosticRead = Readonly<{
  status: 'available';
...
export type BoundedProcessDiagnosticGcReceipt = Readonly<{
...
export type BoundedProcessDiagnosticPublishInput = Readonly<{
...
export type BoundedProcessDiagnosticReadInput = Readonly<{
  operation: SecBoundSemanticOperation;
  requirementId: string;
  receipt: BoundedProcessDiagnosticObjectReceipt;
  signal?: AbortSignal;
...
export type BoundedProcessDiagnosticGcInput = Readonly<{
  operation: SecBoundSemanticOperation;
  requirementId: string;
  signal?: AbortSignal;
}>;
