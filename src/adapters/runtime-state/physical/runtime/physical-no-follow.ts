...
function windowsRetainedOrdinaryFileLinkCount(handle: bigint, label: string): number {
...
function windowsTruncateRetainedOrdinaryFile(handle: bigint, size: number, label: string): void {
  ...
  const capability: RetainedNoFollowFileTransaction = Object.freeze({
    ...
    createExclusive: async (relativePath: string, bytes: Uint8Array, operation: string, creationMode = 0o600) => {
      ...
      try {
        ...
        else { leaf = requireLinuxLibc().symbols.openat((parent as ReturnType<typeof linuxParent>).fd, Buffer.from(`${name}\0`), LINUX_O_RDWR | LINUX_O_NOFOLLOW | LINUX_O_CLOEXEC | LINUX_O_CREAT | LINUX_O_EXCL, creationMode); if ((leaf as number) < 0) throw physicalError('PHYSICAL_NO_FOLLOW_IDENTITY_CHANGED', `${operation} create failed (errno ${linuxErrno()}).`); writeFileSync(leaf as number, bytes); }
      ...
      if (process.platform === 'win32') {
        ...
        try {
          ...
          if (!after.isFile() || String(after.dev) !== source.identity.device || String(after.ino) !== source.identity.inode ||
              after.size !== BigInt(expected.byteLength)) {
