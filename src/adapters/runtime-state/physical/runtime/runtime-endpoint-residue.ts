...
import {
  settleResources as settlePhysicalResources,
  type ResourceSettlementFailure as PhysicalResourceSettlementFailure
...
const RUNTIME_ENDPOINT_RESIDUE_SCHEMA =
  'sec-runtime-endpoint-residue-observation-v1' as const;
const RUNTIME_GENERATION_CENSUS_SCHEMA =
...
interface RuntimeGenerationCensusEntry {
  readonly id: string;
...
// Retained roots were issued by the native filesystem owner, not by a
// path-string classifier. A POSIX absolute path is also win32.isAbsolute.
...
function evidenceCandidates(
  ...
  for (const root of [...roots].sort((left, right) => right.path.length - left.path.length)) {
    ...
    const folded = windowsPath ? evidence.toLowerCase() : evidence;
    const rootText = windowsPath ? root.path.replaceAll('/', '\\') : root.path;
    const rootFolded = windowsPath ? rootText.toLowerCase() : rootText;
    let offset = 0;
    while (offset < folded.length) {
      ...
      const key = `${root.root.objectId}\0${nativePathKey(candidate)}`;
  ...
  for (const candidate of evidenceCandidates(input.roots, input.providerEvidence)) {
    ...
    try {
      ...
      const retainedRoot = chain.ancestors.find((identity) => (
        nativePathKey(identity.path)
          === nativePathKey(candidate.root.root.path)
      )) ?? (nativePathKey(chain.target.path)
        === nativePathKey(candidate.root.root.path)
        ? chain.target
        : null);
      ...
      candidate.root.assertCurrent();
      if (observed === null
          || nativePathKey(observed.path)
            !== nativePathKey(candidate.path)
