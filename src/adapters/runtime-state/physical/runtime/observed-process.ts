...
const MAX_COMMAND_STDIN_BYTES = 64 * 1024 * 1024;
...
interface ObservedCommandStreamEvidence {
  readonly bytes: number;
  readonly digest: `sha256:${string}`;
  readonly observerTruncated: boolean;
}

interface ObservedCommandTerminationEvidence {
...
type ObservedNativeProcessResourceKind =
  | 'root-process'
  | 'stdin-worker'
  | 'termination-helper';
...
interface ObservedNativeProcessResourceToken {
  start(): void;
  settle(): void;
}
...
type ObservedProcessTimerHandle = ReturnType<typeof setTimeout>;
...
interface ObservedProcessRuntime {
...
interface ObservedProcessTreeInspectionRequest {
...
interface ObservedProcessTreeInspectionResult {
  readonly treeClosed: boolean;
  readonly forcedAttempted: boolean;
}

interface ObservedProcessTreeTerminationRequest {
  readonly child: ChildProcess;
  readonly deadlineAtMs: number;
...
interface ObservedProcessTreeTerminationResult {
  readonly gracefulAttempted: boolean;
  readonly forcedAttempted: boolean;
  readonly treeClosed: boolean;
}
...
async function terminateObservedProcessTree(
  request: ObservedProcessTreeTerminationRequest
): Promise<ObservedProcessTreeTerminationResult> {
  return request.runtime.platform === 'win32'
