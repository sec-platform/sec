...
type WindowsHostDirectoryAclProof = Readonly<{
  aclDigest: string;
  ownerSid: string;
}>;
...
const WINDOWS_MUTABLE_AUTHORITY_STABILIZATION_ATTEMPTS = 8;

async function observeStableWindowsHostDirectoryAuthority(
  directoryPath: string,
  aclProbe: WindowsHostDirectoryAclProbe,
  input: Readonly<{ deadlineAtMs?: number }> | undefined,
  expectedPhysical?: PhysicalWindowsDirectory,
  initialAcl?: WindowsHostDirectoryAclProof
): Promise<Readonly<{
  acl: WindowsHostDirectoryAclProof;
  physical: PhysicalWindowsDirectory;
}>> {
  let physical = await physicalWindowsDirectory(directoryPath);
  ...
  let reusableInitialAcl = initialAcl !== undefined && (
    expectedPhysical === undefined ||
    sameDirectoryIdentity(physical.identity, expectedPhysical.identity)
  );
  for (let attempt = 0; attempt < WINDOWS_MUTABLE_AUTHORITY_STABILIZATION_ATTEMPTS; attempt += 1) {
    const acl = reusableInitialAcl && initialAcl !== undefined
      ? initialAcl
      : await aclProbe(physical.rootPath, input);
    reusableInitialAcl = false;
    ...
    if (sameDirectoryIdentity(physicalAfterAcl.identity, physical.identity)) {
      return Object.freeze({ acl, physical: physicalAfterAcl });
    }
    // ACL hardening and ordinary child publication both advance ChangeTime.
    // A proof observed across that transition is stale even when the final ACL
    // is equivalent, so restart from the retained physical object's new epoch.
    physical = physicalAfterAcl;
  }
  throw authorityError(
    'physical-identity-changed',
    'Windows host directory authority did not stabilize during ACL admission'
  );
}
...
function issueGenerationAuthority(input: Readonly<{
  ...
  const assertCurrent = async (): Promise<void> => {
    if (released || releasing) {
      throw authorityError(
        'session-closed',
        released
          ? 'Windows generation proof is released'
          : 'Windows generation proof is closing'
      );
    }
  ...
  const release = async (): Promise<void> => {
    if (released) return;
    if (releasing) {
      throw authorityError('session-closed', 'Windows generation proof release is already in progress');
    }
    releasing = true;
    try {
      input.session.close();
...
async function proveWindowsHostDirectoryAuthority(
  ...
  const admitted = await observeStableWindowsHostDirectoryAuthority(
    directoryPath,
    aclProbe,
    undefined,
    expectedPhysical,
  ...
  const releaseOnce = async (): Promise<void> => {
    ...
    if (releasing) {
      throw authorityError(
        'session-closed',
        'Windows host directory authority release is already in progress'
      );
    }
    releasing = true;
    try {
  ...
  return Object.freeze({
    ...
    assertCurrent: async (input?: Readonly<{ deadlineAtMs?: number }>) => {
      ...
      try {
        const current = await observeStableWindowsHostDirectoryAuthority(
          physical.rootPath,
          aclProbe,
          input,
          physical
        );
        if (current.acl.ownerSid !== acl.ownerSid ||
          current.acl.aclDigest !== acl.aclDigest) {
          throw authorityError(
            'physical-identity-changed',
...
export async function sealExistingWindowsReadOnlyTreeAuthority(
  ...
  try {
    ...
    const assertCurrent = async (): Promise<void> => {
      if (released || releasing || releaseFailure !== undefined) {
        throw authorityError(
          'session-closed',
          releaseFailure === undefined
            ? 'Windows read-only tree authority is closing or released'
            : 'Windows read-only tree authority closeout is unresolved',
    ...
    const release = async (): Promise<void> => {
      ...
      if (releasing) {
        throw authorityError(
          'session-closed',
          'Windows read-only tree authority release is already in progress'
        );
      }
      releasing = true;
      ...
      if (failure !== undefined) {
        const error = authorityError(
          'acl-untrusted',
          'Windows read-only tree retirement left ACL residue',
          failure.error
        );
    ...
    for (let index = recoveryTargets.length - 1; index >= 0; index -= 1) {
      ...
      try {
        ...
        recoveryFailure ??= Object.freeze({ error: cleanupError });
      }
    }
    session.close();
    if (recoveryFailure !== undefined) {
      throw authorityError(
        ...
        new AggregateError([error, recoveryFailure.error])
      );
    }
    throw error;
  }
