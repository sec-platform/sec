...
function readLocalContinuationHandoff(filePath: string): string {
  const absolutePath = path.resolve(filePath);
  const parent = inspectNoFollowDirectoryChain(
    path.dirname(absolutePath),
    'LocalContinuation handoff parent'
  ).target;
  const bytes = readNoFollowOrdinaryFile(parent, path.basename(absolutePath), {
    maximumBytes: LOCAL_CONTINUATION_CHECKPOINT_MAX_BYTES
  });
  if (bytes === null) throw new Error('LocalContinuation handoff file is absent.');
  return decodeExactUtf8(bytes, 'LocalContinuation handoff');
}
