...
export interface BranchLifecycleCloseoutTargetScope
  ...
  /** Full preparation inventory; only stable policy/configuration facts are reused. */
...
function listTargetLocalBranches(
  ...
  if (result.status !== 0) {
    throw new Error(
      `target local branch inventory failed: ${decodeBranchLifecycleChildError(result)}`
    );
  }
...
function listTargetRemoteBranches(
  ...
  if (result.status !== 0) {
    throw new Error(
      `target remote branch inventory failed: ${decodeBranchLifecycleChildError(result)}`
    );
  }
...
function listCloseoutTargetWorktrees(
  ...
  if (result.status !== 0) {
    unknowns.push(
      `worktree registry inventory failed: ${decodeBranchLifecycleChildError(result)}`
    );
    return [];
  }
 ...
 * The authenticated provider supplies the exact PR observation. This producer
 * independently re-observes every local/Git fact that can authorize the target
 * Effect without rescanning unrelated PRs or worktree contents.
 */
export function collectBranchLifecycleCloseoutTargetInventory(
  ...
  if (exactPullRequest.number !== input.pullRequestNumber
      ...
      || exactPullRequest.url
        !== `https://github.com/${fullName}/pull/${input.pullRequestNumber}`) {
