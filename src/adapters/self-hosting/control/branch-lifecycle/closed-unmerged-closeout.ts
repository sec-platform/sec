...
export async function executeClosedUnmergedCloseoutOperation(input: {
  ...
  if (effectStart === null) {
    ...
    if (startAuthorization.localAction === 'delete-exact' && adapter.localRefDeleteAtomicity !== 'supported') {
      return blocked(operation, 'effect-start-precondition', [
        'Git local ref exact delete is unavailable before remote branch effects'
      ]);
    }
  ...
  if (authorization.localAction === 'delete-exact' && adapter.localRefDeleteAtomicity !== 'supported') {
