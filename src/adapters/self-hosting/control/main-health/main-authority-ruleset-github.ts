...
function parseCli(argv: readonly string[]): Readonly<{
  ...
  if (!baseValid || (argv.length !== 7 && argv.length !== 9)) {
    throw new Error('Usage: bun src/adapters/self-hosting/control/main-health/main-authority-ruleset-github.ts observe --repository <owner/name> --branch <branch> --output <json> [--integration-id <positive-integer>]');
  }
  if (argv.length === 7) {
