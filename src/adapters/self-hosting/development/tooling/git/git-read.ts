...
import { isProductionGitReadSession, type GitReadSession, type GitReadSessionBudget } from '../../../../providers/git-read/runtime/session.ts';
...
/**
 * One bounded budget for a complete tooling observation.  The authority
 * session owns the counters; these values only narrow its canonical ceiling
 * and are deliberately shared by census and settlement.
 */
export const GIT_READ_OPERATION_BUDGET: GitReadSessionBudget = Object.freeze({
  deadlineMs: 30_000,
  maxProcesses: 64,
  maxTotalArgumentBytes: 8 * 1024 * 1024,
...
const GIT_READ_COMMAND_MAX_BUFFER = GIT_READ_OPERATION_BUDGET.maxCommandStdoutBytes;
...
export async function runGitRead(
  ...
  const command = await session.run(args, { input: options.input });
  if (command.kind !== 'completed') {
    return Object.freeze({
      status: null,
      stdout: Buffer.alloc(0),
      stderr: Buffer.from(command.detail, 'utf8'),
      error: new Error(`Git ${args[0] ?? 'command'} did not complete: ${command.reason}`)
    });
  }
  const stdout = Buffer.from(command.result.stdout);
  const stderr = Buffer.from(command.result.stderr, 'utf8');
  if (stdout.byteLength > maxBuffer) {
    return Object.freeze({
      status: null,
      stdout,
      stderr,
      error: new Error(`Git ${args[0] ?? 'command'} exceeded the ${maxBuffer}-byte output bound`)
    });
  }
  return Object.freeze({
...
export async function gitReadBytes(
  session: GitReadSession,
  args: readonly string[],
  options: Readonly<{
    input?: Buffer;
  ...
  const result = await runGitRead(session, args, options);
...
export async function gitReadText(
  session: GitReadSession,
  args: readonly string[],
  options: Readonly<{
    input?: Buffer;
  ...
  return decodeExactUtf8(await gitReadBytes(session, args, options), options.label ?? 'Git output');
}
...
function consumeGitRecords(session: GitReadSession, count: number, label: string): void {
  const failure = session.consumeRecords(count);
  if (failure !== null) {
    throw new Error(`${label} exceeded the Git read record budget: ${failure.reason}`);
  }
}
...
export async function resolveExactHeadCommit(session: GitReadSession): Promise<string> {
...
export async function readCommitBlobInventory(
  session: GitReadSession,
  commit: string
): Promise<readonly GitTreeBlobEntry[]> {
  ...
  const fields = parseNulUtf8(
    await gitReadBytes(
      ...
      { maxBuffer: GIT_READ_COMMAND_MAX_BUFFER, label: 'git ls-tree' }
    ),
    'git ls-tree'
  );
  consumeGitRecords(session, fields.length, 'git ls-tree');
...
export async function readBlobBatch(
  ...
  maxBuffer = GIT_READ_COMMAND_MAX_BUFFER
  ...
  const output = await gitReadBytes(
    session,
    ['cat-file', '--batch'],
    {
  ...
  consumeGitRecords(session, uniqueIds.length, 'git cat-file --batch');
  return blobs;
}

export async function readBlobEntryBatch(
  session: GitReadSession,
  entries: readonly GitTreeBlobEntry[]
): Promise<ReadonlyMap<string, Buffer>> {
  ...
  const blobs = await readBlobBatch(
    session,
    [...expectedSizes.keys()],
    Math.max(1024 * 1024, Math.min(GIT_READ_COMMAND_MAX_BUFFER, maxBuffer))
  );
...
interface IsolatedAttributeReader {
  readonly read: (paths: readonly string[]) => Promise<ReadonlyMap<string, GitTextAttributes>>;
  readonly dispose: () => void;
}

async function createIsolatedAttributeReader(
  session: GitReadSession,
  sourceCommit: string
): Promise<IsolatedAttributeReader> {
  const exactCommit = assertGitObjectId(sourceCommit, 'Attribute source commit');
  const gitObjectsPath = (await gitReadText(
    session,
  ...
  const objectDirectory = path.isAbsolute(gitObjectsPath)
    ...
    : path.resolve(session.cwd, gitObjectsPath);
  ...
  try {
    ...
    writeFileSync(
      path.join(shadowGitDir, 'objects', 'info', 'alternates'),
      `${objectDirectory}\n`,
      'utf8'
    );
  } catch (error) {
  ...
  const read = async (paths: readonly string[]): Promise<ReadonlyMap<string, GitTextAttributes>> => {
    ...
    const fields = parseNulUtf8(
      await gitReadBytes(
        ...
        ],
        {
          input,
          maxBuffer: Math.min(
            GIT_READ_COMMAND_MAX_BUFFER,
            Math.max(1024 * 1024, input.byteLength * 8 + 1024 * 1024)
          ),
    ...
    consumeGitRecords(session, paths.length, 'git check-attr');
...
export async function withIsolatedTextAttributeReader<Value>(
  session: GitReadSession,
  sourceCommit: string,
  execute: (
    readBatch: (paths: readonly string[]) => Promise<ReadonlyMap<string, GitTextAttributes>>
  ) => Value | Promise<Value>
): Promise<Value> {
  const reader = await createIsolatedAttributeReader(session, sourceCommit);
  try {
    return await execute(reader.read);
  } finally {
    reader.dispose();
  }
}
