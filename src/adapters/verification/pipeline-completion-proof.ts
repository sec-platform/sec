...
import { buildProvenance } from '../compilation/emit/write-provenance.ts';
...
async function readRequiredArtifactBytes(filePath: string, label: string): Promise<Uint8Array> {
  try { return new Uint8Array(await readFile(filePath)); }
  catch (error) { throw new Error(`Pipeline completion ${label} is missing or unreadable: ${String(error)}`); }
}
