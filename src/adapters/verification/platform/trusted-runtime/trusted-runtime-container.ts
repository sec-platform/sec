...
export const TRUSTED_RUNTIME_WORKSPACE_SETUP_SCRIPT = [
  ...
  `  mkdir -p ${TRUSTED_RUNTIME_DEPENDENCY_CACHE_CONTAINER_PATH}`,
  `  BUN_INSTALL_CACHE_DIR=${TRUSTED_RUNTIME_DEPENDENCY_CACHE_CONTAINER_PATH} CI=1 ${TRUSTED_RUNTIME_DEPENDENCY_PACKAGE_COMMAND.join(' ')}`,
  '  if [ "$mode" = "full" ]; then',
