...
async function publishReleaseSet(
  ...
  const failedName = `.sec-release-set-failed-${key}`;
  const failedRoot = path.join(parent.path, failedName);
  ...
  try {
    ...
    const retainedFailed = inspectExactNoFollowDirectoryPresence(failedRoot, 'Release set failed preimage');
    ...
    if (retainedBackup.state === 'present') {
      if (retainedDestination.state === 'absent') {
        ...
        try {
          ...
          findings.push(retainedFailedReleaseSetFinding(displaced, failureText(error)));
          relocateRetainedNoFollowDirectory({
