...
type ExactReleaseGitTreePhysicalRecord = {
  readonly stage: PhysicalDirectoryIdentity;
  readonly stageParent: PhysicalDirectoryIdentity;
  retired: boolean;
};

const exactReleaseGitTreePhysicalRecords =
  new WeakMap<object, ExactReleaseGitTreePhysicalRecord>();

export async function disposeExactReleaseGitTree(
  source: ExactReleaseGitTree
): Promise<void> {
  const record = exactReleaseGitTreePhysicalRecords.get(source);
  if (record === undefined) {
    throw new Error('Exact release Git tree was not issued by its materialization owner');
  }
  if (record.retired) return;
  retireReleaseTree(record.stage, record.stageParent, 'Exact release Git tree cleanup');
  record.retired = true;
}
...
async function materializeEntry(
  ...
  const parent = ensureOrdinaryParent(sourceRoot, entry.path, operation);
  ...
  const receipt = publishExclusiveDurableCanonicalFile({
    ...
    permissionMode: mode,
    validate: (current) => {
      if (!Buffer.from(current).equals(bytes) || current.byteLength !== entry.byteSize) {
  ...
  if (observed === null || observed.device !== receipt.physical.device
      || observed.inode !== receipt.physical.inode || observed.size !== entry.byteSize
      || observed.bytes === null || !Buffer.from(observed.bytes).equals(bytes)) {
    throw new Error(`Release source materialization identity differs from Git blob: ${entry.path}`);
  }
...
export async function materializeExactReleaseGitTree(
  ...
  return withAuthorityGitReadSession({
    ...
    const stageParent = materializeReleaseDirectory(
      operation.stageParent,
      'Exact release Git tree stage parent'
    );
    const stage = allocateReleaseStage(stageParent, 'sec-release-source-');
    const sourceRoot = createNoFollowOrdinaryDirectoryChain(
      stage,
      ['source'],
      undefined,
      0o700
    ...
    try {
      for (const batch of chunkEntries(entries)) {
        ...
        for (const entry of batch) {
          ...
          await materializeEntry(sourceRoot, entry, bytes, operation);
        }
      }
      ...
      const issued = Object.freeze({
        ...
        stageRoot: stage.path,
        sourceCommit,
        sourceTree,
        fileCount: entries.length
      });
      exactReleaseGitTreePhysicalRecords.set(issued, {
        stage,
        stageParent,
        retired: false
      });
      ...
      try {
        // Failure cleanup has a fresh physical recovery budget and is not
        // suppressed by the expired/cancelled Git-read execution budget.
        retireReleaseTree(stage, stageParent, 'Failed exact release Git tree stage');
      } catch (cleanupError) {
        throw new AggregateError(
          [error, cleanupError],
          `Exact release Git tree materialization failed and staging cleanup did not converge: ${stage.path}`
        );
      }
      throw error;
    }
