...
function compileOperation(
  ...
  switch (kind) {
    ...
    case 'code-scanning-alerts':
      return read(
        `/repos/${repo}/code-scanning/alerts?state=open&tool_name=CodeQL&pr=${positiveInteger(operation.pullRequestNumber, 'pull request number')}&per_page=100&page=${page(operation.page)}`
      );
