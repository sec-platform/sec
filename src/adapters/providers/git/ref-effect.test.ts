  ...
  try {
    ...
    try {
      const receipt = await deleteExactGitRef({
        ...
        ref: 'refs/heads/absent-topic',
        expectedOldSha: sha
      });
      assertGitRefDeleteReceipt(receipt);
      expect(receipt.disposition).toBe('already-absent');
      expect(receipt.updateOrdinal).toBeNull();
      // Admission observes the ref, worktree discovery runs next, and only
      // the third native command can witness absence at receipt issuance.
      expect(receipt.readbackOrdinal).toBe(3);
    } finally {
      closeProvider(opened);
    }
  } finally {
...
test('GitRefEffect rejects wrong-preimage, symbolic, foreign, and worktree-bound targets', async () => {
  const root = fixture();
