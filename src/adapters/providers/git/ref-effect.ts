...
/**
 * Delete one already-authorized local or remote-tracking ref by exact preimage.
 * Default-branch identity belongs to the caller's domain contract; this Effect
 * owner rejects symbolic refs and observes worktree bindings.
 */
export async function deleteExactGitRef(input: Readonly<{
  ...
  if (before.observation === null) {
    // Worktree discovery is an awaited native observation. An absent ref can
    // appear during that interval, so the receipt must bind the last ref
    // observation rather than the admission snapshot.
