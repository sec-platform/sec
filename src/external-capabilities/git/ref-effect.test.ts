...
test('GitRefEffect CAS-deletes exact local and remote-tracking refs with absence readback', async () => {
  const root = fixture();
  try {
    const sha = git(root, ['rev-parse', 'HEAD']);
    git(root, ['branch', 'topic']);
    git(root, ['update-ref', 'refs/remotes/origin/topic', sha]);
    for (const ref of ['refs/heads/topic', 'refs/remotes/origin/topic']) {
      ...
      try {
        ...
        expect(receipt.expectedOldSha).toBe(sha);
        expect(receipt.updateOrdinal).toBe(3);
        expect(receipt.readbackOrdinal).toBe(4);
        expect(spawnSync('git', ['show-ref', '--verify', '--quiet', ref], { cwd: root }).status).toBe(1);
        expect(opened.processSession.processCount).toBe(4);
      } finally {
...
test('GitRefEffect returns an owner-issued already-absent observation without attempting deletion', async () => {
  const root = fixture();
  try {
    const opened = openProvider(root);
    try {
      const receipt = await deleteExactGitRef({
        ...
        expectedOldSha: git(root, ['rev-parse', 'HEAD'])
      });
      assertGitRefDeleteReceipt(receipt);
      expect(receipt.disposition).toBe('already-absent');
      expect(receipt.updateOrdinal).toBeNull();
      expect(receipt.readbackOrdinal).toBe(1);
      expect(opened.processSession.processCount).toBe(2);
    } finally {
      closeProvider(opened);
    }
  } finally {
...
test('GitRefEffect rejects symbolic, wrong-preimage, foreign, and worktree-bound targets before deletion', async () => {
  const root = fixture();
  ...
  try {
    ...
    try {
      ...
      expect(foreign.processSession.processCount).toBe(0);
    } finally {
      closeProvider(foreign);
    }

    const malformed = openProvider(root);
    try {
      await expect(deleteExactGitRef({
        provider: malformed.provider,
        ref: 'refs/heads/a..b',
        expectedOldSha: sha
      })).rejects.toThrow(/bounded option-safe Git branch name/u);
      expect(malformed.processSession.processCount).toBe(0);
    } finally {
      closeProvider(malformed);
    }

    for (const [ref, expectedOldSha, message] of [
      ['refs/remotes/origin/HEAD', sha, /symbolic ref/u],
      ['refs/remotes/origin/topic', '0'.repeat(40), /preimage does not match/u],
      ['refs/heads/bound', sha, /bound to any registered worktree/u]
    ] as const) {
      const opened = openProvider(root);
      try {
        await expect(deleteExactGitRef({ provider: opened.provider, ref, expectedOldSha }))
          .rejects.toThrow(message);
        expect(git(root, ['rev-parse', '--verify', ref])).toBe(sha);
      } finally {
        closeProvider(opened);
      }
    }
    // Native ref removal leaves the registered worktree's symbolic HEAD as
    // a live consumer. Ref absence alone must not issue a terminal receipt.
    git(root, ['update-ref', '-d', 'refs/heads/bound', sha]);
    const absentBound = openProvider(root);
    try {
      await expect(deleteExactGitRef({ provider: absentBound.provider,
        ref: 'refs/heads/bound', expectedOldSha: sha })).rejects.toThrow(/bound to any registered worktree/u);
      expect(git(linked, ['symbolic-ref', 'HEAD'])).toBe('refs/heads/bound');
      expect(absentBound.processSession.processCount).toBe(2);
    } finally {
      closeProvider(absentBound);
    }
  } finally {
    try { git(root, ['worktree', 'remove', '--force', linked]); } catch { /* fixture cleanup */ }
...
test('GitRefEffect reports ambiguous native failure and never replays the same provider capability', async () => {
  ...
  try {
    const sha = git(root, ['rev-parse', 'HEAD']);
    git(root, ['branch', 'topic']);
    writeFileSync(path.join(root, '.git', 'refs', 'heads', 'topic.lock'), 'contended\n', 'utf8');
    const opened = openProvider(root);
    try {
      await expect(deleteExactGitRef({
        provider: opened.provider,
      ...
      expect(git(root, ['rev-parse', '--verify', 'refs/heads/topic'])).toBe(sha);
      await expect(deleteExactGitRef({
        provider: opened.provider,
        ref: 'refs/heads/topic',
        expectedOldSha: sha
