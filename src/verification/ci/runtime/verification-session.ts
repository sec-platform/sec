...
import { VERIFICATION_SESSION_RUNTIME_ENTRYPOINT_PATH, VERIFICATION_REGISTRY_PROJECTION_SCHEMA, parseVerificationSession, type VerificationRegistryEntry, type VerificationSession } from '../../session/contract/session.ts';
...
} from '../../../control/branch-lifecycle/branch-closeout-contract.ts';
import {
...
} from '../../../control/branch-lifecycle/branch-closeout-receipt.ts';
import {
...
} from '../../../control/branch-lifecycle/branch-closeout.ts';
import { assertGitBranchName, branchLifecycleDigest } from '../../../control/branch-lifecycle/branch-lifecycle-audit.ts';
import {
...
} from '../../../control/branch-lifecycle/branch-lifecycle-command.ts';
import { collectBranchLifecycleInventory } from '../../../control/branch-lifecycle/branch-lifecycle-inventory.ts';
import {
  BRANCH_REF_CLOSEOUT_CAPABILITY,
  type BranchCloseoutAttempt,
  type BranchCloseoutPreparation,
  type BranchLifecycleInventory
} from '../../../control/branch-lifecycle/branch-lifecycle-types.ts';
import { verifyRecoveryAuthorityLive } from '../../../control/branch-lifecycle/branch-recovery.ts';
import {
...
} from '../../../control/integration/integration-authorization-publication.ts';
import {
  observeGitHubIssue,
...
} from '../../../control/branch-lifecycle/local-main-closeout.ts';
...
} from '../../../control/integration/merge-gate.ts';
...
function blobDigest(
  ...
  if (result.status !== 0) {
    throw new Error(
      `Cannot read manifest blob ${filePath} at ${ref}: ${decodeBranchLifecycleChildError(result)}`
    );
  }
...
export function collectOpenPullRequestEntries(
  repositoryRoot: string,
  pullRequests: readonly OpenPullRequestFact[]
): VerificationRegistryEntry[] {
...
export function projectWorkPackageRegistry(input: {
  ...
  defaultBranch: string;
  defaultRef: string;
  pullRequestEntries?: VerificationRegistryEntry[];
...
const USAGE = `Usage:
  bun src/verification/ci/runtime/verification-session.ts project --default-ref <ref> [--open-prs true] [--repository <owner/name>] [--json]
  bun src/verification/ci/runtime/verification-session.ts prepare --pr <n> --request-output <request.json> [--repository <owner/name>] [--json]
  bun src/verification/ci/runtime/verification-session.ts observe-hosted --request <request.json> --output <facts.json> [--json]
  bun src/verification/ci/runtime/verification-session.ts prepare-hosted --request <request.json> --facts <facts.json> --output <envelope.json> [--json]
  bun src/verification/ci/runtime/verification-session.ts artifact-status --artifact <artifact.json> [--json]
  bun src/verification/ci/runtime/verification-session.ts finalize-hosted --envelope <envelope.json> (--evidence <evidence.json> | --previous-artifact <artifact.json>) --output <artifact.json> [--json]
  bun src/verification/ci/runtime/verification-session.ts prepare-integration-hosted --repository <owner/name> --output <projection.json> [--json]
  bun src/verification/ci/runtime/verification-session.ts integrate-hosted --repository <owner/name> --output <projection.json> [--json]
  bun src/verification/ci/runtime/verification-session.ts local-main-closeout --repository <owner/name> --pr <n> --protected-root <path> --expected-local-head <sha> [--json]
  bun src/verification/ci/runtime/verification-session.ts closeout-mutate-hosted --repository <owner/name> --output <projection.json> [--json]
  bun src/verification/ci/runtime/verification-session.ts closeout-publish-hosted --repository <owner/name> --output <projection.json> [--json]
  bun src/verification/ci/runtime/verification-session.ts resume --request <request.json> [--repository <owner/name>] [--json]
  bun src/verification/ci/runtime/verification-session.ts freeze --artifact <artifact.json> --session-output <session.json> --scope-output <scope.json> [--json]
  bun src/verification/ci/runtime/verification-session.ts status --request <request.json> [--repository <owner/name>] [--json]
  bun src/verification/ci/runtime/verification-session.ts status-offline --session-file <session.json> [--json]
`;
