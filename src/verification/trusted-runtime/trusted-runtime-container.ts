...
import { createVerificationActionKey, createVerificationActionPlan, createVerificationActionTerminal, encodeVerificationActionData, type VerificationActionKeyDigest, type VerificationActionKey, type VerificationActionPlan } from '../action/contract/action.ts';
...
async function reclaimAbandonedTrustedRuntimeContainers(input: Readonly<{
  ...
  for (const id of ids) {
    ...
    try {
      first = await inspectContainerIdentity(input.repositoryRoot, input.dockerEndpoint, id);
    } catch (error) {
    ...
    try {
      confirmed = await inspectContainerIdentity(input.repositoryRoot, input.dockerEndpoint, first.id);
    } catch (error) {
