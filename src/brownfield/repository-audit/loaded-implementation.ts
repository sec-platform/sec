...
export function requireRepositoryAuditLoadedImplementationObservation(
  value: unknown
): RepositoryAuditLoadedImplementationObservation {
  if (value === null || typeof value !== 'object'
      || !issuedLoadedImplementationObservations.has(value)) {
    throw new Error('Repository Audit loaded implementation observation was not owner-issued.');
  }
  const observation = value as RepositoryAuditLoadedImplementationObservation;
  const record = loadedImplementationObservationRecords.get(value);
  if (record === undefined
      || observation.entrypointAddress !== record.producerClosure.entrypoint.address
      || observation.implementationDigest !== record.request.implementationDigest
      || observation.operationIdentityDigest !== record.operation.plan.identity.identityDigest
      || observation.boundAttemptDigest !== record.operation.boundAttemptDigest) {
    throw new Error('Repository Audit loaded implementation observation binding changed.');
  }
  return observation;
}
