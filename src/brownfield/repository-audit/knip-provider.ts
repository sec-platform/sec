...
export type KnipProviderDiagnostic = Readonly<{
  provider: 'knip';
  status: 'unresolved';
  reason:
...
export async function executeKnipUnusedSymbolProvider(
  ...
  try {
    ...
    try {
      await settlePhysicalResourcesAsync({
        ...(primaryError === undefined ? {} : { primary: { label: 'Knip provider execution', error: primaryError } }),
        cleanup
      });
  ...
  if (primaryError !== undefined) {
    return unresolved(executionFailed ? 'execution-failed' : 'physical-boundary-unsettled', primaryError);
  }
