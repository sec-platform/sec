...
import { createExclusiveNoFollowDirectory, deleteRetainedNoFollowEntry, inspectExactNoFollowDirectoryPresence, inspectNoFollowDirectoryChain, inspectNoFollowOrdinaryFileEntry, publishExclusiveDurableCanonicalFile, replaceDurableCanonicalFile, scanNoFollowDirectoryTreeMetadata, scanNoFollowDirectoryTree, type PhysicalDirectoryChain, type PhysicalDirectoryIdentity } from '../../runtime-state/physical/runtime/physical-no-follow.ts';
import { PhysicalNoFollowError } from '../../runtime-state/physical/runtime/physical-no-follow.ts';
import { createAuthorityGitReadSession, isolatedGitReadEnvironment, type GitReadProviderResolutionFailure, type GitReadSessionResolution, type GitReadSession } from '../../external-capabilities/git-read/runtime/session.ts';
...
const DEPS_ENSURE_COMMAND = 'bun ./src/development/runner/cli.ts deps:ensure';
const HOOKS_RECONCILE_COMMAND = 'bun ./src/development/hooks/install.ts --lifecycle';
const IMPORTS_FREEZE_COMMAND = 'bun ./src/development/runner/cli.ts imports:freeze';
...
function hasValidatedInheritedCommonLineage(input: {
  ...
  const worktreeConfiguredPath = resolveGitPath(input.repoRoot, input.worktreeConfigured);
  return pathWithin(commonRoot.target.path, generationPath)
    ...
    && canonicalPath(worktreeConfiguredPath) === canonicalPath(generationPath)
    && canonicalPath(commonConfiguredPath) === canonicalPath(worktreeConfiguredPath);
}
...
async function installGitHooksInternalWithBudget(options: {
  ...
  const inheritedCommonLineage = hasValidatedInheritedCommonLineage({
    isPrimaryWorktree,
    markerRoot: bootstrapMarkerRootExisting,
    marker: bootstrapMarker,
    commonRoot: existingCommonRoot,
