...
import { countLineDiff } from '../../system-architecture/foundation/runtime/diff.ts';
...
import { REPAIR_PLAN_FORMAT_VERSION, validateRepairPlan, type RepairBlocker, type RepairFailurePoint, type RepairPlan, type RepairTask, type RepairTaskPreview } from '../../semantic/repair/contract/types.ts';
...
function prepareRepairWriteTasks(
  ...
  for (const task of prepared) {
    const identity = portableLogicalPathCollisionKey(task.repairTask.targetFile, 'Repair target');
...
function readRepairPreviewTarget(targetPath: string, targetFile: string): string {
  ...
  return bytes === null ? '' : decodeExactUtf8(bytes, `Repair preview target ${targetFile}`);
}
