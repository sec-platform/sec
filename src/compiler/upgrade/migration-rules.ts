...
export function readOwnJsonMutationValue(object: Record<string, unknown>, key: string): unknown {
...
function assertSafeJsonMergeObject(value: Record<string, unknown>, field: string): void {
  for (const key of Object.keys(value)) {
    ...
    const child = readOwnJsonMutationValue(value, key);
...
export function validateDbExpandContractMigrationEntry({
  ...
  if (entry.copyJobCode !== undefined) {
    ...
    dbExpandContractCopyJobProjectPath(entry);
  }
}

/** The planned impact and the physical writer must use this same path. */
export function dbExpandContractCopyJobProjectPath(
  ...
  return `src/jobs/db-migrations/${entry.id}.ts`;
}
...
export function applyConfigUpdates(config: unknown, updates: Array<{ path: string[]; value?: unknown; operation?: 'set' | 'delete' }>): unknown {
  ...
  for (const update of updates) {
    ...
    for (const segment of update.path.slice(0, -1)) {
      ...
      current = readOwnJsonMutationValue(current, segment) as Record<string, unknown>;
    }
...
function resolveJsonArrayTarget(
  ...
  return { root, parent: current, key, target: readOwnJsonMutationValue(current, key) };
}
...
function mergeJsonObjects(target: Record<string, unknown>, source: Record<string, unknown>): void {
  for (const key of Object.keys(source)) {
    ...
    const value = readOwnJsonMutationValue(source, key);
    const existing = readOwnJsonMutationValue(target, key);
