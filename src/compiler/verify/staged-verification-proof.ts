...
import { cloneAndDeepFreeze, deepFreeze, rawSha256 } from '../../system-architecture/foundation/runtime/canonical.ts';
...
async function consumeProof(
  ...
  const state = issuedProofs.get(proof);
  if (!state || state.consumed || state.consuming ||
...
export async function consumeStagedVerificationProof(
  ...
  return consumeProof(liveProjectRoot, lock, proof);
}
...
export async function revalidateStagedVerificationProof(
  ...
  return revalidateProof(liveProjectRoot, lock, proof);
}
