...
import { loadCanonicalBunRuntimeVersion } from '../../toolchain/runtime.ts';
...
async function buildIsolatedRunnerBundle(): Promise<Uint8Array> {
  ...
  const sourceBundle = await readSemanticMutationIsolatedRunnerBuildOutput(
    () => Bun.build({
      entrypoints: ['src/compiler/orchestration/semantic-mutation-isolated-verification-runner.ts'],
      format: 'esm',
      minify: {
        whitespace: true,
        syntax: true,
