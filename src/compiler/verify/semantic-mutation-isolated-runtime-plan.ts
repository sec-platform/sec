...
const SEMANTIC_MUTATION_ISOLATED_COMPILER_RELATIVE_ROOT = '.isolated-compiler' as const;
...
const SEMANTIC_MUTATION_ISOLATED_COMPILER_RUNTIME_ASSET_RELATIVE_ROOT =
...
export type SemanticMutationIsolatedRuntimeInputSources =
...
interface SemanticMutationIsolatedRuntimePlan {
  readonly bootstrapBundle: Uint8Array;
  readonly dependencyBinding: Uint8Array;
  readonly directories: readonly string[];
  readonly files: readonly RuntimeInputFile[];
...
const runtimePlansByBinding = new WeakMap<object, SemanticMutationIsolatedRuntimePlan>();
...
function canonicalPlanPayload(plan: Omit<SemanticMutationIsolatedRuntimePlan, 'planRevision'>): unknown {
  return {
...
export async function issueSemanticMutationIsolatedRuntimeCapability(input: {
  ...
  try {
    ...
    } satisfies Omit<SemanticMutationIsolatedRuntimePlan, 'planRevision'>;
...
function requireRuntimePlan(binding: unknown): SemanticMutationIsolatedRuntimePlan {
...
function generatedBytes(plan: SemanticMutationIsolatedRuntimePlan, relativePath: string): Uint8Array {
...
export async function assertSemanticMutationRuntimeDestinationManifestForTests(input: {
  ...
  } as SemanticMutationIsolatedRuntimePlan;
  const inspector = await createReparsePointInspector();
  try {
