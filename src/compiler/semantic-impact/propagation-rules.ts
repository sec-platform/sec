...
export function assertImpactPropagationRuleRegistry(
  ...
  >) {
    ...
    if (
      ...
      fail(`Impact edge rule "${predicate}" must match its canonical tuple`, {
        predicate,
        expected,
        actual: rule
      });
  ...
  if (unexpectedEdge) {
    fail(`Impact registry contains unexpected executable edge "${unexpectedEdge}"`, {
