...
} from './generated-state-lifecycle.ts';

type GeneratedStateOperationV1 = 'inspect' | 'plan' | 'cleanup';

function parseGeneratedStateOperationV1(args: readonly string[]): Readonly<{
  operation: GeneratedStateOperationV1;
...
export async function runGeneratedStateOperationV1(
  args: readonly string[],
  repositoryRoot = process.cwd()
): Promise<unknown> {
  const parsed = parseGeneratedStateOperationV1(args);
