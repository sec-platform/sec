...
type TextCensusThresholdAdmission<C extends string = string> =
  | Readonly<{ status: 'accepted'; threshold: 'any' | C | undefined }>
  | Readonly<{ status: 'rejected'; expected: readonly string[] }>;

type TextCensusProjection = Readonly<{
  value: unknown;
  text: string;
  thresholdMatched: boolean;
}>;

export interface TextCommandOperations<C extends string = string> {
  admitThreshold(value: string | undefined): TextCensusThresholdAdmission<C>;
  runCensus(
    workspaceRoot: string,
    threshold: 'any' | C | undefined
...
export function bindTextCommandHandlers<C extends string>(
  operations: TextCommandOperations<C>
): TextCommandHandlers {
  ...
  return Object.freeze<TextCommandHandlers>({
