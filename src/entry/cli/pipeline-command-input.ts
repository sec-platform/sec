import {
  bindPipelineCliInvocation,
  PIPELINE_DEFAULT_VERIFICATION_LANE,
  rejectPipelineOutputIssue as rejectPipelineOutputMessage
} from '../../application/pipeline-request.ts';
...
export function rejectPipelineOutputIssue(issue: JsonOutputIssue): never {
  return rejectPipelineOutputMessage(issue.message);
}
...
export function parsePipelineCompileOptions(options: Readonly<Record<string, unknown>>) {
  const { json, compact, from, through, lane } = captureCliOptions(options,
    ['json', 'compact', 'from', 'through', 'lane'].map((name) => ({ name, scope: 'property' as const })));
  const output = parsePipelineOutputOptions({ json, compact });
  const invocation = bindPipelineCliInvocation({ from, through, verificationLane: lane });
  return Object.freeze({ output, invocation });
}
