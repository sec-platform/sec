...
export const SEMANTIC_GENERATOR_TASK_STATUSES = ['pending', 'generated', 'verified', 'failed'] as const;

type SemanticGeneratorConsumeKind = (typeof SEMANTIC_GENERATOR_CONSUME_KINDS)[number];
type SemanticGeneratorArtifactKind = (typeof SEMANTIC_GENERATOR_ARTIFACT_KINDS)[number];
type SemanticGeneratorTaskStatus = (typeof SEMANTIC_GENERATOR_TASK_STATUSES)[number];

interface ManifestGeneratorTypeBinding {
  name: string;
  importFrom: string;
}
...
interface SemanticGeneratorArtifactBinding {
  generatorEntityId: string;
  artifactEntityId: string;
  semanticRevision: string;
