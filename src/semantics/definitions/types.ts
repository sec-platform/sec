...
type SemanticEffectKind = (typeof SEMANTIC_EFFECT_KINDS)[number];
...
interface SemanticContractField {
  id: string;
  type: string;
  required?: boolean;
  mutable?: boolean;
...
interface SemanticContractEvent {
  id: string;
  label?: string;
  payloadType?: string;
}

interface SemanticContractPolicy {
  id: string;
  label?: string;
  rule?: string;
  verifiedBy?: string[];
...
interface SemanticContractPermission {
  id: string;
  label?: string;
  scope?: string;
}
