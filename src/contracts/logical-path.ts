...
export interface CanonicalPortableLogicalPathOptions {
 ...
 */
export function isCanonicalPortableLogicalPath(
  value: string,
  options: CanonicalPortableLogicalPathOptions = {}
): boolean {
 ...
 */
export function isCanonicalPortableLogicalPathPrefix(value: string): boolean {
  return value.endsWith('/')
    && value.length > 1
    && isCanonicalPortableLogicalPath(value.slice(0, -1));
}

export function assertCanonicalPortableLogicalPath(
  value: string,
  label = 'Logical path',
  options: CanonicalPortableLogicalPathOptions = {}
): string {
  if (!isCanonicalPortableLogicalPath(value, options)) {
 ...
 */
export function portableLogicalPathCollisionKey(
  value: string,
  label = 'Logical path'
): string {
  const canonical = assertCanonicalPortableLogicalPath(value, label);
  return canonical
    .split('/')
