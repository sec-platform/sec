...
import { createNoFollowDirectoryChain, deleteRetainedNoFollowEntry, inspectExactNoFollowDirectoryPresence, inspectNoFollowDirectoryChain, publishExclusiveDurableCanonicalFile, readNoFollowOrdinaryFile, relocateRetainedNoFollowDirectoryAcrossParents, relocateRetainedNoFollowDirectory, replaceDurableCanonicalFile, scanNoFollowDirectoryTreeInventory, scanNoFollowDirectoryTree, type PhysicalDirectoryIdentity } from '../../runtime-state/physical/runtime/physical-no-follow.ts';
...
import { acquireWorkspaceWriteLease, assertWorkspaceWriteLease, assertWorkspaceWriteLeaseRetirementProof, assertWorkspaceWriteLeaseRetirement, completeWorkspaceWriteLeaseRetirement, recoverWorkspaceWriteLeaseRetirement, resumeWorkspaceWriteLeaseRetirement, withWorkspaceWriteLease, type WorkspaceWriteLeaseRetirementReceipt } from '../../workspace/lease.ts';
...
export function observeWorktreePhysicalInventory(targetPathInput: string): WorktreePhysicalInventory {
...
export async function prepareWorktreePhysicalCloseout(
  ...
  try {
    ...
    try {
      ...
      return await prepareWorktreePhysicalCloseoutUnderLease(input, repository, targetLease, leaseHeldWorking, generatedStateRetirement);
    } finally {
...
async function main(): Promise<void> {
  ...
  const usage =
    'Usage:\n' +
    '  bun src/control/branch-lifecycle/worktree-physical-closeout.ts prepare --repository-root <path> --target <path> --expected-branch <name> --expected-head <sha> --expected-tree <sha> --expected-recovery-authority <digest>\n' +
    '  bun src/control/branch-lifecycle/worktree-physical-closeout.ts execute --repository-root <path> --target <path> --expected-branch <name> --expected-head <sha> --expected-tree <sha> --expected-recovery-authority <digest> --authorization <path>';
