...
async function configureWithSession(
  ...
  if (providerResolution.status !== 'ready') {
    throw new Error(`Branch lifecycle Git config provider is unavailable: ${providerResolution.reason}`);
  }
  ...
  try {
    for (const key of [
      ...
      const target = await issueGitConfigTargetCapability({ path: configPath });
      try {
...
export async function configureBranchLifecycleClone(
  ...
  let result: BranchPruneConfigurationObservation | undefined;
  let primaryError: unknown;
  try {
  ...
  if (primaryError !== undefined) throw primaryError;
  if (result === undefined) throw new Error('Branch lifecycle Git config operation completed without a result.');
  return result;
}
