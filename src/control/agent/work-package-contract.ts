...
import { CI_VERIFICATION_CONTRACT_REVISION } from '../../verification/ci/contract/plan.ts';

export const CodexDevelopmentWorkPackageSchema = 'codex-development-work-package-v1' as const;
...
export type CodexDevelopmentWorkPackageTask = {
  id: string;
  owner: string;
  ownedPaths: string[];
};
...
export function CodexDevelopmentDecodeWorkPackageManifest(
  ...
  tests.forEach((testPath, index) => {
    ...
    if (!/^(?:src|tests)\/(?:[^/]+\/)*[^/]+\.test\.ts$/u.test(testPath)) {
      throw new Error(`${label} must name one canonical co-located or system .test.ts module.`);
    }
  });
...
export function CodexDevelopmentAssertCurrentWorkPackageRevision(
