...
import { createExclusiveNoFollowDirectory, createNoFollowOrdinaryDirectoryChain, deleteRetainedNoFollowEntry, inspectExactNoFollowDirectoryPresence, inspectNoFollowDirectoryChain, inspectNoFollowOrdinaryFileEntry, publishExclusiveDurableCanonicalFile, readNoFollowOrdinaryFile, retainNoFollowDirectoryForChildProcess, retainNoFollowOrdinaryFileForChildProcess, scanNoFollowDirectoryTreeInventory, type NoFollowDirectoryTreeInventoryEntry, type PhysicalDirectoryIdentity, type RetainedNoFollowChildProcessDirectory, type RetainedNoFollowChildProcessFile } from '../../../runtime-state/physical/runtime/physical-no-follow.ts';
...
import {
  ...
  isDocsDoctorGitObjectId,
  type DocsDoctorGitObjectFormat
} from '../index-snapshot.ts';
import { scanMachineLedgers } from './ledgers.ts';
import {
...
async function scanControlPlanes(
  ...
  try {
    ...
    CodexDevelopmentAssertControlPlaneBinding({ spec: currentState, pointer });
...
export async function scanDocumentation(
  ...
  for (const record of registry.documents) {
    ...
    if (
      ...
      || !CodexDevelopmentIsCanonicalRepositoryPath(record.path)
    ) {
      pushIssue(issues, {
        level: 'error',
  ...
  try {
    ...
    if (options.readDefaultBranchBlob !== undefined) {
      ...
      const priorCatalog = parseSecRoadmapWorkCatalog(priorRoadmapSource);
...
export interface DocsDoctorIndexTreeSnapshot {
  readonly objectFormat: DocsDoctorGitObjectFormat;
...
function ancestorInventory(
  ...
  directories: ReadonlyMap<string, NoFollowDirectoryTreeInventoryEntry>
) {
...
function deleteDocsDoctorSnapshot(
  snapshotParent: PhysicalDirectoryIdentity,
  snapshotRoot: PhysicalDirectoryIdentity
): void {
  const inventory = scanNoFollowDirectoryTreeInventory(snapshotRoot);
...
export function captureDocsDoctorIndexTree(
  ...
  const snapshotToken = randomUUID().replaceAll('-', '')
    .slice(0, DOCS_DOCTOR_INDEX_SNAPSHOT_LAYOUT.snapshotTokenHexLength);
  const snapshotLayout = compileDocsDoctorIndexSnapshotLayout({
    cacheRoot,
    objectFormat,
    platform: process.platform,
    snapshotToken
  ...
  const indexParent = inspectNoFollowDirectoryChain(
    path.dirname(indexPath),
    'docs-doctor canonical Git index parent'
  ).target;
  const indexBytes = readNoFollowOrdinaryFile(indexParent, path.basename(indexPath));
  if (indexBytes === null) {
    throw new Error('docs-doctor: canonical Git index is absent');
  }
  const objectChain = inspectNoFollowDirectoryChain(
    objectDirectory,
    'docs-doctor canonical Git object directory'
  );
  const cacheAuthority = acquireSecRuntimeCachePhysicalAuthority({
    repositoryRoot: resolvedRepositoryRoot,
    cacheRoot,
    requiredDirectories: [snapshotParent]
  });
  const snapshotParentIdentity = cacheAuthority.directory(snapshotParent);
  const snapshotRoot = createExclusiveNoFollowDirectory(
    snapshotParentIdentity,
    snapshotLayout.snapshotName
  );
  let retained = false;
  let originalObjects: RetainedNoFollowChildProcessDirectory | null = null;
  let snapshotObjects: RetainedNoFollowChildProcessDirectory | null = null;
  let snapshotIndexFile: RetainedNoFollowChildProcessFile | null = null;
  try {
    const snapshotIndex = path.join(
      snapshotRoot.path,
      DOCS_DOCTOR_INDEX_SNAPSHOT_LAYOUT.indexName
    );
    const publishedIndex = publishExclusiveDurableCanonicalFile({
      parent: snapshotRoot,
      name: DOCS_DOCTOR_INDEX_SNAPSHOT_LAYOUT.indexName,
      bytes: indexBytes,
      validate: (observed) => {
    ...
    const snapshotRootChain = inspectNoFollowDirectoryChain(
      snapshotRoot.path,
      'docs-doctor snapshot root'
    );
    const publishedIndexEntry = inspectNoFollowOrdinaryFileEntry(
      snapshotRoot,
      DOCS_DOCTOR_INDEX_SNAPSHOT_LAYOUT.indexName
    );
    ...
    const snapshotObjectIdentity = createNoFollowOrdinaryDirectoryChain(
      snapshotRoot,
      [DOCS_DOCTOR_INDEX_SNAPSHOT_LAYOUT.objectDirectoryName]
    );
    originalObjects = retainNoFollowDirectoryForChildProcess(
      objectChain,
      3,
      'docs-doctor canonical Git object directory'
    );
    snapshotObjects = retainNoFollowDirectoryForChildProcess(
    ...
    snapshotIndexFile = retainNoFollowOrdinaryFileForChildProcess(
      snapshotRootChain,
      publishedIndexEntry,
      5,
    ...
    if (result.error || result.status !== 0 || !isDocsDoctorGitObjectId(treeSha, objectFormat)) {
 ...
 */
export function parseCapturedGitTreeBlobFrame(
  source: Uint8Array,
  repositoryPath: string,
  objectFormat: DocsDoctorGitObjectFormat = 'sha1'
): Buffer {
  if (!CodexDevelopmentIsCanonicalRepositoryPath(repositoryPath)) {
  ...
  if (match !== null && !isDocsDoctorGitObjectId(match[1]!, objectFormat)) {
...
export function readCapturedGitTreeBlob(
  ...
  if (!remoteRef && !isDocsDoctorGitObjectId(treeSha, objectFormat)) {
  ...
  return parseCapturedGitTreeBlobFrame(result.stdout, repositoryPath, objectFormat);
}

function listCapturedWorkPackagePaths(
  repositoryRoot: string,
  snapshot: DocsDoctorIndexTreeSnapshot
): readonly string[] {
