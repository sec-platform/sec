...
import { assertReviewStabilityReceiptCurrent, parseReviewStabilityReceipt, REVIEW_OBSERVER_PRODUCER_IDENTITY, renderIndependentReviewTrailer, type ReviewStabilityReceipt } from '../../verification/review/contract/stability.ts';
import {
...
export const CodexDevelopmentTrustedRuntimeMergeGateInputSchema =
...
export const CodexDevelopmentMergeGateProducerIdentity =
  'src/control/integration/merge-gate.ts' as const;
...
export interface CodexDevelopmentMergeGatePlatformObservation {
...
async function main(): Promise<void> {
  ...
  if ((command !== 'authorize' && command !== 'authorize-trusted-runtime') ||
    ...
    throw new Error(
      'Usage: bun src/control/integration/merge-gate.ts authorize|authorize-trusted-runtime --input <json> --output <json>'
    );
  }
