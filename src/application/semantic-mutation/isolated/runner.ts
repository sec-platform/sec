...
type SemanticMutationIsolatedRunnerFailureStage =
  | 'preflight'
  | 'verify-all'
  | 'postcondition';

type SemanticMutationIsolatedRunnerProgressCheckpoint =
  | 'catch-armed'
  | 'verify-all'
  | 'postcondition'
  | 'failure-caught'
...
interface SemanticMutationIsolatedUnsupportedStagedSources {
  readonly prismaTemplatePresent: boolean;
  readonly linkedOpaqueModulePresent: boolean;
}

interface SemanticMutationIsolatedCompileReceipt {
...
interface SemanticMutationIsolatedCompileObserver<TBoundary extends string> {
  onExecutionBoundary(boundary: TBoundary): void;
  onTransactionStart(): void;
}

export interface IsolatedRunnerOperations<TBoundary extends string> {
...
export type IsolatedRunnerResult<TBoundary extends string> =
  | Readonly<{
...
export class IsolatedCatchTreeFailure extends Error {
  constructor() {
...
export class IsolatedStagingTreeFailure extends Error {
  constructor() {
...
export async function runSemanticMutationIsolatedVerifyAll<TBoundary extends string>(
  operations: IsolatedRunnerOperations<TBoundary>
): Promise<IsolatedRunnerResult<TBoundary> | null> {
  try {
    await operations.assertStagingTree();
  } catch {
    throw new IsolatedStagingTreeFailure();
  }
  ...
  try {
    ...
    try {
      ...
      throw new IsolatedCatchTreeFailure();
    }
