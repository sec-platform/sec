...
import {
  assessIsolatedSemanticMutationArtifacts
} from '../assurance/verification/semantic-mutation/isolated-artifact-assessment.ts';
import type {
  SemanticMutationIsolatedVerificationEvidenceArtifacts
...
export interface SemanticMutationIsolatedArtifactSet
  extends SemanticMutationIsolatedVerificationEvidenceArtifacts {
  readonly status: 'passed' | 'failed';
  readonly stagedVerificationProofSource?: StagedVerificationProofSource;
}

export interface SemanticMutationIsolationFailureObservation {
  readonly stage: string;
}

export interface SemanticMutationIsolatedVerificationOperations {
  ...
  classifyFailure(error: unknown): SemanticMutationIsolationFailureObservation;
  blockedEvidenceDigest(failure: SemanticMutationIsolationFailureObservation): string;
  ...
  recordBlockedFailure?(failure: SemanticMutationIsolationFailureObservation): void;
...
export async function coordinateSemanticMutationIsolatedVerification(
  plan: ReadyPlan,
  capabilityPlan: SemanticMutationVerificationCapabilityPlan,
  operations: SemanticMutationIsolatedVerificationOperations
  ...
  const report = await executeVerification.call(operations, {
    ...
    try {
      ...
      await commitFence.call(operations);
      const assessment = assessIsolatedSemanticMutationArtifacts({
        attempted: plan.staged,
