...
function buildFailurePoints(report: VerificationReport): RepairFailurePoint[] {
  ...
  addFailedRepairPoint(points, report.policy.status, {
    ...
    message: uniqueSorted(
      report.policy.violations.map(violation => violation.message)
    ).join('; ') || 'Policy verification failed',
    targetIds: uniqueSorted(
      report.policy.violations.flatMap(violation => [violation.id, ...violation.files])
    )
  });
...
function buildRepairBlockers(
  failurePoints: readonly RepairFailurePoint[]
): RepairBlocker[] {
