...
import { defaultLimit } from '../../foundation/concurrency.ts';
...
function readOptionalScaffoldText(targetPath: string, relativePath: string): string | null {
  ...
  return bytes === null ? null : decodeExactUtf8V1(bytes, `Runtime library target ${relativePath}`);
}
