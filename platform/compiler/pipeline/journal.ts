...
import { formatJsonFile } from './fs.ts';
...
import { publishExistingParentCanonicalWorkspaceFileV1 } from './workspace-file-publication.ts';
...
const PIPELINE_SOURCES = new Set<PipelineSource>([
  'api', 'cli', 'workbench', 'reference', 'upgrade', 'repair', 'ci'
]);
...
function readJournal(workspaceRoot: string): PipelineJournal {
  ...
  let raw: unknown;
  try {
    raw = readOptionalRetainedJsonV1<unknown>(journalPath, 'Pipeline journal');
  } catch (error) {
    throw new Error(
...
async function publishJournal(
  ...
  await publishExistingParentCanonicalWorkspaceFileV1({
    workspaceRoot,
