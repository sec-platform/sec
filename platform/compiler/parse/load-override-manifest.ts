...
import { pathEntryExists } from '../../shared/fs.ts';
import {
...
import type {
  ...
  OverrideEntry,
  OverrideManifest
} from '../../shared/provenance-types.ts';
import {
  decodeExactUtf8V1,
...
export function validateOverrideManifest(manifest: OverrideManifest): OverrideManifest {
  ...
  for (const entry of manifest.overrides) {
    ...
    for (const phase of entry.appliesAfter) {
      ...
      const targetPhaseKey = `${phase}:${portableLogicalPathCollisionKeyV1(
        entry.target,
        'Override target'
      )}`;
...
export async function resolveOverrideManifestPath(workspaceRoot: string): Promise<string> {
  const {
    ...
    legacySourceOverridesRoot
  } = getWorkspacePaths(workspaceRoot);
  if (await pathEntryExists(overrideManifestPath)) return overrideManifestPath;

  const legacySourceOverrideManifestPath = path.join(
    legacySourceOverridesRoot,
    'override-manifest.yaml'
  );
  return (await pathEntryExists(legacySourceOverrideManifestPath))
    ? legacySourceOverrideManifestPath
    : legacyOverrideManifestPath;
}
...
export async function loadOverrideManifest(workspaceRoot: string): Promise<OverrideManifest> {
  ...
  const source = decodeExactUtf8V1(bytes, 'Override manifest');
