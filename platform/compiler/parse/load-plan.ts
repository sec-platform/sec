...
export function validatePlan(plan: PlanFile): void {
  ...
  for (const source of plan.registry.sources) {
    ...
    if (!isCanonicalPortableLogicalPathV1(source.path)) {
      throw new CompilerError(
        'PLAN-VALIDATION-012',
  ...
  for (const slot of plan.slots) {
    ...
    if (!isCanonicalPortableLogicalPathV1(slot.target) || !slot.target.startsWith('custom/')) {
      throw new CompilerError('PLAN-VALIDATION-008', `Slot "${slot.id}" must target canonical custom/ path in v0.1`);
    }
    if (
      slot.sourcePath &&
      (!isCanonicalPortableLogicalPathV1(slot.sourcePath) ||
