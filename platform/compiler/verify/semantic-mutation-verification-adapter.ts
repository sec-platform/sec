...
function normalizeRequirement(requirement: VerificationRequirementV1): VerificationRequirementV1 {
...
function capabilityPlanRevision(
  value: Omit<SemanticMutationVerificationCapabilityPlanV1, 'capabilityPlanRevision'>
): string {
...
export async function planSemanticMutationVerificationCapabilities(input: {
  ...
  readonly isolationCapabilityProbe?: SemanticMutationIsolationCapabilityProbeV1;
}): Promise<SemanticMutationVerificationCapabilityPlanV1> {
...
function requiredVerificationDigest(requirements: readonly VerificationRequirementV1[]): string {
...
function exactEndpoint(value: unknown): value is SemanticMutationBaseV2 {
...
function exactRequirement(value: unknown): value is VerificationRequirementV1 {
  if (!value || typeof value !== 'object' || Array.isArray(value)) return false;
...
export function assertSemanticMutationVerificationReportInvariant(
  value: unknown
): asserts value is SemanticMutationVerificationReportV1 {
  if (!exactKeys(value, [
  ...
  const report = value as unknown as SemanticMutationVerificationReportV1;
  ...
  const requirements: VerificationRequirementV1[] = [];
...
export async function executeSemanticMutationVerification(
  ...
  runner: SemanticMutationVerifyAllRunnerV1
): Promise<SemanticMutationVerificationReportV1> {
