...
export const SEMANTIC_MUTATION_ISOLATION_CAPABILITY_EVIDENCE_REVISION =
  'semantic-mutation-isolation-capability-evidence-v1' as const;

export type SemanticMutationIsolationCapabilityEvidenceV1 = Readonly<{
  formatRevision: typeof SEMANTIC_MUTATION_ISOLATION_CAPABILITY_EVIDENCE_REVISION;
  status: 'available' | 'unavailable' | 'unproven';
}>;

export type SemanticMutationIsolationCapabilityProbeV1 = () => unknown | Promise<unknown>;

const provenAvailableEvidence = new WeakSet<object>();

function evidence(
  status: SemanticMutationIsolationCapabilityEvidenceV1['status'],
  proven: boolean
): SemanticMutationIsolationCapabilityEvidenceV1 {
...
export async function probeSemanticMutationIsolationCapability(
  probe?: SemanticMutationIsolationCapabilityProbeV1
): Promise<SemanticMutationIsolationCapabilityEvidenceV1> {
...
export function hasProvenSemanticMutationIsolationCapability(
  value: unknown
): value is SemanticMutationIsolationCapabilityEvidenceV1 & { readonly status: 'available' } {
