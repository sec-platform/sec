...
function verificationExecutionRevision(
  value: Omit<SemanticMutationVerificationExecutionRefV2, 'verificationExecutionRevision'>
): string {
...
function verificationDiagnostic(
  plan: ReadyPlan,
  verification: SemanticMutationVerificationExecutionRefV2 | undefined,
  attempted: SemanticMutationBaseV2 | undefined
...
function resultRevision(value: Omit<SemanticMutationResultV2, 'resultRevision'>): string {
  return sha256({ domain: 'semantic-mutation-result-v2', ...value });
}

export function buildSemanticMutationResult(
  plan: SemanticMutationPlanV2,
  evidence?: SemanticMutationTerminalEvidenceV2
  ...
  if (plan.status === 'rejected') {
    ...
    return cloneAndDeepFreeze({ ...withoutRevision, resultRevision: resultRevision(withoutRevision) } as SemanticMutationResultV2);
  }
  ...
  if (evidence.status === 'rejected') {
    ...
    return cloneAndDeepFreeze({ ...rejected, resultRevision: resultRevision(rejected) } as SemanticMutationResultV2);
  }
  ...
  if ((evidence.verification as SemanticMutationVerificationExecutionRefV2).status !== 'passed') {
  ...
  return cloneAndDeepFreeze({ ...withoutRevision, resultRevision: resultRevision(withoutRevision as Omit<SemanticMutationResultV2, 'resultRevision'>) } as SemanticMutationResultV2);
}

export function semanticMutationResultRevision(
  value: Omit<SemanticMutationResultV2, 'resultRevision'>
): string {
  return resultRevision(value);
}

export function assertSemanticMutationResultInvariant(
  result: SemanticMutationResultV2,
  plan: SemanticMutationPlanV2
): void {
  assertSemanticMutationPlanInvariant(plan);
  ...
  if ((result.status === 'rolled-back' || result.status === 'recovery-required') &&
    (result.verification as SemanticMutationVerificationExecutionRefV2).status !== 'passed') {
