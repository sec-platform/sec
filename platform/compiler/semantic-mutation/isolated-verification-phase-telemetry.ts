...
export type SemanticMutationIsolatedPhase =
  (typeof SEMANTIC_MUTATION_ISOLATED_PHASES)[number];
export type SemanticMutationIsolatedPhaseState = 'started' | 'completed';

export interface SemanticMutationIsolatedPhaseTelemetryEventV1 {
  readonly formatVersion: typeof SEMANTIC_MUTATION_ISOLATED_PHASE_TELEMETRY_FORMAT;
  readonly phase: SemanticMutationIsolatedPhase;
  readonly state: SemanticMutationIsolatedPhaseState;
  readonly durationMs: number;
}

export type SemanticMutationIsolatedPhaseTelemetryReadResult =
  | Readonly<{
      readonly status: 'valid';
      readonly events: readonly SemanticMutationIsolatedPhaseTelemetryEventV1[];
    }>
...
const TELEMETRY_FILE_PREFIX = 'semantic-mutation-isolated-phase-telemetry-v1-';
...
function telemetryFileName(
  ...
  return `${pending ? '.' : ''}${TELEMETRY_FILE_PREFIX}${phase}-${state}.json`;
}
...
function eventBytes(event: SemanticMutationIsolatedPhaseTelemetryEventV1): Uint8Array {
...
async function publishEvent(
  stagingWorkspaceRoot: string,
  event: SemanticMutationIsolatedPhaseTelemetryEventV1
): Promise<void> {
  try {
...
async function resetPhases(
  stagingWorkspaceRoot: string,
  phases: readonly SemanticMutationIsolatedPhase[]
): Promise<void> {
  try {
...
export async function resetSemanticMutationIsolatedPhaseTelemetry(
  ...
  await resetPhases(stagingWorkspaceRoot, SEMANTIC_MUTATION_ISOLATED_PHASES);
}
...
export async function resetSemanticMutationIsolatedExecutionPhaseTelemetry(
  ...
  await resetPhases(
    ...
    SEMANTIC_MUTATION_ISOLATED_PHASES.filter((phase) => !SNAPSHOT_PHASES.has(phase))
  );
}

function parseEvent(
  bytes: Uint8Array,
  expectedPhase: SemanticMutationIsolatedPhase,
  expectedState: SemanticMutationIsolatedPhaseState
): SemanticMutationIsolatedPhaseTelemetryEventV1 | undefined {
...
async function readOptionalEvent(
  ...
  expectedPhase: SemanticMutationIsolatedPhase,
  expectedState: SemanticMutationIsolatedPhaseState
): Promise<SemanticMutationIsolatedPhaseTelemetryEventV1 | 'absent' | 'read-error' | 'protocol-error'> {
  let handle;
  try {
...
export async function readSemanticMutationIsolatedPhaseTelemetry(
  stagingWorkspaceRoot: string
): Promise<SemanticMutationIsolatedPhaseTelemetryReadResult> {
  try {
    const names = await readdir(telemetryRoot(stagingWorkspaceRoot));
    const ownedNames = new Set(SEMANTIC_MUTATION_ISOLATED_PHASES.flatMap((phase) =>
    ...
    if (names.some((name) =>
      (name.startsWith(TELEMETRY_FILE_PREFIX) || name.startsWith(`.${TELEMETRY_FILE_PREFIX}`)) &&
      !ownedNames.has(name))) {
  ...
  const events: SemanticMutationIsolatedPhaseTelemetryEventV1[] = [];
  for (const phase of SEMANTIC_MUTATION_ISOLATED_PHASES) {
...
export function semanticMutationIsolatedPhaseTelemetryOwnedPaths(
  stagingWorkspaceRoot: string
): readonly string[] {
  return Object.freeze(SEMANTIC_MUTATION_ISOLATED_PHASES.flatMap((phase) =>
    [...STATES].flatMap((state) => [
