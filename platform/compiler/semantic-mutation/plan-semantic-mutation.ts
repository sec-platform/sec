...
function impactRequirements(impact: ReturnType<typeof buildImpactPropagation>): VerificationRequirementV1[] {
...
function baseFields(
  ...
  verification: SemanticMutationInputV2['verificationPlanning'] | undefined
) {
  return {
...
function earlyRejection(
  preflight: Exclude<ReturnType<typeof preflightSemanticMutation>, { readonly rejectedAt: 'request' }>,
  stage: Exclude<SemanticMutationPlanV2, { status: 'ready' } | { rejectedAt: 'request' } | { rejectedAt: 'fact-delta' | 'expectation' | 'impact' | 'impact-verification' }>['rejectedAt'],
...
function validSourceChange(value: unknown): value is SemanticMutationSourceChangeV2 {
...
function planSemanticMutationWithProducers(
  ...
  const stageFields = {
    ...baseFields(preflight, undefined),
    sourceChanges: [structuredClone(sourceChange)] as [SemanticMutationSourceChangeV2],
...
export function assertSemanticMutationPlanInvariant(plan: SemanticMutationPlanV2): void {
