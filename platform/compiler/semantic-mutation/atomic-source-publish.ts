...
export interface SemanticMutationTransactionArtifactsV1 {
...
export async function writeSemanticMutationTransactionArtifacts(
  ...
  commitFence: SemanticMutationCommitFence
): Promise<SemanticMutationTransactionArtifactsV1> {
...
export async function readSemanticMutationTransactionArtifacts(
  ...
  const plan = JSON.parse(planJson) as SemanticMutationSourceEditPlanV1;
  const manifest = JSON.parse(manifestJson) as SemanticMutationRollbackManifestV2;
  assertSemanticMutationSourceEditArtifactsInvariant(plan, manifest);
