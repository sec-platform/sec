...
import {
  ...
  type SemanticMutationRequestRecordV1,
...
function recordRevision(
  value: Omit<SemanticMutationRecoveryRecordV1, 'recordRevision'>
): string {
...
function assertVerificationBinding(record: SemanticMutationRecoveryRecordV1): void {
...
function assertRecoveryCrossBindings(record: SemanticMutationRecoveryRecordV1): void {
...
export function assertSemanticMutationRecoveryRecordInvariant(
  ...
  previous?: SemanticMutationRecoveryRecordV1
): void {
...
export async function loadSemanticMutationRecoveryRecords(
  ...
  const records: SemanticMutationRecoveryRecordV1[] = [];
  for (const name of names) {
    await assertSemanticMutationRecoveryRecordsDirectory(transactionRoot);
    const serialized = await readFile(path.join(directory, name), 'utf8');
    await assertSemanticMutationRecoveryRecordsDirectory(transactionRoot);
    const parsed = JSON.parse(serialized) as SemanticMutationRecoveryRecordV1;
...
export async function appendSemanticMutationRecoveryRecord(
...
): Promise<SemanticMutationRecoveryRecordV1> {
  ...
  } as Omit<SemanticMutationRecoveryRecordV1, 'recordRevision'>;
  const record = cloneAndDeepFreeze({
    ...withoutRevision,
    recordRevision: recordRevision(withoutRevision)
  }) as SemanticMutationRecoveryRecordV1;
...
export function projectSemanticMutationRequestRecordView(
  record: SemanticMutationRecoveryRecordV1,
  suppliedIdentity?: SemanticMutationRequestIdentityV1
): Extract<SemanticMutationRequestRecordViewV1, { readonly recordKind: 'transaction' }>;
export function projectSemanticMutationRequestRecordView(
  record: SemanticMutationRejectedTerminalRecordV1,
  suppliedIdentity: SemanticMutationRequestIdentityV1
): Extract<SemanticMutationRequestRecordViewV1, { readonly recordKind: 'rejected-terminal' }>;
export function projectSemanticMutationRequestRecordView(
  record: SemanticMutationRequestRecordV1,
  suppliedIdentity?: SemanticMutationRequestIdentityV1
