...
function selector(value: unknown, label: string): SemanticFactSelectorV1 {
  const record = expectRecord(value, label);
...
function condition(value: unknown, label: string): SemanticMutationConditionV1 {
  const record = expectRecord(value, label);
...
function conditions(value: unknown, label: string): SemanticMutationConditionV1[] {
...
function operation(value: unknown, label: string): SemanticMutationOperationV1 {
  const record = expectRecord(value, label);
...
function operations(value: unknown): SemanticMutationOperationV1[] {
...
function expectedFact(value: unknown, label: string): ExpectedSemanticFactV1 {
  const record = expectRecord(value, label);
...
function expectedFacts(value: unknown, label: string): ExpectedSemanticFactV1[] {
...
function assertionChange(value: unknown, label: string): ExpectedFactAssertionChangeV1 {
  const record = expectRecord(value, label);
...
function expectation(value: unknown): SemanticMutationExpectationV1 {
...
function verificationRequirement(value: unknown, label: string): VerificationRequirementV1 {
  const record = expectRecord(value, label);
...
function verificationRequirements(value: unknown, label: string): VerificationRequirementV1[] {
...
export function normalizeSemanticMutationRequest(input: unknown): NormalizedSemanticMutationRequestV2 {
  ...
  const normalized: SemanticMutationRequestV2 = {
...
export function normalizeSemanticMutationAuthorization(
  ...
  const normalized: Omit<SemanticMutationAuthorizationContextV2, 'authorizationRevision'> = {
    ...
    allowedOperationKinds: allowedOperationKinds as SemanticMutationOperationV1['kind'][],
...
export function semanticMutationAuthorizationRevision(
  input: Omit<SemanticMutationAuthorizationContextV2, 'authorizationRevision'>
): string {
  return sha256({
    domain: 'semantic-mutation-authorization-v2',
    taskId: input.taskId ?? '',
...
export function normalizeVerificationRequirementsForTesting(
  input: unknown
): readonly VerificationRequirementV1[] {
  return cloneAndDeepFreeze(verificationRequirements(input, 'verification'));
}
