 ...
 */
export type RetainedCommandExecutableV1 = RetainedNoFollowOrdinaryFileV1;

export type RetainedCommandWorkingDirectoryV1 = RetainedNoFollowChildProcessDirectoryV1;

export interface RetainedCommandBoundaryV1 {
  readonly executable: RetainedCommandExecutableV1;
  readonly workingDirectory: RetainedCommandWorkingDirectoryV1;
}

export type RunRetainedCommandOptionsV1 = Omit<
  RunCommandOptions,
...
export class RetainedCommandTransportErrorV1 extends Error {
  constructor(
    message: string,
...
export const RETAINED_EXECUTABLE_CHILD_DESCRIPTOR_V1 = 3 as const;
export const RETAINED_WORKING_DIRECTORY_CHILD_DESCRIPTOR_V1 = 4 as const;
...
type RunCommandStdioV1 = Array<'ignore' | 'pipe' | number>;

function retainedCommandSpawnBoundaryV1(boundary: RetainedCommandBoundaryV1): Readonly<{
  cwd: string;
  stdio: RunCommandStdioV1;
}> {
  assertRetainedNoFollowCapabilityV1(
    boundary.executable,
    'executable',
  ...
  const stdio: RunCommandStdioV1 = ['ignore', 'pipe', 'pipe'];
  ...
  const parseDescriptor = (
    capability: RetainedCommandExecutableV1 | RetainedCommandWorkingDirectoryV1,
  ...
  if (executableSource === workingDirectorySource
      || executableSource === RETAINED_WORKING_DIRECTORY_CHILD_DESCRIPTOR_V1
      || workingDirectorySource === RETAINED_EXECUTABLE_CHILD_DESCRIPTOR_V1) {
    throw new Error(
      'Retained Linux executable and working-directory descriptor mappings must remain disjoint'
    );
  }
  while (stdio.length <= RETAINED_WORKING_DIRECTORY_CHILD_DESCRIPTOR_V1) stdio.push('ignore');
  stdio[RETAINED_EXECUTABLE_CHILD_DESCRIPTOR_V1] = executableSource;
  stdio[RETAINED_WORKING_DIRECTORY_CHILD_DESCRIPTOR_V1] = workingDirectorySource;
...
function assertRetainedCommandBoundaryCurrentV1(boundary: RetainedCommandBoundaryV1): void {
  assertRetainedNoFollowCapabilityV1(
    boundary.executable,
    'executable',
    'retained command executable'
...
function runCommandCapture(
  ...
  stdoutMode: 'bytes',
  retainedBoundary?: RetainedCommandBoundaryV1
...
async function runCommandCapture(
  ...
  options.signal?.throwIfAborted();
  const spawnBoundary = retainedBoundary === undefined
    ? Object.freeze({ cwd: options.cwd, stdio: ['ignore', 'pipe', 'pipe'] as RunCommandStdioV1 })
    : retainedCommandSpawnBoundaryV1(retainedBoundary);
  if (retainedBoundary !== undefined) assertRetainedCommandBoundaryCurrentV1(retainedBoundary);
...
export function runRetainedCommand(
  boundary: RetainedCommandBoundaryV1,
  args: string[],
  options: RunRetainedCommandOptionsV1
): Promise<CommandResult> {
...
function retainedCommandTransportErrorV1(
  command: string,
  outcome: ObservedCommandOutcome,
  cause?: Error
): RetainedCommandTransportErrorV1 {
  return new RetainedCommandTransportErrorV1(
    cause?.message ?? (
...
async function runRetainedCommandCaptureV1(
  ...
  const spawnBoundary = retainedCommandSpawnBoundaryV1(boundary);
  const command = boundary.executable.childPath;
  let expectedExecutableDigest: ReturnType<RetainedCommandExecutableV1['digest']> | undefined;
  const stdoutChunks: Buffer[] = [];
  ...
  const assertBoundary = (): void => {
    try {
      assertRetainedCommandBoundaryCurrentV1(boundary);
    } catch (error) {
  ...
  if (outcome.status !== 'exited' || !outcome.termination.treeClosed || failure !== undefined) {
    throw retainedCommandTransportErrorV1(command, outcome, failure);
  }
