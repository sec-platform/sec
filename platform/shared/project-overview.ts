...
const OPTIONAL_TOOL_REPORT_PATHS: Record<ToolEvidenceReportKind, string> = {
...
function buildQualityReport(kind: ToolEvidenceReportKind, reportsByKind: Map<ToolEvidenceReportKind, ToolEvidenceReport>): ProjectOverviewQualityReport {
  ...
  if (!report) {
    return {
      ...
      diagnosticCount: 0,
      affectedFileCount: 0,
      rawReportPaths: [OPTIONAL_TOOL_REPORT_PATHS[kind]]
    };
  }
...
export async function buildProjectOverviewFromWorkspace(workspaceRoot = process.cwd()): Promise<ProjectOverview> {
  ...
  const [
    ...
    readOptionalJson<ToolEvidenceReport>(path.join(paths.workspaceRoot, OPTIONAL_TOOL_REPORT_PATHS['code-quality'])),
    readOptionalJson<ToolEvidenceReport>(path.join(paths.workspaceRoot, OPTIONAL_TOOL_REPORT_PATHS['architecture-boundary'])),
    readOptionalJson<ToolEvidenceReport>(path.join(paths.workspaceRoot, OPTIONAL_TOOL_REPORT_PATHS['semantic-pattern']))
  ]);
