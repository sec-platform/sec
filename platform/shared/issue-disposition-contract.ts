...
export const ISSUE_DISPOSITION_PLAN_SCHEMA_V1 = 'sec-issue-disposition-plan-v1' as const;
export const ISSUE_DISPOSITION_SCHEMA_V1 = 'sec-issue-disposition-v1' as const;
export const ISSUE_DISPOSITION_POLICY_REVISION_V1 = 'issue-disposition-policy-v1' as const;

export type IssueDispositionDigestV1 = `sha256:${string}`;
export type IssueDispositionModeV1 = 'progress-only' | 'close-tracking-after-readback';

export interface GitHubIssueReferenceV1 {
  readonly repository: string;
  readonly issueNumber: number;
}

export interface GitHubPullRequestClosingFactsV1 {
  readonly repository: string;
  ...
  readonly closingIssues: readonly GitHubIssueReferenceV1[];
  readonly responseDigest: IssueDispositionDigestV1;
}

export interface GitHubClosingKeywordOccurrenceV1 extends GitHubIssueReferenceV1 {
  readonly keyword: 'close' | 'closes' | 'closed' | 'fix' | 'fixes' | 'fixed'
    | 'resolve' | 'resolves' | 'resolved';
  readonly offset: number;
}

export interface IssueDispositionPlanV1 {
  readonly schema: typeof ISSUE_DISPOSITION_PLAN_SCHEMA_V1;
  readonly policyRevision: typeof ISSUE_DISPOSITION_POLICY_REVISION_V1;
  readonly repository: string;
  readonly prNumber: number;
  readonly manifestPath: string;
  readonly manifestDigest: IssueDispositionDigestV1;
  readonly trackingIssueNumber: number | null;
  readonly mode: IssueDispositionModeV1;
  readonly titleBodyDigest: IssueDispositionDigestV1;
  readonly linkedClosingIssues: readonly GitHubIssueReferenceV1[];
  readonly planDigest: IssueDispositionDigestV1;
}

export type IssueDispositionKindV1 = 'progressed';

export interface IssueDispositionV1 {
  readonly schema: typeof ISSUE_DISPOSITION_SCHEMA_V1;
  readonly dispositionId: IssueDispositionDigestV1;
  readonly operationId: IssueDispositionDigestV1;
  readonly planDigest: IssueDispositionDigestV1;
  readonly planMode: IssueDispositionModeV1;
  readonly repository: string;
  readonly prNumber: number;
  readonly issueNumber: number | null;
  readonly manifestPath: string;
  readonly manifestDigest: IssueDispositionDigestV1;
  readonly currentSpecRevision: IssueDispositionDigestV1;
  readonly acceptanceIds: readonly IssueDispositionDigestV1[];
  readonly newMainSha: string;
  readonly newMainTreeSha: string;
  readonly evidenceRefs: readonly IssueDispositionDigestV1[];
  ...
  readonly kind: IssueDispositionKindV1;
  readonly blockers: readonly string[];
  readonly receiptDigest: IssueDispositionDigestV1;
}
...
function hash(value: unknown): IssueDispositionDigestV1 {
  return `sha256:${createHash('sha256').update(encodeVerificationActionDataV2(value)).digest('hex')}`;
}

function decodedSourceDigest(source: string): IssueDispositionDigestV1 {
...
function digest(value: unknown, label: string): IssueDispositionDigestV1 {
  if (typeof value !== 'string' || !/^sha256:[0-9a-f]{64}$/u.test(value)) {
    return fail(`${label} must be one SHA-256 digest.`);
  }
  return value as IssueDispositionDigestV1;
}
...
function normalizeReferences(
  value: readonly GitHubIssueReferenceV1[],
  label: string
): readonly GitHubIssueReferenceV1[] {
...
export function parseGitHubClosingKeywordOccurrencesV1(
  source: string,
  defaultRepository: string
): readonly GitHubClosingKeywordOccurrenceV1[] {
  ...
  const occurrences: GitHubClosingKeywordOccurrenceV1[] = [];
  for (const match of text.matchAll(CLOSING_KEYWORD_PATTERN)) {
    const keyword = match[1]!.toLowerCase() as GitHubClosingKeywordOccurrenceV1['keyword'];
...
export function parseGitHubPullRequestClosingFactsPageV1(input: Readonly<{
  ...
  facts: Omit<GitHubPullRequestClosingFactsV1, 'closingIssues' | 'responseDigest'>;
  closingIssues: readonly GitHubIssueReferenceV1[];
  totalCount: number;
  nextCursor: string | null;
  responseDigest: IssueDispositionDigestV1;
}> {
  try {
    let decoded: unknown;
    try {
...
export function parseIssueDispositionDirectiveV1(body: string): IssueDispositionModeV1 {
  ...
  return match[1] as IssueDispositionModeV1;
}

export function parseWorkPackageLocatorForDispositionV1(body: string): string {
  const lines = boundedText(body, 'pull request body').replaceAll('\r\n', '\n').split('\n');
...
export function renderPullRequestBodyV1(input: Readonly<{
  ...
  if (parseGitHubClosingKeywordOccurrencesV1(rendered, 'invalid/invalid').length > 0) {
    return fail('pull request renderer input contains a GitHub closing-keyword pattern.');
  }
  parseWorkPackageLocatorForDispositionV1(rendered);
  parseIssueDispositionDirectiveV1(rendered);
  return rendered;
}
...
export function createIssueDispositionPlanV1(input: Readonly<{
  repository: string;
  prNumber: number;
  manifestPath: string;
  ...
  title: string;
  body: string;
  linkedClosingIssues: readonly GitHubIssueReferenceV1[];
  ...
  const path = manifestPath(input.manifestPath);
  if (parseWorkPackageLocatorForDispositionV1(body) !== path) {
    return fail('pull request Work-Package locator differs from the selected manifest.');
  }
  const mode = parseIssueDispositionDirectiveV1(body);
  ...
  const lexical = [
    ...parseGitHubClosingKeywordOccurrencesV1(title, repositoryIdentity),
    ...parseGitHubClosingKeywordOccurrencesV1(body, repositoryIdentity)
  ];
  ...
  const withoutDigest = Object.freeze({ schema: ISSUE_DISPOSITION_PLAN_SCHEMA_V1,
    policyRevision: ISSUE_DISPOSITION_POLICY_REVISION_V1,
...
export function parseIssueDispositionPlanV1(source: string): IssueDispositionPlanV1 {
  ...
  exactKeys(value, ['schema', 'policyRevision', 'repository', 'prNumber', 'manifestPath',
    ...
    'planDigest'], 'plan');
  if (value.schema !== ISSUE_DISPOSITION_PLAN_SCHEMA_V1
    || value.policyRevision !== ISSUE_DISPOSITION_POLICY_REVISION_V1) return fail('plan schema or policy mismatch.');
  const rebuilt = createParsedPlanV1(value);
  if (rebuilt.planDigest !== value.planDigest) return fail('plan digest mismatch.');
  return rebuilt;
}

function createParsedPlanV1(value: Record<string, unknown>): IssueDispositionPlanV1 {
  ...
  const withoutDigest = Object.freeze({ schema: ISSUE_DISPOSITION_PLAN_SCHEMA_V1,
    ...
    linkedClosingIssues: normalizeReferences(
      value.linkedClosingIssues as readonly GitHubIssueReferenceV1[], 'plan linkedClosingIssues') });
  return Object.freeze({ ...withoutDigest, planDigest: hash(withoutDigest) });
}

export function createIssueAcceptanceIdV1(input: Readonly<{
  manifestDigest: IssueDispositionDigestV1;
  index: number;
  text: string;
}>): IssueDispositionDigestV1 {
...
function normalizeAcceptanceIds(
  value: readonly IssueDispositionDigestV1[]
): readonly IssueDispositionDigestV1[] {
...
function normalizeDigests(value: readonly IssueDispositionDigestV1[], label: string): readonly IssueDispositionDigestV1[] {
...
export function compileIssueDispositionV1(input: Readonly<{
  plan: IssueDispositionPlanV1;
  currentSpecRevision: IssueDispositionDigestV1;
  acceptanceIds: readonly IssueDispositionDigestV1[];
  newMainSha: string;
  newMainTreeSha: string;
  evidenceRefs: readonly IssueDispositionDigestV1[];
  expectedProviderState: 'OPEN' | 'CLOSED';
}>): IssueDispositionV1 {
  const plan = parseIssueDispositionPlanV1(encodeVerificationActionDataV2(input.plan));
  ...
  const semantic = Object.freeze({ schema: ISSUE_DISPOSITION_SCHEMA_V1,
...
export function parseIssueDispositionV1(source: string): IssueDispositionV1 {
  ...
  if (value.schema !== ISSUE_DISPOSITION_SCHEMA_V1) return fail('disposition schema mismatch.');
  if (value.planMode !== 'progress-only' && value.planMode !== 'close-tracking-after-readback') {
    return fail('disposition planMode is invalid.');
  }
  const acceptanceIds = normalizeAcceptanceIds(value.acceptanceIds as readonly IssueDispositionDigestV1[]);
  const evidenceRefs = normalizeDigests(value.evidenceRefs as readonly IssueDispositionDigestV1[], 'evidenceRefs');
  ...
  if (!Array.isArray(value.blockers)
    || encodeVerificationActionDataV2(value.blockers) !== encodeVerificationActionDataV2(semantic.blockers)
...
export function decideUnexpectedIssueReopenV1(input: Readonly<{
  repository: string;
  issueNumber: number;
