export const DOCUMENT_AUTHORITY_REGISTRY_SCHEMA =
  'sec-document-authority-registry-v1' as const;
...
function parseRecord(value: unknown, index: number): DocumentationAuthorityRecord {
  ...
  return {
    ...
    lifecycle: enumValue(value.lifecycle, DOCUMENT_AUTHORITY_LIFECYCLES, `${label}.lifecycle`),
...
function assertProjectionGraphAcyclic(records: readonly DocumentationAuthorityRecord[]): void {
  ...
  const visit = (id: string, chain: string[]): void => {
    ...
    if (visiting.has(id)) {
      throw new Error(`Documentation projection cycle: ${[...chain, id].join(' -> ')}.`);
    }
    visiting.add(id);
    const record = byId.get(id);
    if (!record) throw new Error(`Projection references unknown document ${id}.`);
...
export function parseDocumentationAuthorityRegistry(
  ...
  assertProjectionGraphAcyclic(documents);
  return {
