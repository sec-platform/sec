...
function decodeWindowsJobActiveProcessCount(information: Uint8Array): number {
...
/** Test-only native ABI decoder seam. */
export function decodeWindowsJobActiveProcessCountForTests(information: Uint8Array): number {
  return decodeWindowsJobActiveProcessCount(information);
}

function windowsWaitDisposition(result: number): 'signaled' | 'pending' | 'failed' {
  if (result === WINDOWS_WAIT_OBJECT_0) return 'signaled';
  if (result === WINDOWS_WAIT_TIMEOUT) return 'pending';
  return 'failed';
}

/** Test-only wait-result seam for fail-closed polling. */
export function windowsWaitDispositionForTests(result: number): 'signaled' | 'pending' | 'failed' {
...
/** Test-only native pipe error seam. */
export function windowsPipeFailureDispositionForTests(errorCode: number): 'eof' | 'failed' {
  return windowsPipeFailureDisposition(errorCode);
}
