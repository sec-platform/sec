...
import type { ProvenanceArtifact, ProvenanceFile } from './provenance-types.ts';
...
function optionalPortablePath(value: unknown, label: string): string | undefined {
  ...
  if (text !== undefined && !isCanonicalPortableLogicalPathV1(text)) {
...
function validateArtifact(value: unknown, index: number): ProvenanceArtifact {
  ...
  if (typeof raw.path !== 'string' || !isCanonicalPortableLogicalPathV1(raw.path)) {
  ...
  if (raw.hash !== undefined && (typeof raw.hash !== 'string' || !/^[0-9a-f]{64}$/u.test(raw.hash))) {
...
export function validateProvenanceFileV1(value: unknown): ProvenanceFile {
  ...
  return deepFreeze({ formatVersion: '1', artifacts });
}
 ...
 */
export function readOptionalProvenanceFileV1(
  filePath: string,
  label = 'Provenance'
): ProvenanceFile | null {
  const raw = readOptionalRetainedJsonV1<unknown>(filePath, label);
  return raw === null ? null : validateProvenanceFileV1(raw);
}
