...
import { canonicalEquals, deepFreeze, uniqueSorted } from './canonical-primitives.ts';
import { CompilerError } from './errors.ts';
import { writeJson, type CommitFence } from './fs.ts';
...
function requireCanonicalBaselineArtifactPath(artifactPath: string): string {
  if (!isCanonicalPortableLogicalPathV1(artifactPath)) {
...
function validateProjectBaseline(value: unknown): ProjectBaselineFile {
  ...
  for (const rawArtifact of baseline.artifacts) {
    ...
    if (typeof artifact.path !== 'string' || !isCanonicalPortableLogicalPathV1(artifact.path)) {
...
export function readProjectBaseline(workspaceRoot: string): ProjectBaselineFile | null {
  let raw: unknown;
  try {
    raw = readOptionalRetainedJsonV1<unknown>(
