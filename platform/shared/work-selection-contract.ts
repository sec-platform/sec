...
export type SecWorkDecisionStatus =
  | 'continue-active'
  | 'closeout'
  | 'reconcile'
  | 'select-next'
...
export type SecWorkCandidateDecisionStatus =
  ...
  | 'unresolved'
  | 'human-required';
export type SecWorkDigestV1 = `sha256:${string}`;

export interface SecWorkSelectionIdentityV1 {
  readonly exactMain: string;
  readonly roadmapRevision: SecWorkDigestV1;
  readonly candidateSetRevision: SecWorkDigestV1;
  readonly selectionPolicyRevision: typeof SEC_WORK_SELECTION_POLICY_REVISION;
}

export interface SecCurrentWorkLifecycleV1 {
...
export interface SecWorkDependencyFactV1 {
  readonly ref: string;
  readonly status: 'satisfied' | 'unsatisfied' | 'unresolved';
}

export interface SecWorkCandidateV1 {
  ...
  readonly prerequisiteFacts: readonly SecWorkDependencyFactV1[];
  readonly orderedAfterFacts: readonly SecWorkDependencyFactV1[];
...
export interface SecWorkSelectionInputV1 {
  readonly schema: typeof SEC_WORK_SELECTION_INPUT_SCHEMA;
  readonly identity: SecWorkSelectionIdentityV1;
  readonly current: SecCurrentWorkLifecycleV1;
  readonly candidates: readonly SecWorkCandidateV1[];
}

export interface SecWorkCandidateDecisionV1 {
  readonly workId: string;
  readonly candidateRef: string;
  readonly currentSpecRef: string;
  readonly currentSpecRevision: SecWorkDigestV1;
...
export interface SecWorkSelectionPreconditionV1 {
  readonly workId: string;
  readonly currentSpecRef: string | null;
  readonly currentSpecRevision: SecWorkDigestV1 | null;
  readonly reasonCode: string;
  readonly blockerRef: string;
}

export interface SecWorkCurrentSpecBindingV1 {
  readonly workId: string;
  readonly currentSpecRef: string;
  readonly currentSpecRevision: SecWorkDigestV1;
}

export interface SecWorkDecisionV1 {
  readonly schema: typeof SEC_WORK_DECISION_SCHEMA;
  readonly policyRevision: typeof SEC_WORK_SELECTION_POLICY_REVISION;
  readonly inputDigest: SecWorkDigestV1;
  ...
  readonly selectedCurrentSpecRevision: SecWorkDigestV1 | null;
  readonly currentSpecBindings: readonly SecWorkCurrentSpecBindingV1[];
  readonly blockedCandidateRefs: readonly string[];
  readonly reasonCodes: readonly string[];
  readonly requiredPreconditions: readonly SecWorkSelectionPreconditionV1[];
  readonly rejectionWitnesses: readonly SecWorkCandidateDecisionV1[];
...
const FRESHNESS_ORDER = new Map<SecWorkCandidateV1['reproductionOrEvidenceFreshness'], number>([
  ['fresh', 0],
  ['stale', 1],
  ['missing', 2]
]);
...
function digest(value: unknown, label: string): SecWorkDigestV1 {
  ...
  return parsed as SecWorkDigestV1;
}
...
function parseDependencyFacts(value: unknown, label: string): SecWorkDependencyFactV1[] {
...
function parseCandidate(value: unknown, index: number): SecWorkCandidateV1 {
  ...
  const parsed: SecWorkCandidateV1 = {
...
function normalizeCandidates(value: unknown): SecWorkCandidateV1[] {
...
export function computeSecWorkCandidateSetRevisionV1(
  candidates: readonly SecWorkCandidateV1[]
): SecWorkDigestV1 {
  return sha256(normalizeCandidates(candidates)) as SecWorkDigestV1;
}

export function parseSecWorkSelectionInputV1(value: unknown): SecWorkSelectionInputV1 {
...
function compareCandidateRank(left: SecWorkCandidateV1, right: SecWorkCandidateV1): number {
...
): SecWorkSelectionPreconditionV1[] {
  const projected: SecWorkSelectionPreconditionV1[] = [];
  const precondition = (
...
function currentPreconditions(input: SecWorkSelectionInputV1): SecWorkSelectionPreconditionV1[] {
  const projected: SecWorkSelectionPreconditionV1[] = [];
...
function finalizeDecision(
  ...
  const withoutDigest = {
    ...
    inputDigest: sha256(input) as SecWorkDigestV1,
  ...
  return deepFreeze({
    ...withoutDigest,
    decisionDigest: sha256(withoutDigest) as SecWorkDigestV1
  });
}

export function compileSecWorkDecisionV1(value: unknown): SecWorkDecisionV1 {
  const input = parseSecWorkSelectionInputV1(value);
  const emptyWitnesses: readonly SecWorkCandidateDecisionV1[] = Object.freeze([]);
  const emptyPreconditions: readonly SecWorkSelectionPreconditionV1[] = Object.freeze([]);
  if (
...
export function assertSecWorkDecisionV1(
  value: unknown,
  input: unknown
): SecWorkDecisionV1 {
  const compiled = compileSecWorkDecisionV1(input);
