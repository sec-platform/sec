    ...
    return withAuthorityGitReadSessionV1({
      ...
      const run = async (args: readonly string[]): Promise<ByteCommandResult> => {
        ...
        if (command.kind !== 'completed') {
          throw new GitReadAuthorityErrorV1('Reference Git observation failed.', command);
        }
      ...
      if (recordFailure !== null) {
        throw new GitReadAuthorityErrorV1('Reference Git record budget failed.', recordFailure);
      }
      return {
