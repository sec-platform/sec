...
import type { CiVerificationGatePhase, CiVerificationGateStep } from '../../ci/index.ts';
import {
...
import { VERIFICATION_GATE_RESULT_SCHEMA_V1 } from '../../result/contract/schema.ts';
...
export interface CiVerificationActionPlanClosureV1 {
  readonly schema: typeof CI_VERIFICATION_ACTION_PLAN_CLOSURE_SCHEMA_V2;
  readonly producerRevision: typeof CI_VERIFICATION_ACTION_PRODUCER_REVISION_V2;
...
export function buildCiVerificationActionPlanV1(options: {
  ...
  const action = createVerificationActionKeyV2({
    ...
    producer: {
      ...
      revision: CI_VERIFICATION_ACTION_PRODUCER_REVISION_V2
    },
    operation: {
    ...
    resultSchemaRevision: VERIFICATION_GATE_RESULT_SCHEMA_V1
  });
...
export function buildCiVerificationActionPlanClosureV1(options: {
  ...
  const withoutDigest = Object.freeze({
    schema: CI_VERIFICATION_ACTION_PLAN_CLOSURE_SCHEMA_V2,
    producerRevision: CI_VERIFICATION_ACTION_PRODUCER_REVISION_V2,
...
export function parseCiVerificationActionPlanClosureV1(source: string): CiVerificationActionPlanClosureV1 {
  ...
  if (value.schema !== CI_VERIFICATION_ACTION_PLAN_CLOSURE_SCHEMA_V2 ||
      value.producerRevision !== CI_VERIFICATION_ACTION_PRODUCER_REVISION_V2 ||
...
export function resolveCiVerificationDevRunnerTargetV1(options: {
  ...
  if (plan.action.producer.identity !== 'platform/shared/verification-action-ci-contract.ts' ||
      plan.action.producer.revision !== CI_VERIFICATION_ACTION_PRODUCER_REVISION_V2 ||
      plan.action.operation.revision !== CI_VERIFICATION_ACTION_PRODUCER_REVISION_V2) {
