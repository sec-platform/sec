...
import { encodeVerificationActionDataV2 } from '../../action/index.ts';
...
export interface ReviewStabilityPolicyV1 {
  readonly schema: typeof REVIEW_STABILITY_POLICY_SCHEMA_V1;
  readonly policyId: string;
  readonly trustedRevision: string;
  readonly trustedApps: readonly ReviewStabilityTrustedAppV1[];
  readonly allowIndependentHumanApproval: boolean;
  readonly policyDigest: ReviewStabilityDigest;
}

export type ReviewStabilityPolicyInputV1 = Omit<ReviewStabilityPolicyV1, 'schema' | 'policyDigest'>;

export type ReviewPrincipalV1 = Readonly<{
...
export interface ReviewIndependenceV1 {
...
export interface ReviewStabilityReceiptV1 {
  readonly schema: typeof REVIEW_STABILITY_RECEIPT_SCHEMA_V1;
  readonly stage: ReviewStabilityStageV1;
  ...
  readonly policy: ReviewStabilityPolicyV1;
  readonly principal: ReviewPrincipalV1;
  readonly independence: ReviewIndependenceV1;
  readonly producer: ReviewStabilityProducerV1;
  readonly snapshot: ReviewSnapshotV1;
...
export type ReviewStabilityReceiptInputV1 = Omit<ReviewStabilityReceiptV1, 'schema' | 'reviewRevision' | 'receiptDigest'>;
...
export function createReviewSnapshotDigestV1(
  snapshot: Omit<ReviewSnapshotV1, 'snapshotDigest'>
...
export function createReviewStabilityPolicyV1(input: ReviewStabilityPolicyInputV1): ReviewStabilityPolicyV1 {
  ...
  const withoutDigest = Object.freeze({
    schema: REVIEW_STABILITY_POLICY_SCHEMA_V1,
...
export function parseReviewStabilityPolicyV1(source: string): ReviewStabilityPolicyV1 {
  ...
  if (parsed.schema !== REVIEW_STABILITY_POLICY_SCHEMA_V1) fail('policy schema mismatch.');
  const policy = createReviewStabilityPolicyV1(parsed as unknown as ReviewStabilityPolicyInputV1);
  if (policy.policyDigest !== parsed.policyDigest) fail('policy digest mismatch.');
  return policy;
}

export const SEC_REVIEW_STABILITY_POLICY_V1 = createReviewStabilityPolicyV1({
...
export function createReviewStabilityReceiptV1(input: ReviewStabilityReceiptInputV1): ReviewStabilityReceiptV1 {
  ...
  let principal: ReviewPrincipalV1;
  ...
  const computedSnapshotDigest = createReviewSnapshotDigestV1(snapshotWithoutDigest);
  ...
  const normalized = Object.freeze({ schema: REVIEW_STABILITY_RECEIPT_SCHEMA_V1, stage: input.stage, repository: text(input.repository, 'repository'), prNumber: Number.isSafeInteger(input.prNumber) && input.prNumber > 0 ? input.prNumber : fail('prNumber must be positive.'), sessionRevision: digest(input.sessionRevision, 'sessionRevision'), scopeAuthorizationRevision: digest(input.scopeAuthorizationRevision, 'scopeAuthorizationRevision'), scopeAuthorizationReceiptDigest: digest(input.scopeAuthorizationReceiptDigest, 'scopeAuthorizationReceiptDigest'), headSha, headTreeSha: sha(input.headTreeSha, 'headTreeSha'), policy, principal, independence, producer, snapshot, reviewedAt: instant(input.reviewedAt, 'reviewedAt'), expiresAt: instant(input.expiresAt, 'expiresAt') });
...
export function parseReviewStabilityReceiptV1(source: string): ReviewStabilityReceiptV1 {
  ...
  if (parsed.schema !== REVIEW_STABILITY_RECEIPT_SCHEMA_V1) fail('schema mismatch.');
  const receipt = createReviewStabilityReceiptV1(parsed as unknown as ReviewStabilityReceiptInputV1);
...
export function assertReviewStabilityReceiptCurrentV1(receipt: ReviewStabilityReceiptV1, live: { readonly stage: ReviewStabilityStageV1; readonly sessionRevision: ReviewStabilityDigest; readonly scopeAuthorizationRevision: ReviewStabilityDigest; readonly scopeAuthorizationReceiptDigest: ReviewStabilityDigest; readonly headSha: string; readonly headTreeSha: string; readonly expectedPolicyDigest: ReviewStabilityDigest; readonly snapshotDigest: ReviewStabilityDigest; readonly expectedReviewRevision: ReviewStabilityDigest; readonly now: string }): void {
