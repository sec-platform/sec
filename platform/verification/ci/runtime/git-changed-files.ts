...
import { CodexDevelopmentIsCanonicalRepositoryPathV1 } from '../../../foundation/paths.ts';
...
export type CodexDevelopmentGitPathBlobObservationV1 = Readonly<{
  path: string;
...
export type CodexDevelopmentGitPathBlobEntryV1 = Readonly<{
  mode: '100644' | '100755';
  blobSha: string;
}>;

export type CodexDevelopmentTestImpactTransitionObservationV1 = Readonly<{
  schema: 'sec-test-impact-transition-observation-v1';
  baseSha: string;
  headSha: string;
  records: readonly CodexDevelopmentGitChangedRecordV1[];
  removedPathBlobs: readonly CodexDevelopmentGitPathBlobObservationV1[];
}>;
...
export function CodexDevelopmentTestImpactTransitionDigestV1(
  ...
  const observedPaths = observation.removedPathBlobs.map((entry) => {
    if (entry === null || typeof entry !== 'object' || Array.isArray(entry)
        ...
        || !CodexDevelopmentIsCanonicalRepositoryPathV1(entry.path)
...
export function CodexDevelopmentAssertTestImpactTransitionSelectionV1(input: {
  baseSha: string;
  headSha: string;
  ...
  const digest = CodexDevelopmentTestImpactTransitionDigestV1(input.observation);
...
): CodexDevelopmentGitPathBlobEntryV1 | null {
...
export function parseGitPathBlobBatchOutput(
  stdout: Uint8Array,
  repositoryPaths: readonly string[]
): Map<string, CodexDevelopmentGitPathBlobEntryV1> {
  ...
  const result = new Map<string, CodexDevelopmentGitPathBlobEntryV1>();
...
export function CodexDevelopmentCreateTestImpactTransitionObservationV1(input: {
  baseSha: string;
  headSha: string;
  records: readonly CodexDevelopmentGitChangedRecordV1[];
  readPathBlob: (revision: string, repositoryPath: string) => CodexDevelopmentGitPathBlobEntryV1 | null;
  ...
  CodexDevelopmentTestImpactTransitionDigestV1(observation);
  return observation;
}
