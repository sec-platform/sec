...
export const VERIFICATION_PROVIDER_CAPABILITY_SCHEMA_V1 =
  'sec-verification-provider-capability-v1' as const;
export const VERIFICATION_PROVIDER_AVAILABILITY_EPOCH_SCHEMA_V1 =
  'sec-verification-provider-availability-epoch-v1' as const;

export type VerificationProviderRoleV1 = 'writer' | 'reviewer' | 'hosted-verification';
export type VerificationProviderAvailabilityV1 = 'available' | 'unavailable' | 'degraded' | 'unknown';
export type VerificationProviderCapabilityIdV1 =
  | 'github-writer'
  | 'codex-review'
...
export interface VerificationProviderCapabilityV1 {
  readonly schema: typeof VERIFICATION_PROVIDER_CAPABILITY_SCHEMA_V1;
  readonly capability: VerificationProviderCapabilityIdV1;
  readonly role: VerificationProviderRoleV1;
  readonly provider: VerificationProviderIdV1;
  readonly availability: VerificationProviderAvailabilityV1;
...
export type VerificationProviderCapabilityInputV1 = Omit<
  VerificationProviderCapabilityV1,
  'schema'
>;

export interface VerificationProviderAvailabilityEpochV1 {
  readonly schema: typeof VERIFICATION_PROVIDER_AVAILABILITY_EPOCH_SCHEMA_V1;
  readonly epochId: string;
  readonly observedAt: string;
  readonly expiresAt: string;
  readonly capabilities: readonly VerificationProviderCapabilityV1[];
...
export function createVerificationProviderCapabilityV1(
  ...
  const provider = text(input.provider, 'provider') as VerificationProviderIdV1;
...
export function createVerificationProviderAvailabilityEpochV1(input: {
  epochId: string;
  observedAt: string;
  expiresAt: string;
  capabilities: readonly VerificationProviderCapabilityInputV1[];
  ...
  const withoutDigest = Object.freeze({
    schema: VERIFICATION_PROVIDER_AVAILABILITY_EPOCH_SCHEMA_V1,
    epochId,
    observedAt,
    expiresAt,
    capabilities
...
export function assertProviderCapabilityUsableV1(input: {
  epoch: VerificationProviderAvailabilityEpochV1;
  capability: VerificationProviderCapabilityIdV1;
  expectedRole: VerificationProviderRoleV1;
  now: string;
}): VerificationProviderCapabilityV1 {
  ...
  const capability = resolveProviderAvailabilityV1(input.epoch, input.capability);
...
export function resolveProviderAvailabilityV1(
  epoch: VerificationProviderAvailabilityEpochV1,
  capability: VerificationProviderCapabilityIdV1
): VerificationProviderCapabilityV1 {
  if (epoch.schema !== VERIFICATION_PROVIDER_AVAILABILITY_EPOCH_SCHEMA_V1) {
 ...
 */
export function assertProviderRetryGuardV1(input: {
  previous: { availability: VerificationProviderAvailabilityV1; epochId: string };
  requested: { capability: VerificationProviderCapabilityIdV1; epochId: string };
}): void {
