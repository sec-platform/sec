...
import {
  GitReadAuthorityErrorV1,
  withAuthorityGitReadSessionV1
} from '../../external-capabilities/git-read/index.ts';
...
export async function listTrackedProjectPaths(workspaceRoot: string): Promise<Set<string> | null> {
  return withAuthorityGitReadSessionV1({
      cwd: workspaceRoot,
      ...
      if (command.kind !== 'completed') {
        throw new GitReadAuthorityErrorV1('Unable to observe tracked project paths.', command);
      }
      ...
      if (recordFailure !== null) {
        throw new GitReadAuthorityErrorV1('Tracked project path inventory exceeded its authority budget.', recordFailure);
      }
