...
export const MAIN_AUTHORITY_RULESET_RECEIPT_SCHEMA_V1 =
  'sec-main-authority-ruleset-receipt-v1' as const;

export type MainAuthorityRulesetDigestV1 = `sha256:${string}`;

export interface MainAuthorityRulesetReceiptV1 {
  readonly schema: typeof MAIN_AUTHORITY_RULESET_RECEIPT_SCHEMA_V1;
  readonly status: 'enforced';
  readonly repository: string;
  readonly defaultBranch: string;
  readonly terminalStatusContext: typeof SEC_INTEGRATION_AUTHORIZATION_STATUS_CONTEXT_V1;
  ...
  readonly rulesetDigest: MainAuthorityRulesetDigestV1;
}
...
function digest(value: unknown): MainAuthorityRulesetDigestV1 {
...
function requiredStatusRuleIsCanonical(
  ...
  for (const [index, entry] of checks.entries()) {
    ...
    if (check.context === SEC_INTEGRATION_AUTHORIZATION_STATUS_CONTEXT_V1) {
...
export function createMainAuthorityRulesetReceiptV1(input: Readonly<{
  repository: string;
  ...
  const expectedIntegrationId = input.expectedIntegrationId === undefined
    ? CI_GITHUB_ACTIONS_IDENTITY_POLICY_V1.app.id
  ...
  const semantic = Object.freeze({
    schema: MAIN_AUTHORITY_RULESET_RECEIPT_SCHEMA_V1,
    repository,
    defaultBranch,
    terminalStatusContext: SEC_INTEGRATION_AUTHORIZATION_STATUS_CONTEXT_V1,
