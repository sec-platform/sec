...
export const INTEGRATION_AUTHORIZATION_SCHEMA_V1 = 'sec-integration-authorization-v1' as const;
export type IntegrationAuthorizationDigest = `sha256:${string}`;

export interface IntegrationAuthorizationIssuerV1 {
...
export interface IntegrationAuthorizationV1 {
  readonly schema: typeof INTEGRATION_AUTHORIZATION_SCHEMA_V1;
  ...
  readonly issuer: IntegrationAuthorizationIssuerV1;
  readonly receiptDigest: IntegrationAuthorizationDigest;
}

export type IntegrationAuthorizationInputV1 = Omit<IntegrationAuthorizationV1, 'schema' | 'authorizationId' | 'receiptDigest'>;
...
export function createIntegrationAuthorizationV1(input: IntegrationAuthorizationInputV1): IntegrationAuthorizationV1 {
  ...
  const withoutDigest = Object.freeze({
    schema: INTEGRATION_AUTHORIZATION_SCHEMA_V1,
...
export function parseIntegrationAuthorizationV1(source: string): IntegrationAuthorizationV1 {
  const parsed = record(JSON.parse(source) as unknown);
  exact(parsed);
  if (parsed.schema !== INTEGRATION_AUTHORIZATION_SCHEMA_V1) fail('schema mismatch.');
  const authorization = createIntegrationAuthorizationV1(parsed as unknown as IntegrationAuthorizationInputV1);
...
export interface IntegrationAuthorizationLiveStateV1 {
  readonly now: string;
...
export function assertIntegrationAuthorizationUsableV1(
  authorization: IntegrationAuthorizationV1,
  live: IntegrationAuthorizationLiveStateV1
): void {
  const current = parseIntegrationAuthorizationV1(encodeVerificationActionDataV2(authorization));
