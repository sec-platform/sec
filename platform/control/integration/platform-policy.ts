...
export type IntegrationPlatformDigestV1 = `sha256:${string}`;
...
export const SEC_INTEGRATION_PLATFORM_POLICY_DIGEST_V1 =
  `sha256:${createHash('sha256').update(
    encodeVerificationActionDataV2(SEC_INTEGRATION_PLATFORM_POLICY_V1)
  ).digest('hex')}` as const;

export interface IntegrationPlatformObservationV1 {
  ...
  readonly rulesetDigest: IntegrationPlatformDigestV1;
...
function digest(value: unknown): IntegrationPlatformDigestV1 {
  if (typeof value !== 'string' || !/^sha256:[0-9a-f]{64}$/u.test(value)) {
    fail('observation digest must be a SHA-256 digest.');
  }
  return value as IntegrationPlatformDigestV1;
}
 ...
 */
export function canonicalizeIntegrationPlatformObservationV1(
  value: IntegrationPlatformObservationV1
): IntegrationPlatformObservationV1 {
  ...
  if (!SEC_INTEGRATION_PLATFORM_POLICY_V1.allowPlatformEnforcementUnavailable
      || SEC_INTEGRATION_PLATFORM_POLICY_V1.claimsNoBypassEnforcement) {
