...
61-module runtime closure，运行时 closure 也不包含它。目录位置因此正在放大
...
- 当前包自身修改 trust classifier，因此必须由旧 trusted revision 执行
  candidate-as-SUT/manual bootstrap；禁止使用新 classifier 自证。
