...
`hooks:install` 读取 common 与 worktree-effective `core.hooksPath`；若未配置，还会审计 Git 默认 hooks 目录中的非 sample hook。配置前必须确认当前 branch 的全部 `.githooks` 同时是 index mode `100755` 的 tracked entry、实际文件，且工作树 raw blob 与 index 完全一致，避免 branch-without-hook、non-executable hook 或未提交 hook 被物化成 authority。Installer 对 hook 名称与原始字节建立 generation digest，先在 Git common dir 的 `sec-managed-hooks-v1/` 中写入并验证不可变 generation，再原子切换 repository-common `core.hooksPath`；因此 `git worktree add` 即使由缺少当前 `.githooks` 的旧 checkout 发起，也能在新 checkout 首次完成前运行同一 tracked hook 内容。已存在的其他真实 hook authority 使显式安装 fail closed，package lifecycle 只提示人工集成并保留原 authority；CI 或 Gitless lifecycle 直接 no-op，显式命令仍 fail closed；legacy `.githooks`、旧 managed generation 与缺失的陈旧路径可迁移，真实 custom authority 不被覆盖。Common generation 只是 tracked hook 的部署快照，`deps:ensure` 仍是唯一 dependency builder；不存在第二套 bootstrap、选择器或安装算法。
