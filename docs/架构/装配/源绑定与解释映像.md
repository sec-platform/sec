...
**E5 使用与替换。** 每个请求pin选定image。升级构建另一个image，在新请求接纳点切换默认引用，旧请求不被热替换。真实安全撤销可以阻断旧方法的新作用；不能借升级改写旧任务的类型或恢复意图。

**E6 释放。** 没有请求、缓存或恢复责任还引用旧实例时才卸载。进程/连接必须实际停止或转移；仅image引用计数为零不证明外部设备已结束。失联资源转入对应未结算责任，不阻塞不相交纯结果释放。
