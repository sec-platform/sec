...
- 现有模板、SDK、脚手架、低代码、AI 编码助手和行业 SaaS 已经覆盖大量单点能力，但中大型项目仍反复消耗在跨模块拼接、业务编排、胶水逻辑、验收维护、升级迁移和复用失效上。
- 本产品要解决的不是“缺少更多代码生成”，而是缺少一个能把隐性专家劳动结构化为规格、块接口、连接关系、AI 可填 slot、验收和来源追踪的工程编译层。
- 目标转化链路是：`隐性专家劳动 -> 结构化规格 -> 标准模块 -> 管脚连接 -> AI 填 slot -> 验收验证 -> 可持续维护`。

### 推荐命名语义

- `Engineering Compiler`：强调工程规格到可运行软件系统的编译过程，是当前文档的主名称。
- `SpecEngineer`：可作为产品人格或开发者工作台名称，强调用户在写规格、调图、定义 slot 和验收，而不是直接写满仓代码。
- `Project Harness Engineering` / `Spec Harness Compiler`：强调本产品本质是给 AI 和人类共同使用的工程约束系统；AI 被 harness 收束，而不是自由游走。
- `Spec-Driven AI Engineering`：适合作为方法论表述，说明这不是 vibe coding，而是受控工程综合。

### 与相邻物种的边界

- 相比模板/脚手架：模板通常只解决初始生成，本产品必须覆盖组合、验证、来源追踪、升级、override 和重复编译。
- 相比低代码：低代码常把用户锁在运行时和 UI 编排器中，本产品输出真实源码，并保留源码视图和下钻能力。
- 相比自由 AI 编程助手：AI 编程助手直接面对整仓上下文，本产品要求 AI 先面对 `plan / graph / pin / slot / acceptance / provenance`，再在必要时下钻源码。
...
- 标准业务后台、B2B SaaS、管理台、工单/工作追踪、CRM/ERP 子域、内部工具、运营后台、数据录入与审批系统。
- 这类项目的共同点是：结构重复度高、模块边界清晰、验收场景可描述、运行性能通常不是核心瓶颈、业务差异集中在 policy/adapter/UX slot。

### 可以中期扩展

- 面向客户的产品型 Web/App：适合用工程编译器管理账户、权限、数据对象、后台、配置和部分标准页面，但核心增长体验、品牌交互和实验性 UI 需要保留人工/UX slot 空间。
- 平台型系统：适合管理控制面、租户、权限、计费、审计、配置和集成，不应直接生成复杂数据面内核。
- AI/Agent 产品应用层：适合编译工具权限、任务流、审计、知识入口和 UI，不应把模型推理本身视为普通业务块。

### 必须谨慎切入

- 高性能核心系统、数据库、编译器后端、实时渲染、游戏主循环、网络协议栈、底层基础设施等热路径系统。
- 这类系统可采用 `Outer Shell Mode` 或 `Hybrid Kernel Mode`：工程编译器生成控制面、配置面、测试 harness、benchmark harness、适配层和观测面；核心 kernel 由专家维护，只通过严格接口暴露。

### 冷热分层原则

- 冷路径：后台 CRUD、配置、审批、报表、管理台，优先纳入标准 block。
- 温路径：业务规则、策略判断、跨系统 adapter、局部 UX，优先设计为 slot。
- 热路径：性能关键算法、实时循环、强一致内核、底层运行时，优先设计为 `Kernel Block` 或外部系统接口。
...
- 当前实现母例固定为“登录后管理 Customer 的最小后台”。
- 这是为了复用现有实现、文档和验收资产，先证明最小闭环，不在 `v0.1` 中切换方向。
- 标准产品母例建议在 `v0.2+` 演进为“多租户 B2B 工作追踪 / 工单 SaaS”：`auth/basic-session + tenant/basic-workspace + ticket/basic`，因为它比 Customer CRUD 更能覆盖工作流、状态流转、权限、通知、审计、搜索、导出和升级场景。
- 母例演进规则：`v0.1` 不重命名、不迁移当前 customer 母例；`v0.2` 可新增 ticket/work-tracking 块；`v1` 再将其作为对外 demo 和标准行业包。
...
- 视图优先，源码可达：默认在规格、图谱、slot、验收和 provenance 层工作，但任意产物必须能下钻到真实源码。
- 任务优先，聊天辅助：用户主要操作对象是任务、slot、规则、升级计划、失败项和验收报告，聊天只作为解释和辅助输入。
- 规格为源，源码为产物：源码可人工修改，但必须进入 override / provenance / 回写流程，不能静默取代上游规格。
- 默认在图谱层工作，必要时下钻到代码：AI 也必须先读结构化工程对象，再按任务信封读取有限源码上下文。
...
## 新增整合：模型能力、算力与 Benchmark 边界

- Frontier model 能显著提高 slot 综合、repair、解释和迁移建议质量，但不能替代 compile contract、权限边界、验收和 provenance。
- 模型能力越强，越需要 harness：强模型在开放仓库里可能扩大修改面，在结构化任务信封里才能稳定转化为工程产出。
- 端侧或小模型适合承担检索、分类、摘要、简单修复建议、报告生成和本地低风险辅助；复杂对齐、跨模块 repair、升级迁移规划仍应使用更强模型。
- 衡量平台不能只看通用代码 benchmark，而应看项目级 benchmark：从 plan 到可运行项目的一次成功率、验收通过率、slot diff 面积、repair 收敛次数、升级安全率、provenance 覆盖率和人工 review 成本。
- 模型进步会降低 slot surface 的成本，但不会消除 block graph、pin、slot、acceptance、policy、provenance 和 upgrade 的工程必要性。
...
- 当前产品只能承诺第一层，并为第二层预留 block version、registry、upgrade、override、provenance、policy 和 graph；第三层之后只作为研究方向和产品远景，不进入短期实现承诺。

## 新增整合：学习成本与采用策略

- 早期用户不应被要求一次性理解完整“工程语言”，而应从 plan、block、slot、acceptance 四个最小对象开始。
- 产品默认入口应是可运行母例、可视 graph、slot 任务、验收报告和源码下钻，而不是要求用户先学习一套抽象理论。
- 对资深工程师，价值主张是减少跨模块胶水、验收维护、升级和 review 面积；对业务/产品角色，价值主张是把产品意图转成可验证工程规格。
- 最早的 adoption 路线应服务小团队和重复后台项目，而不是直接挑战底层系统、超大型单体或高度差异化 C 端体验。
- 如果平台成立，新的角色会从“只写代码”转向“规格工程师、块库维护者、验收设计者、平台治理者、AI harness 设计者”。
