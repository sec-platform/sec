...
`bun run sec:closeout -- --pr <number>` 直接消费同一 VerificationSession、ActionKey、Review、MainHealth、MergeGate 与 terminal status contracts，不经过 GitHub Actions dispatch、artifact upload 或 self-hosted runner registration。它从 clean exact live `main` 启动固定 OCI image，并始终把candidate当作不受信data交给base-owned verifier。ordinary profile在执行前物理断开outer container全部network；bootstrap profile只允许exact-base coordinator/materializer使用outer network完成唯一一次依赖树物化，candidate代码只能进入由`CI_VERIFICATION_HOSTED_SANDBOX_POLICY_V1`拥有、强制`network=none`并执行真实route/egress readback的inner SUT sandbox。不得把outer coordinator网络误投影成candidate权限，也不得为了outer断网重复生成临时`node_modules`或让candidate verifier自证。旧 trust root 仅可用自身已冻结archive协议完成一次替代者bootstrap；new-main必须把exact-base依赖树投影为content-addressed Docker local volume并在private mount namespace内retained read-only bind，随后以consumer-zero删除dependency tar copy/extract，禁止继续优化或复制legacy transport。Docker capability必须先由唯一 endpoint owner解析为本机 `npipe://` 或 `unix://` transport，并把 context、endpoint、daemon、OS与architecture写入Action/receipt/cache identity；`tcp://` remote endpoint、context漂移或daemon漂移一律不能伪装成local trusted runtime。同一ActionKey的physical start先持有repository-scoped operation mutation lease；每次container attempt再使用collision-free retained identity并绑定operation/repository/base/head/endpoint/image/host/pid/nonce labels。只有两次Docker identity读回一致且同宿主owner进程已死亡时才能回收；live、foreign、unknown或malformed资源全部保留并阻断同一ActionKey，不能被唯一名称绕过形成第二次physical start。Linux dependency cache只能使用由image ID与exact base dependency identity内容寻址、由endpoint/daemon observation绑定且带canonical labels的Docker `local` named volume；Windows host bind path、mutable locator和跨文件系统POSIX语义不得进入该cache。cache marker明确`authority=none`，只影响性能；ordinary closeout不暴露volume delete/prune，active container mount与operation lease共同构成使用fence，retirement必须另行消费superseded decision、证明exact daemon零引用并按精确volume observation执行/readback。dependency download、dependency tree materialization、candidate archive、每个scenario result与exact-head finalization分别拥有ActionKey；只有自身inputs、implementation、environment或provider revision变化的节点进入`MissingOrStale`，未变化的PASS、known failure与authenticated in-flight必须复用，禁止用candidate head变化机械失效全部下游。closeout的所有host侧Git命令与bundle producer共用canonical isolated Git child environment，不继承global/system config、template、prompt、askpass/SSH、replace-object或optional-lock authority。所有可复用Session/Action/Gate/status receipt位于repository-scoped仓库外 SEC Runtime State；repository tree、GitHub state与disposable cache仍是不同 authority domain。
