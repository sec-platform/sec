...
MainHealth receipt绑定 exact main SHA/tree、唯一父 baseline SHA/tree、固定 image、Docker endpoint 与 affected-closure plan digest。同一 receipt 永不重跑；main 变化只让 selector重新投影 delta，不等于所有 Gate失效。selector输出的每个 Gate只因其输入 closure变化而执行，空 closure不启动测试，已有 frozen candidate Evidence则整体 carry-forward；不得用 main SHA变化、时间刷新或没有 Actions check runs作为全量重跑理由。若同一exact main/trust下的本地trusted receipt与完整hosted ledger语义冲突，不能last-writer-wins或永久locked：status、WorkSelection与repair observation只返回typed `provider-conflict`，绝不在观察函数内隐式执行Effect。唯一显式reconciliation owner先验证两端canonical bytes、provider provenance和本地文件physical preimage，从这些已认证输入编译内容寻址retirement Effect authorization，再持有该main receipt namespace的唯一物理mutation lease，发布retirement record，以device/inode/size/bytes CAS移除active本地receipt并读回absence与record。record只证明授权与preimage，不能代替删除终态；lease不可得、CAPABILITY_UNAVAILABLE或CAS失败都必须保留现状并fail closed，且CAPABILITY_UNAVAILABLE永远不能降格为absent。完成后重新解析provider集合，使hosted ledger成为唯一active observation。
