...
### 4.1 线性、仿射与动态换算不能共用一个乘数

`Quantity<Unit>`中的Unit至少说明dimension、scale/offset规则、允许运算、数值表示与适用domain。仅当转换真是固定线性比例时，才可用一个乘数。绝对温度、日历时间、货币、带基准的指数等需要不同合同，不能为了统一都压成`value * factor`。

温度特别区分**绝对量**和**差值**：摄氏温度与开尔文温度之间有固定offset，而1 °C的温差与1 K的温差大小相同。把“20 °C + 10 °C”按两个绝对温度直接相加本身就需要领域定义；不能由同单位语法自动判合法。

```text
UnitConversion =
  Linear { scale }
| Affine { scale, offset, absoluteOrInterval }
| Contextual { providerRef, contextRefs, observationTime, uncertainty }
```

货币/市场价格等contextual conversion绑定rate source、base/quote、时间、市场/法域、费用与精度；汇率变化不会改变历史Quantity本身，只会改变某次换算Observation。概率、百分比和普通无量纲比例即使底层都可表示为实数，也保留领域语义和合法范围，不能因dimensionless就自由混用。

测量值可附uncertainty/precision/resolution和测量方法；格式化的小数位数不是测量精度证明。转换、聚合和误差传播消费实际单位/不确定性合同，不能把display rounding后的值重新当原measurement。
...
### 4.1 Instant、Duration与民用时间分离

时间值按用途至少区分：

```text
Instant                 一个时间轴上的点
Duration                两个点之间或逻辑持续量
LocalDateTime           无时区的民用日历表示
ZonedCivilTime          LocalDateTime + zone + zone-rules revision + disambiguation
MonotonicReading        仅用于同一clock domain内测经过时间/期限
```

`LocalDateTime + timezone name`并不总能唯一得到Instant。时区规则切换可能产生不存在的local time（gap）或同一个local time对应两个Instant（fold）；构造必须显式`reject/earlier/later/compatible`等实际领域策略，不能悄悄选宿主默认。已经发生的事件优先保存Instant及当时使用的zone/rule来源；未来民用排程保存其civil rule和时区语义，tzdb更新后按任务政策决定重算未来触发点，而不是改写历史发生时间。

timeout、lease elapsed和性能测量优先使用适合的monotonic clock；墙钟调整、NTP校时、DST或用户改时间不能反向改变已流逝Duration。需要真实UTC/TAI/leap-second语义的领域必须声明time scale和Provider；普通“时间戳”字符串不自动具备该精度。

### 4.2 Locale、文本值与呈现规则分离

Locale不是Text identity或数值语义的隐式全局变量。语言/脚本/地域、calendar、numbering system、collation、plural rules、date/number format都属于明确`LocaleProfile`或展示/解析Provider。更换locale可以改变显示、排序和用户输入解析，但不会自动改变作者Text、symbol identity、stored number或Instant。

中立计算文本继续采用既定UTF-8与当前ASCII identifier规则；Text值不做隐式Unicode normalization。外部语言若允许Unicode identifier，由该language Provider固定其Unicode版本、allowed characters、normalization/case规则和安全限制。grapheme cluster、Unicode scalar、UTF-8 byte、UTF-16 code unit是不同坐标；用户可见“一个字符”不能代替编辑/ABI索引单位。

collation/case-fold只在明确消费它的排序/匹配中生效；不能让当前系统locale改变map key、symbol lookup、canonical encoding或hash identity。bidi、confusable和不可见字符可产生security/display diagnostics，但UI警告不得擅自把原字节规范化成另一个作者源。
