...
1. **产品收敛与发布闭环**
   ...
   - 优先切口：
     ...
     - artifact / graph / review golden output：active。
       ...
       - `platform contract ci --json [--compact]` 输出稳定 CI 命令合同，包含 demo checklist diagnostic step、每个 step 的 producesCount 与 produces。
...
- 切口 C：团队 CI：
  ...
  - 标准命令序列：
    - 状态：done。
    - `platform contract ci --json [--compact]` 输出 PR fast gate、full runtime gate、diagnostic review/explain/demo checklist 与 artifact upload 标准入口。
  - artifact 上传清单：
    - 状态：done。
