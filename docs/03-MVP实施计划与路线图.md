   ...
   - 优先切口：
     ...
     - 收敛整改总表：active。
       ...
       - P1：给 shared/types、orchestrator、cli、compiler/emit 建立边界拆分地图，先定目标后重构。
         - `shared/types.ts`：已抽离 workspace path、policy、verification、acceptance、explain、provenance/override、plan/manifest 输入合同类型并保留 barrel；后续按 lock、review、repair、upgrade 上下文继续拆分。
