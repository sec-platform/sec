   ...
   - 优先切口：
     ...
     - 收敛整改总表：active。
       ...
       - P1：补测试分层地图，区分 fast/runtime/all、contract freeze、reference drift、benchmark。
         ...
         - `platform contract ci --json [--compact]` 输出稳定团队 CI 命令合同，覆盖顶层 verify commands、顶层 quality commands、顶层 diagnostic commands、artifact upload 路径入口与聚合 produced artifact paths。
...
2. **升级迁移引擎增强**
   ...
   - 已完成：
     ...
     - 升级迁移 entry schema 校验，包含重复 migration id 的 planning 阶段阻断和 entry id/kind/path diagnostics/review/CLI 归因。
