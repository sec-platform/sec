   ...
   - 优先切口：
     ...
     - 收敛整改总表：active。
       ...
       - P1：补测试分层地图，区分 fast/runtime/all、contract freeze、reference drift、benchmark。
         ...
         - `platform contract ci --json [--compact]` 输出稳定团队 CI 命令合同，覆盖 PR fast gate、full runtime gate、顶层 quality commands、diagnostic review/explain 与 artifact upload 路径入口。
