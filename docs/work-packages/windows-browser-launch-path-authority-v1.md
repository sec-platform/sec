...
acceptance:
  ...
  - "Every direct runtime dependency is pinned to one exact numeric release by runtime-dependency-spec, and every installed package manifest must match the spec name/version before readiness is accepted. Test fixtures consume the same spec instead of inventing placeholder versions."
  - "Compiler dependency generation publication uses one bounded Windows-only rename primitive for EPERM, EACCES and EBUSY. Every retry revalidates the exact source directory identity and target absence; rollback uses the same primitive, target identity can prove an already-completed rename, and non-transient errors fail immediately."
  ...
  - "Runtime verifier behavior sentinels inject one explicit source environment and never mutate process.env. Parallel tests therefore cannot inherit a temporary isolated browser projection, while production continues to consume the real process environment."
  ...
  - "POSIX continues to use the canonical physical staging path and creates no projection; a genuine POSIX registry-derived executable relative path survives plan capture and materialization without win64 reconstruction."
