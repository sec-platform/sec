...
- V2 prior head `e027ba664faa42f341cb7f55d3e3becf24c4a911` 的唯一canonical affected在161.13秒结束为111 pass / 2 skip / 2 fail / 822 assertions。两个失败均来自snapshot-cache synthetic tests读取当前仍驻留的per-authority LRU counters；并行test file可合法插入其他authority并逐出旧settled slot，因此该oracle不稳定，不能解释为production cache失败。修复后的working tree改用每个staging root的path-free capture/revalidate/single-flight telemetry，并只以`entries - flights <= 4`检查全局settled-slot上限；active flights仍不可逐出。phase telemetry的`completed`由`finally`发布，只表示wrapper终止而非operation成功。修复后focused selection为3 pass / 1 skip / 0 fail / 62 assertions；必须冻结新head并重新运行一次canonical affected，`e027ba6`失败结果不可改名或组合为PASS。
...
A0 是全部 Gate 的唯一 `gate_owner`。新增structural-cache focused test与一次real-cache pre-child sentinel已经通过；V1 browser/runner/policy focused evidence只在intervening diff不触及其输入时复用。`e027ba6` canonical affected已因parallel-unsafe test oracle失败并使该head永久不可作为PASS证据；冻结修复后的单一candidate head后必须重新运行一次canonical affected aggregate。V1与`e027ba6`失败aggregate及任何exact-title结果永久只作诊断。affected通过后运行一次canonical Risk。Risk已经包含完整Contract Freeze、selector slow suites与workspace-fast，因此不得另跑独立Freeze；其后只运行typecheck、changed-only imports、dependency architecture、docs/scope/patch与GitNexus compare。不得运行hosted Scope/Quick/Full/release，也不得把预期 `manual-bootstrap-required` 当成失败或PASS。
