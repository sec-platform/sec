...
tracking: issue-311
base: 2bdd3526d6f42993bfab3db87b66683afa06323a
...
acceptance:
  ...
  - rolling-plan selection and non-selection transitions use one digest-bound typed machine projection union and one whole-document renderer; committed replan binds sole-parent exact HEAD/tree plus source manifest pointer and rolling raw-byte digests without copying a nested projection digest or prior prose, repairs stale derived manifest digests without weakening package tracking path or base identity, automatically refreshes when exact source-tree delta includes any non-projection path, and WorkDecision replan or MainHealth may not mutate headings prose or digest fields independently
