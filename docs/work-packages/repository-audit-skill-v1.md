...
acceptance:
  ...
  - "Every tracked path is classified; active Markdown and registered heuristic runtime surfaces have explicit Skill ownership; possible hidden behavior is reported for deterministic-versus-heuristic triage."
  ...
  - "The stale completed Work Package is archived, the new pointer/rolling plan agree on one live manifest, and the repository audit has no critical or high finding."
  ...
  - "discover-all declares its repository scope, scans platform and scripts TypeScript, uses SHA-256 and does not silently truncate call relations."
...
tests:
  - "bun test tests/contract/agent-skills.test.ts tests/contract/repository-audit.test.ts tests/contract/docs-doctor.test.ts tests/contract/test-impact.test.ts tests/contract/sandbox-architecture-contract.test.ts tests/unit/ci-pr-risk-selection.test.ts --timeout 180000"
  - "bun run typecheck"
  - "bun run docs:doctor"
