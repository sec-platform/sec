...
id: docs-document-authority-v5-reconciliation-v1
tracking: issue-132
...
acceptance:
  ...
  - "The exact candidate changes only owned documentation paths, contains no verifier trust-root, TypeScript, workflow, test, package, generated, probe, source-package, or user-root drift, and passes the selected focused active-documentation contracts, docs doctor, manifest ownership, YAML parsing, patch hygiene, and GitNexus compare."
tests:
...
# Docs Document Authority V5 Reconciliation V1
...
- Base/head：`origin/main@8aa2d2db8dd7541ddc05d9367a4070a73fa3f8a2` / candidate head 未冻结；branch `codex/docs-document-authority-v5`。
...
- GitHub：唯一开放 PR #137，Draft head `cf0183d495fbc8223fb8e216393f7f20867e653a`、base `8aa2d2d`；唯一开放 Issue #132 仅作路线导航。#137 的 runtime/V10 候选保持 suspended，不与本包共享 formal execution closure。
...
本包是当前唯一 formal Work Package。Canonical 文档由 A0 串行修改；并行 reviewer 只读。`docs/scripts/docs-doctor.ts` 属于 verifier trust root，其 semantic-superset 实现与 differential fixtures明确后置为独立候选，必须与 #137/V10 revision seam重新协调；本包恢复并保持该脚本的 latest-main bytes。
...
以下任一条件触发停止并从最新事实重算：`origin/main`、Goal九文件集合或 raw bytes变化；PR #137 merge/close/head/base或 Review/CI blocker改变依赖；Nexus exact baseline变化；必须修改 forbidden/trust-root/production path；V5 文案无法在不建立第二 owner的情况下吸收；最终 changed records超出 ownership。
