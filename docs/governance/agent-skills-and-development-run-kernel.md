...
报告 schema 为 `sec-repository-audit-v1`，至少包含 exact clean head/tree/default identity、tracked-path与表面分类计数、活动 Markdown、每个 tracked entry 唯一且稳定的 `contentCoverage` 状态/原因 ledger、逐条保留 path/line/text/Skill 的行为候选 ledger、17 个行为 owner、finding、unknown 和优化候选。默认命令对未扫描、无法证明语法、unowned 候选与 unknown fail closed；显式 diagnostic 输出不能作为 Evidence。搜索命中、抽样目录、README、PR body、单一代码图或外部 Provider 不能代替 tracked-path census。
...
该计数采用一个 primary kind：只有 `.md` 可进入 Markdown 及其细分生命周期；其余已登记 Agent 运行面优先记为 `heuristic-runtime`，再落入产品、验证、配置或普通内容。因此启发式 owner coverage 与文件扩展名不能互相冒充，某一物理类别计数为零本身也不证明遗漏。
