...
本地个人使用 profile 下，`isolated` 的 canonical 含义是 process/staging isolation：Verification-owned host 只启动仓库拥有的可信 runner，可写环境绑定 retained staging workspace，runner bundle 不引用 host source/Git，`PATH` 为空，writer lease 在 spawn 前、完整 child-tree lifecycle 中和完成后持续核对，且只有在 timeout/abort 后 child tree 已关闭、streams 已排空并完成 artifact exact binding 时才可继续。Windows 上 manifest-bound trusted absolute bootstrap 可从短 volume-root 初始 cwd 启动，但必须从自身固定入口位置推导唯一 staging root，并在任何 progress、loader 或 verifier import 前 fail closed 地切换到 retained staging cwd；其他平台直接使用 staging cwd。该 capability 不声称阻止恶意绝对路径访问或网络访问；third-party/untrusted executable capability 仍为 non-runnable。Windows AppContainer 属于可选 hardening，不是本 contract 的 isolated 必要条件。
...
v1 semantic contract source 的 `invalidationFromStage` 固定为 `resolve`；后续 stage 闭包由现有 Pipeline stage/pass registry 推导，Mutation 不维护第二张失效表。Plan 和 result 必须显示该 boundary。不能在上述 process/staging isolation 内安全执行、会产生不可逆外部副作用、需要不可信 third-party executable，或无法绑定 staged source digest 的 verifier，以 `SEMANTIC-MUTATION-010` 在 live publish 前拒绝；不能为了运行它而先提交 source。
...
SM-1 只实现 pure request normalization、condition/expectation matcher、plan/result builders/invariants、dedicated types/tests/ownership 与 `semantic.mutation` Contract Freeze；不读取 Workspace、path、YAML，不接 source adapter、live apply、Pipeline、Workbench、CLI、AI、Repair 或 Upgrade。SM-2 已实现固定 authoring index、loaded-source provenance、owner/path/adapter/edit plan/CAS/rollback manifest 与 `semantic.mutation-source-adapter` Contract Freeze。SM-3 实现 lease/staging/rebuild/Delta/Impact/Verification/publish/rollback/recovery。SM-4A 先以 additive public ingress 暴露现有 authorization normalizer/revision 与 SM-2 canonical owner-token/path-policy binding，再以一个共享 trusted product adapter 接入 Workbench/CLI 的 `add-state-transition` plan/apply/query/recover；ingress 不改变本节 contract/revision，adapter 不复制其算法。SM-4B 才冻结 Task Envelope v2，SM-4C 才接 AI Semantic Operator。三个包不得共享未冻结 DTO owner 或把 consumer 逻辑反向写入本 canonical contract。
