  ...
  if (documentationRecordByPath(registry, 'docs/governance/nexus-absorption-ledger.yaml')) {
    ...
    try {
      ...
      if (noOmissionProven) {
        ...
        if (
          typeof pathClassification.total !== 'number'
          || pathClassification.total <= 0
          || pathClassification.classified !== pathClassification.total
          || eprBindings.bound !== eprBindings.expected
          || skillBindings.bound !== skillBindings.expected
          || acceptedParity.proven !== acceptedParity.accepted
          || coverage.unexplainedDeltaCount !== 0
        ) {
