...
本文保存 base 2065492 中旧 active CI 文档的无损文本快照，只用于审计历史判断。当前 lane、Gate、Evidence、Actions 经济性与复用规则只以 `docs/test-feedback-and-ci-lanes.md` 和代码合同为权威；本文件不得参与当前选择或证明新 head。

## 原始快照

以下引用块逐行来自 `2065492f55bab75f65cf124c048fc6b849be73f9:docs/test-feedback-and-ci-lanes.md`。非空行去除 `> `、空行去除 `>` 即可恢复原 Git blob 文本；引用前缀同时防止历史标题和合同被误识别为 active authority。
