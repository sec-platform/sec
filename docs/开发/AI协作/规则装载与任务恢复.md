...
本仓库 `check:affected` 的编辑反馈先由 Git authority 签发准确 `HEAD`、index、worktree、Provider 与 changed-path 观察，再决定是否需要 Source Program。changed-path 集为空时，`check:affected` 形成 `not-applicable` 计划，保留 Git identity 并把 source observation identity 明确记为不存在；它不为了证明空集而构造 TypeScript Program、测试投影或第二套空模型，后续真实 Effect 仍须按原边界重读 Git。全部 changed-path 均属文档验证且不属于 Source Program/tool 输入时，同样只签发 Git-bound `docs:doctor` 计划，不构建 ProjectInput、Source Program 或测试投影；`.documentation/documents.json` 与 `baseline.json` 仍走语义路线。其他非空变更必须将同一进程内签发的 Git selection 与原 session、base 和 transition 绑定后才能进入完整 Source Program；普通对象、路径摘要或调用方声称的文档列表不能触发快速路径。

昂贵验证只对准确对象生产一次。相同 source tree、tsconfig、依赖 generation、compiler/provider 与环境的受信 typecheck PASS 按既有 ActionKey 回读；任一输入变化才重新执行 compiler。当前用户授权的临时编辑期命令 `bun run typecheck` 直接运行仓库锁定的 `@typescript/native` TypeScript 7 原生编译器，而不是 `typescript` 6 的 JS `tsc`；它复用仓库外、按 checkout 与原生版本分离的 build-info，并打印进度与编译器耗时。本地 `check:affected` 所选的 typecheck 步骤调用同一原生入口，不再暗中执行旧受信 checker；源码影响选择仍须独立完成准确证据。该命令只报告该次编辑反馈，不签发 VerificationAction、Git/依赖 authority 或最终 Gate。`bun run typecheck:verified` 保留原受信 owner，`check:full` 明确消费该入口；原生缓存文件不进入候选或交付。Repository Audit 同时保留当前绝对库存和 transition enforcement：unknown、candidate、dominance、test finding 与 architecture violation 的绝对集合供迁移和审查，不得在 exact self-comparison 中再次冒充本候选新增失败；阻断只消费准确 before/after 的 finding delta、reconciliation、architecture evolution、test retirement 与 supersession receipt。比较缺失、不可比、观察退化或 receipt 未决仍 fail closed，不能借“历史存量”规则把真实新增回归放行。

当前性能欠账须由既有 affected selector 与 Source Program owner 消除：除准确空集和纯文档闭包外，`check:affected --plan` 仍会取得全工作树快照并构建完整 TypeScript Program；执行模式仍在零写入选择围栏外先准备依赖，不能把可能写入的 materialization 移进围栏来换取表面速度。受信 typecheck 为求 ActionKey 又先解析模块，cache miss 后重新构造 Program。阶段日志分别记录 Git selection、依赖代际、快照、ProjectInput、Source Program、fact identity 和 checker 的耗时。受信 typecheck 的快键若改用保守输入上界，必须覆盖新增、删除、改名、负依赖解析、嵌套 package/tsconfig 与依赖 generation，并在 miss 时以真实 Program 反核；不能以已知 import 列表、路径后缀或裸 `tsc` 创造第二权威。这项性能优化只完成纯文档的 Source Program 跳过，其余路径未完成；临时原生编辑反馈不替代最终受信验证。
...
仓库远端维护不把 PR/Issue 评论本身当 Effect owner。默认分支上的 `repository-maintenance` workflow 只是受限 hosted transport：当前只接收 `sec-repository-maintenance-request-v1` 的 `exact-ref-retirement` 或 `exact-comment-retirement`；**一条评论只允许一个操作、一个 exact subject**，请求必须绑定仓库与准确 `expectedMainSha`。ref lane 中，transport 只允许单个 `transport/*` ref，正式代码 ref 必须绑定一个准确的 closed PR、原 head branch 与原 head SHA；comment lane 只允许 `commentId + issueNumber + raw body SHA-256` 精确绑定，并由 runtime 根据实际评论内容判定其是否属于封闭的 disposable 集合，调用方不能用 classification 字段扩大可删除类型。任意批处理、任意 ref/path/comment、force push、raw REST/GraphQL delete、分支名前缀 GC、按年龄删除以及把“closed PR/旧评论”本身解释为内容已无价值，都不属于这条合同。

Hosted 准入同时绑定 `issue_comment`、生命周期 Issue #313、默认分支上的准确 workflow revision、评论作者与 `GITHUB_ACTOR`。GitHub 事件中的 `author_association` 只用于缩小触发面，不能代替仓库权限；真正签发 destructive capability 前，GitHub API owner 必须重新读取该 actor 对当前仓库的 collaborator permission，并要求 `maintain|admin`。Actions token 只可由这一个 workflow 身份用于 `read`、`branch-closeout-write` 与 `issue-comment-write`；后者仍被 GitHub API owner 限定为固定 comment observation/create/update/delete surface，不能取得 merge/status/ref/runner 权限。CodeQL projection workflow 仍只拥有它自己的 read/comment 边界，不能继承 repository-maintenance 的 branch closeout 或 comment-retirement policy。Hosted runner 在进入维护 runtime 前必须按锁文件执行 `bun install --frozen-lockfile --ignore-scripts`，不得依赖预存 `node_modules` 或 lifecycle script。

### 评论运输垃圾退役

Issue/PR Conversation 中的评论只有在仍承担工程证据、当前裁决、review finding 或长期可读投影时才保留。下列对象属于可退役的运输噪声，而不是历史 authority：

- 已执行完成的 #313 `sec-repository-maintenance-request-v1` 触发评论；只有其 operation target 已按各自 owner 证明 terminal/absent 时才允许删除；
- 只用于触发 Codex review 的精确 `@codex review` / `@codex security review` 命令评论；该类命令只在所属 PR 已 closed 时退役；
- closed PR 上由 `chatgpt-codex-connector[bot]` 生成的 `codex-pull-request-review-summary` 投影，以及仅提示“为此仓库创建 Codex environment”的 setup transport 消息；真实 review submission、inline finding/thread 不随这些投影删除；
- `chatgpt-codex-connector[bot]` 产生的 code-review usage-limit 提示。

CodeQL projection、真实 review finding、人工裁决、事故/失败复盘以及仍被 current owner 引用的 evidence 不在 disposable 集合中。`exact-comment-retirement` 不接受调用方提供“垃圾类型”；runtime 必须重新读取 comment、核 `issueNumber/commentId/raw body digest/author`，再由封闭 classifier 判断。maintenance-trigger 还必须从旧请求体恢复其 target branch，并在删除评论前重新确认该 ref 已 absent。Comment DELETE 后必须读取 404 作为 settlement。

maintenance workflow 自己产生的 #313 触发评论不允许无限累积。每次 operation 先把原始 request、terminal result 与可用 recovery 一起复制到受控 artifact staging 并成功上传；**只有 artifact upload 成功后**，才重新读取并删除本次 trigger comment。上传、runtime 或 readback 任一步失败时 trigger 保留，作为未结算 transport locator。这样删除评论不会删掉唯一的恢复/审计信息，也不会让“清评论”本身继续制造新的垃圾评论。
...
→ present ref 创建并验证 Git bundle recovery
→ effect boundary 再读 live main / open PR / exact ref old OID
→ GitHub updateRefs exact old-OID CAS
→ ref absence + 完整 open-PR census + live main readback
→ terminal result
```

`invalid|unresolved` 的 active Work Package、关键 repository/default/remote/worktree inventory 未知、main 漂移、PR 身份/consumer 漂移、ref SHA 漂移或 recovery 无法验证都 fail closed。CAS 已成功而客户端响应或后续结果丢失时，下一次相同请求必须先独立读取 closed PR 与 ref：ref 已缺席时不重放删除，但 closed-PR lane 仍从准确 `refs/pull/<n>/head` 重建并验证 recovery bundle，再以 `already-absent` 结算，因此第一次 Action 在删除后丢失 artifact 也可通过同一 operation 身份恢复持久证据。若删除后才观察到新开/重开的 PR 消费者或 main 已变化，物理删除不能冒充完整成功；该 attempt 进入 unsettled，保留/上传本次 bundle 与结果证据，由原 owner 依据新事实恢复或重新裁决。

```mermaid
stateDiagram-v2
  [*] --> Requested: #313 exact request
  Requested --> Blocked: 身份/maintainer/main/active-work/PR/inventory不满足
  Requested --> AlreadyAbsent: ref缺席且准确PR/策略仍成立
  Requested --> Recovered: ref存在且verified bundle已建立
  Recovered --> Blocked: effect-boundary前像或consumer发生变化
  Recovered --> Deleted: exact old-OID CAS成功
  Deleted --> Settled: ref缺席且main/PR readback仍一致
  Deleted --> Unsettled: 删除后出现consumer/main漂移或回读未知
  Unsettled --> Requested: 保留bundle后按新事实重编译
  AlreadyAbsent --> Settled
  Settled --> [*]
```

`repository-maintenance` 不取得 branch-lifecycle 的内容处置权，也不生成“所有 closed branch 都可删”的推断；它只运输已经由维护者明确分类并绑定 exact identity 的请求。branch-lifecycle 继续拥有 recovery、ref 前像、CAS、竞争检查和终态回读。Recovery root 只能来自 owner 的显式参数或由 repository root 推导的固定 sibling root；branch-lifecycle 不从环境变量取得路径 authority，bundle leaf 也不包含用户提供的 branch/ref 字段。单 ref 事务、两页 open-PR 上限与各子动作既有 timeout 共同保证 runtime 在 hosted job hard timeout 前结算，从而给 `if: always()` 的 recovery artifact 上传保留窗口。Workflow 不把含 `..` 的 sibling 路径直接交给 `upload-artifact`：先把 owner 的 canonical recovery store 复制到 `$RUNNER_TEMP` 下的正规 staging 目录，再上传该目录；Action artifact 只是恢复/结果运输，不是新的长期事实源。
...
当前 Git 物理 owner 对 `refs/heads/*` 的本地删除明确返回原子性能力缺失：Git 的 ref 前像比较不能与并发 worktree 绑定检查原子化。若准确 inventory 仍需本地删除，closed-unmerged closeout 必须在 effect-start 和远端删除之前阻断并保留 refs、bundle 与恢复责任，不能先删远端再把必然失败写成偶发残留；没有本地分支时仍可按独立准入结算。这一能力缺口未修复前，上段所述本地分支删除后的 recovery 退役条件不是当前可自动到达的路径。
