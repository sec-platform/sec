...
仓库远端维护不把 PR/Issue 评论本身当 Effect owner。默认分支上的 `repository-maintenance` workflow 只是受限 hosted transport：当前只接收 `sec-repository-maintenance-request-v1` 的 exact-ref-retirement，请求必须绑定仓库、准确 `expectedMainSha` 与 1..64 个有界操作；transport lane 只允许 `transport/*`，重复别名只允许 `transport/*` 与 `work/*` 的同 SHA 集合，正式代码 ref 必须绑定一个准确的 closed PR、原 head branch 与原 head SHA。任意 ref/path、force push、raw REST/GraphQL delete、分支名前缀 GC、按年龄删除以及把“closed PR”本身解释为内容已无价值，都不属于这条合同。

Hosted 准入同时绑定 `issue_comment`、生命周期 Issue #313、默认分支上的准确 workflow revision、评论作者与 `GITHUB_ACTOR`。GitHub 事件中的 `author_association` 只用于缩小触发面，不能代替仓库权限；真正签发 destructive capability 前，GitHub API owner 必须重新读取该 actor 对当前仓库的 collaborator permission，并要求 `maintain|admin`。Actions token 只可由这一个 workflow 身份用于 `read` 与 `branch-closeout-write`；CodeQL projection workflow 仍只拥有它自己的 read/comment 边界，其他 workflow/effect 不继承权限。
...
→ 完整 open-PR census：target 既不是任何 open PR 的 head，也不是 base
...
`repository-maintenance` 不取得 branch-lifecycle 的内容处置权，也不生成“所有 closed branch 都可删”的推断；它只运输已经由维护者明确分类并绑定 exact identity 的请求。branch-lifecycle 继续拥有 recovery、ref 前像、CAS、竞争检查和终态回读，Action artifact 只是恢复/结果运输，不是新的长期事实源。
