...
`invalid|unresolved` 的 active Work Package、关键 repository/default/remote/worktree inventory 未知、main 漂移、PR 身份/consumer 漂移、ref SHA 漂移或 recovery 无法验证都 fail closed。CAS 已成功而客户端响应或后续结果丢失时，下一次相同请求必须先独立读取 closed PR 与 ref：ref 已缺席就是可验证的 `already-absent` 终态，不要求历史 PR 仍因物理 ref 存在而出现在普通 inventory 中，也不重放删除。若删除后才观察到新开/重开的 PR 消费者或 main 已变化，物理删除不能冒充完整成功；该 attempt 进入 unsettled，保留/上传本次 bundle 与结果证据，由原 owner 依据新事实恢复或重新裁决。
