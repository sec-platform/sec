...
仓库远端维护不把 PR/Issue 评论本身当 Effect owner。默认分支上的 `repository-maintenance` workflow 只是受限 hosted transport：当前只接收 `sec-repository-maintenance-request-v1` 的 exact-ref-retirement；**一条评论只允许一个操作、一个 exact ref**，请求必须绑定仓库与准确 `expectedMainSha`。transport lane 只允许单个 `transport/*` ref；正式代码 ref 必须绑定一个准确的 closed PR、原 head branch 与原 head SHA。任意多 ref 批处理、任意 ref/path、force push、raw REST/GraphQL delete、分支名前缀 GC、按年龄删除以及把“closed PR”本身解释为内容已无价值，都不属于这条合同。

Hosted 准入同时绑定 `issue_comment`、生命周期 Issue #313、默认分支上的准确 workflow revision、评论作者与 `GITHUB_ACTOR`。GitHub 事件中的 `author_association` 只用于缩小触发面，不能代替仓库权限；真正签发 destructive capability 前，GitHub API owner 必须重新读取该 actor 对当前仓库的 collaborator permission，并要求 `maintain|admin`。Actions token 只可由这一个 workflow 身份用于 `read` 与 `branch-closeout-write`；CodeQL projection workflow 仍只拥有它自己的 read/comment 边界，其他 workflow/effect 不继承权限。Hosted runner 在进入维护 runtime 前必须按锁文件执行 `bun install --frozen-lockfile --ignore-scripts`，不得依赖预存 `node_modules` 或 lifecycle script。
