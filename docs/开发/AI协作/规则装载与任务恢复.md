...
提交 journal 只拥有未交付结果的恢复责任，不是提交历史归档。`development.commit` 通过 Git 对象与准确 ref transition 独立读回结果，再把同进程 owner-issued result 交给调用者；调用者接收结果后，由同一 owner 核对 journal 原始字节并退役已应用记录。明确未应用的结果交付后，只有同一 owner 重新取得准确未应用观察，才可确认结束并退役该尝试；需要重试时必须在结束确认前消费原恢复入口。CLI 打印、caller 重建 JSON 和 terminal 字段均不能签发退役权限；未交付、未确认结束及未知记录保留原恢复入口，退役失败不能抹掉已确认的提交效果。恢复族的数量上限在新的提交作用前检查，不能提交生效后才因超限丢失可恢复结果。后续提交或 checkout 不撤销已经发生的 ref transition，恢复观察不得用当前 index 或 reflog 的当前位置否定历史应用事实。退役复用 Runtime State 的 retained no-follow 文件身份与 mutation lease，不新增清理脚本、备份目录、版本或平行日志。旧分支删除前，分支 owner 须先结算该 ref 的提交恢复消费者；已失去可重建 transition 的历史记录只能由准确绑定的分支终态接管并退役，不能据内容相似或文件年龄自行认定应用成功。
...
flowchart LR
  ...
  Terminal -->|not-applied；owner重新确认未应用并结束| Retire
  Terminal -->|未确认结束／unknown| Recovery
...
当前入口只实现 closed PR 的 closed-superseded 处置，不为没有生产消费者的 evidence-close 或 PR-close 保留工厂、状态分支与专属测试。初次 preparation 可以消费一次完整观察；每个作用围栏只重新取得目标 PR、准确 main／目标引用、相关 worktree 注册、活动工作与恢复事实，不重列其他 PR、评论和无关 worktree 的文件状态，也不让它们的未知污染目标处置。
...
已有准确终态后的物理收敛不重新签发旧分支的删除权限，也不要求 main 永远停留在当时审查的 SHA。终态继续绑定原 review／preparation／receipt，当前观察只核仓库与 PR 身份、目标引用和 worktree 缺席、相关活动工作与恢复消费者已结束以及恢复对象原始身份；main 前进或其他分支上的活动任务不能复活旧消费者。当前观察未知、目标消费者重现或身份漂移仍阻断；没有既有准确终态时，原 main 前像和全部作用准入保持严格有效。
...
远端删除和 tracking-ref 退役不销毁本地 reflog，不提前消费本地提交 journal；存在的本地 ref 在删除前由 commit owner 结算。若本地 ref 已缺席，历史 journal 保留到分支的准确终态与 live readback 完成，由分支协调器消费同一 owner-issued completion 与验证过的 recovery bundle 后，再要求 commit owner 独立核对准确 PR head、对象／parent／tree、ancestry 和完整 journal family。物理 requirement context 只约束作用交接与资源，不替代该终态权限。未知 journal 或任何身份漂移均在首次删除前保留全族；journal 接管失败时 bundle 不得先退役。终态持久保留原 preparation，部分清理将 preparation 最后删除；重入继续原操作，不生成新 bundle 来冒充恢复。
...
flowchart LR
  ...
  Fence -->|是| Start["短 Provider 会话发布并读回 exact Effect-start"]
  Start --> CAS["目标 scoped 围栏；远端／tracking ref CAS"]
  CAS -->|不确定| Preserve
  CAS --> Local{"本地 ref 存在？"}
  Local -->|是| CommitDrain["commit owner 结算 journal；重新围栏后本地 CAS"]
  CommitDrain -->|未知或未完成| Preserve
  CommitDrain -->|已结算| Terminal["终态绑定 review／preparation；发布后再次 live readback"]
  Local -->|否| Terminal
  Terminal --> Takeover["分支 completion＋验证 bundle；commit owner 接管残余 journal"]
  Takeover -->|未知或漂移| Preserve
  Takeover -->|全部结算| Retire["同 owner 退役 bundle family；preparation 最后删除"]
```
