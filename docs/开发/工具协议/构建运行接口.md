...
<a id="bsp-build-provider-projection"></a>
## BSP作为Build Provider投影，不成为Target authority

Build Server Protocol可用于让JetBrains/VS Code/语言服务器等客户端复用现有build tool信息，但BSP的workspace/target/task/status是外部Provider协议对象。SEC内部仍以真实workspace selection、CompileRequest/Action、Artifact、Test/Run、ProviderSession和Evidence为owner，不建立第二build graph。

```text
BspSessionProjection = derived {
  providerSessionAndRevision,
  bspProtocolVersionAndCapabilities,
  initializeRootAndWorkspaceMapping,
  observedBuildServerGeneration,
  buildTargetMappings,
  sourceResourceDependencyOutputMappings,
  diagnosticProducerMappings,
  taskAndOriginIdMappings,
  supportedOperationsAndLosses
}
```

每个 `BuildTargetIdentifier` 只在当前BSP server/session/state中定位build target。映射到SEC时固定：实际workspace/root、server revision/config state、对应作者/原生build target关系、目标平台/语言、sources/resources/dependencies/output paths及coverage。相同BSP target URI跨server restart、workspace reload或配置切换不自动表示相同SEC Target/Artifact。

### 查询与执行分开

`workspace/buildTargets`、sources、inverseSources、dependencySources/modules、resources、outputPaths等首先作为Provider query observation进入information/build projection；它们不修改作者源。`workspace/reload`会改变build server内存状态，是Provider session control而不是纯读取；reload失败后若server继续保留previous known-good state，SEC继续绑定那一代而不是假装已经切到新配置。

`buildTarget/compile`、test、run和debug属于真实build/test/run operation。BSP `originId`只关联请求，`TaskId`只关联协议task tree；都不能替代SEC request/operation/attempt identity。StatusCode.Ok只说明BSP定义的请求执行成功，不能单独证明：目标artifact是哪份、diagnostics全为空、tests满足GIFT Claim、run达到用户目标或外部Effect已结算。适配器必须捕获实际target set、server state、task notifications、outputs/diagnostics及必要readback，再按各owner接纳。

### Diagnostics按text document与build target保留来源

BSP允许一个text document属于多个build targets，`build/publishDiagnostics`的reset也按同一textDocument+buildTarget清理/追加。因此GIFT不能用“文件路径”作为唯一diagnostic stream。每条导入诊断至少绑定BSP provider/session、build target mapping、source revision/coordinate mapping、origin/task（有则）和reset generation；target A发空reset不能清掉target B的诊断，旧session迟到通知不能覆盖新server generation。

BSP diagnostic、task log和showMessage都是Provider observations；任意message/data字段不当作命令。环境查询可能返回classpath、env或启动信息，其中secret/credential继续受披露策略，不能因为IDE请求run environment就把全环境回显。

### 客户端互操作

客户端可以通过BSP获得项目树、编译/测试/运行入口，但真正的GIFT authoring-assist仍消费当前candidate/selection；build server源列表不自动覆盖未保存buffer。需要rename前compile时，compile结果与正在编辑的candidate revision必须明确对应，否则只作为旧build observation。BSP不可用时，GIFT自身CLI/SDK/build Provider仍可工作，不能把BSP变成产品前置。
